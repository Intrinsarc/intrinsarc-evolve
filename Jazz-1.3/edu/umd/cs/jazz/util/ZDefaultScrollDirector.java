...
/**
 * The default implementation of the scroll director that follows the widely 
 * accepted model of scrolling - namely the scrollbars control the movement
 * of the window over the document rather than the movement of the document
 * under the window.
 *
 * @author Lance Good
 */
public class ZDefaultScrollDirector implements ZScrollDirector, ZCameraListener, ZNodeListener {
	...
	if (camera != null) {
	    ...
	    pos.setLocation((int)(viewBounds.getX()-layerBounds.getX()+0.5),
			    (int)(viewBounds.getY()-layerBounds.getY()+0.5));
	}
     ...
     * @param e The view changed camera event
     */
     ...
     * @param e The bounds changed event
     */
    ...
    /**
     * Invoked when a node has its global bounds changed.  These can
     * be ignored since a global bounds change will have a corresponding
     * bounds change on the root
     * @param e The ignored global bounds event
     */
    public void globalBoundsChanged(ZNodeEvent e) {
    }    

    /**
     * Should the ScrollPane be revalidated? - This occurs when
     * either the scrollbars are showing and should be remove or
     * are note showing and should be added
	...
	if (camera != null) {
	    ...
	    ZBounds cameraBounds = camera.getBoundsReference();
