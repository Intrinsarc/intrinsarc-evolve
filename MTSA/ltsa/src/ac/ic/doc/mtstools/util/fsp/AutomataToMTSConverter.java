...
public class AutomataToMTSConverter {
	...
	public MTS<Long, String> convert(CompactState automata) {
		...
		this.mts = new MTSImpl<Long, String>(modelConverterUtils.rank(automata
				.START()));
	...
	private void addTransitions(CompactState automata) {
		...
		while (!toProcess.isEmpty()) {
			...
			MyList transitions = automata.getTransitions(modelConverterUtils
					.unrank(actualState));
			while (!transitions.empty()) {
				...
				mts.addTransition(actualState, indexToAction[transitions
						.getAction()], // aca podria guardar de donde viene cada
						// action
						rank, indexToTransitionType[transitions.getAction()]);
