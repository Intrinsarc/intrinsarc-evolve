...
public class DrawMachine {
	...
	public DrawMachine(CompactState m, JPanel p, Font fn, Font fl, boolean dn,
	...
	protected Dimension computeDimension(CompactState m) {
		...
		for (int i = 0; i < m.maxStates; i++) {
			...
			while (p != null) {
				...
				while (tr != null) {
					...
					int diff = tr.next - i;
					if (diff > maxFwd
							|| (diff == maxFwd && ntrans[tr.next + 1] > maxFwdLabels)) {
						maxFwd = diff;
						fwdToState = tr.next + 1;
						fwd = true;
					}
					if (diff < maxBwd
							|| (diff == maxBwd && ntrans[tr.next + 1] > maxBwdLabels)) {
						maxBwd = diff;
		...
		int pwidth = errorState == 0 ? 10 + nameWidth + STATESIZE + endWidth
				+ (m.maxStates - 1) * SEPARATION : 10 + STATESIZE + endWidth
				+ m.maxStates * SEPARATION;
	...
	public void draw(Graphics g) {
		...
		if (m.maxStates > MAXDRAWSTATES) {
			...
			g.setFont(nameFont);
			g.drawString(m.name + " -- too many states: " + m.maxStates, topX,
					topY + 20);
		} else {
			// display name
			g.setFont(nameFont);
			...
			g.setColor(Color.black);
			if (displayName)
				g.drawString(m.name, zeroX - nw, zeroY - 5);
			...
			for (int i = 0; i < m.maxStates; i++) {
				...
				while (p != null) {
					...
					if (event.charAt(0) != '@')
						while (tr != null) {
							ntrans[tr.next + 1]++;
							drawTransition(g, i, tr.next, event,
									ntrans[tr.next + 1], i == lastselected
											&& tr.next == selected
					...
					if (event.charAt(0) != '@')
						while (tr != null) {
							...
							if (!newLabelFormat) {
								drawTransition(g, i, tr.next, event,
										ntrans[tr.next + 1], i == lastselected
												&& tr.next == selected, true);
							} else {
								if (ntrans[tr.next + 1] == 1)
									drawTransition(g, i, tr.next,
											labels[i + 1][tr.next + 1],
											ntrans[tr.next + 1],
											i == lastselected
													&& tr.next == selected
	...
	private void initCompactLabels() {
		...
		for (int i = 0; i < mach.maxStates; i++) {
			EventState current = EventState.transpose(mach.states[i]);
			while (current != null) {
				String[] events = EventState.eventsToNextNoAccept(current,
						mach.alphabet);
