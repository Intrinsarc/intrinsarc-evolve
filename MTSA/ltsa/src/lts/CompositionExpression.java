	...
	CompositeState compose(Vector actuals) {
		...
		for (Enumeration e = machines.elements(); e.hasMoreElements();) {
			...
			if (o instanceof CompactState)
				...
				// There is no On-The-Fly model checking algorithm for MTSs
				// yet
				assert (cs.getCompositionType() != -1);
				CompositeState toCompose = new CompositeState(cs.getName(), cs
						.getMachines());
				toCompose.setCompositionType(cs.getCompositionType());
				toCompose.setFlags(cs);
		...
		if (c.makeController) {
			ControllerGoalDefinition pendingGoal = ControllerGoalDefinition
					.getDefinition(goal);
			ControllerGoal goal = GoalDefToControllerGoal.getInstance()
					.buildControllerGoal(pendingGoal);
			// DIPI: refactor, add the composite to the builder and encapsulate
			// this code.
			for (Symbol safetyDef : pendingGoal.getSafetyDefinitions()) {
				CompactState compileConstraint = AssertDefinition
						.compileConstraint(output, safetyDef.getName());
				Validate.notNull(compileConstraint, "LTL PROPERTY: "
						+ safetyDef.getName() + " not defined.");
				// DIPI: temporal. To remove.
				MTS<Long, String> convert = AutomataToMTSConverter
						.getInstance().convert(compileConstraint);
				convert.removeAction("@" + safetyDef.getName());
				compileConstraint = MTSToAutomataConverter.getInstance()
						.convert(convert, safetyDef.getName());
				if (!c.machines.contains(compileConstraint)) {
					c.machines.add(compileConstraint);
				}
			}
			c.goal = goal;
		}
