...
public class CompactState implements Automata {
	...
	private void relational_relabel(Relation oldtonew) {
		...
		for (int i = 1; i < alphabet.length; i++) {
			...
			if (o != null) {
				...
				if (o != null) {
					if (o instanceof String) {
						na.setElementAt(((String) o)
								+ alphabet[i].substring(prefix_end), i);
					} else { // one - to - many
						Vector v = (Vector) o;
						na.setElementAt(((String) v.firstElement())
								+ alphabet[i].substring(prefix_end), i);
						for (int j = 1; j < v.size(); ++j) {
	 ...
	 * sequence of events ocurring from the given set of states. If stopOnError
	...
	BitSet nfaSimulation(List<String> word, List<String> wordAlphabet,
		...
		for (String event : word) {
			...
			if (!alphaMap.containsKey(event))
				...
				try {
					state = nfaClosure(nfaNextState(state, alphaMap.get(event),
	...
	public void setGoal(ControllerGoal goal) {
		this.goal = goal;
	}

	public ControllerGoal getGoal() {
		return goal;
	}
}