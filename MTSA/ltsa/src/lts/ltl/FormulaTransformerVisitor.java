...
/**
 * Visits a formula and stores the resulting transformed Formula and the Fluents
...
public class FormulaTransformerVisitor implements FormulaVisitor {
	...
	private Fluent createFluent(Proposition proposition) {
		...
		boolean initialValue = (predicateDefinition.initial() == 1) ? true
				: false;

		Set<Symbol> initiatingActions = this
				.transformFluentActions(predicateDefinition
						.getInitiatingActions());
		Set<Symbol> terminatingActions = this
				.transformFluentActions(predicateDefinition
						.getTerminatingActions());

		return new FluentImpl(name, initiatingActions, terminatingActions,
				initialValue);
	}
	...
	private Set<Symbol> transformFluentActions(Vector fluentActions) {
		...
		for (Object fluentAction : fluentActions) {
			// The fluentAction must have a toString method representing the
			// action
