...
public class FormulaSyntax {
	...
	private FormulaSyntax(FormulaSyntax lt, Symbol op, FormulaSyntax rt,
			Symbol prop, ActionLabels r, ActionLabels a, Vector v) {
		left = lt;
		right = rt;
		operator = op;
		proposition = prop;
	...
	public Formula expand(FormulaFactory fac, Hashtable locals,
			Hashtable globals) {
		if (proposition != null) {
			if (range == null) {
				if (PredicateDefinition.definitions != null
					...
					AssertDefinition p = (AssertDefinition) AssertDefinition.definitions
	...
	private Vector paramValues(Vector paramExprs, Hashtable locals,
			Hashtable globals) {
		if (paramExprs == null)
			return null;
	...
	public boolean isPropositionalLogic() {
		...
		if (this.operator != null) {
			...
			} else if (opKind == Symbol.AND || opKind == Symbol.OR
					|| opKind == Symbol.EQUIVALENT || opKind == Symbol.ARROW) {
				return this.right.isPropositionalLogic()
						&& this.left.isPropositionalLogic();
			} else {
				return false;
			}
