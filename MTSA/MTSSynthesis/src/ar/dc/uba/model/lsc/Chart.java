...
/**
 * Sequence Chart. The generated language is the chart�s set of linearisations
 * 
 * @author gsibay
 * 
 */
public class Chart {
	...
	private static Predicate<UniqueLocation> IS_CONDITION_UNIQUE_LOCATION_PREDICATE = new TransformedPredicate<UniqueLocation, Location>(
			TO_LOCATION_TRANSFORMER, Location.isConditionPredicate);

	/**
	 * Locations can be interactions or conditions. This list defines a partial
	 * order. All the algorithms assume that there can be, at least, one
	...
	public Chart(List<Location> locations,
			LocationNamingStrategy locationNamingStrategy)
			throws ChartLanguageGenerationException {

		// The location's name may not be unique. However when building the
		// partial order (and using it)
		// a unique identifier is necessary. The position is used to get a
		// unique identifier for a Location.
		this.uniqueLocations = Collections.unmodifiableList(this
				.createChartUniqueLocations(locations));
	...
	public Set<Condition> getConditions() {
		...
		if (this.cachedConditions == null) {
			...
			for (UniqueLocation uniqueLocation : this.uniqueLocations) {
				if (IS_CONDITION_UNIQUE_LOCATION_PREDICATE
						.evaluate(uniqueLocation)) {
					// ConditionLocation are only used for the chart.
					// Condition is used for other operations outside the chart
					ConditionLocation conditionLocation = (ConditionLocation) uniqueLocation
							.getLocation();
					conditions.add(this
							.conditionLocationToCondition(conditionLocation));
				}
			}
	...
	public Set<Symbol> getInteractionSymbols() {
		...
		for (Interaction interaction : interactions) {
			result.add(new SingleSymbol(interaction
					.getName(this.locationNamingStrategy)));
		}
		return result;
	}
	...
	public Set<Interaction> getInteractions() {
		...
		CollectionUtils.select(this.getUniqueLocations(), NotPredicate
				.getInstance(IS_CONDITION_UNIQUE_LOCATION_PREDICATE),
				uniqueLocationInteractions);

		Set<Interaction> interactions = new HashSet<Interaction>();
		Iterator<UniqueLocation> uniqueLocationInteractionsIt = uniqueLocationInteractions
				.iterator();
		while (uniqueLocationInteractionsIt.hasNext()) {
			UniqueLocation uniqueLocationInteraction = (UniqueLocation) uniqueLocationInteractionsIt
					.next();
			interactions.add((Interaction) uniqueLocationInteraction
					.getLocation());
		}
		return interactions;
	}
	...
	public List<Location> getLocations() {
		...
		CollectionUtils.collect(this.getUniqueLocations(),
				TO_LOCATION_TRANSFORMER, locations);
		return locations;
	}
	...
	private Set<List<Location>> calculateLinearisations()
		...
		BinaryRelation<UniqueLocation, UniqueLocation> lowerThanPO = BinaryRelationUtils
				.getInverseRelation(greaterThanPO);

		// Get the starting symbols (the symbols that can be at the start of a
		// word)
		Set<UniqueLocation> firsts = this.getStartingLocationsNotViolatingASAP(
				uniqueLocations, greaterThanPO);

		Set<List<UniqueLocation>> wordsAsUniqueLocation = new HashSet<List<UniqueLocation>>();

		// for every possible first symbol, calculate the possible continuations
		// and add them
		for (UniqueLocation first : firsts) {
			List<UniqueLocation> word = new LinkedList<UniqueLocation>();
			word.add(first);
			Set<UniqueLocation> remainingLocations = new HashSet<UniqueLocation>(
					uniqueLocations);
			remainingLocations.remove(first);
			wordsAsUniqueLocation.addAll(this.getContinuations(word,
					greaterThanPO, lowerThanPO, remainingLocations));
		}

		// The set of word of UniqueLocations have to be transformed into a
		// linearisation
		for (List<UniqueLocation> wordAsUniqueLocation : wordsAsUniqueLocation) {
			List<Location> linearisation = new LinkedList<Location>();

			CollectionUtils.collect(wordAsUniqueLocation,
					TO_LOCATION_TRANSFORMER, linearisation);
	...
	/**
	 * Transforms the Location to a Symbol according to the
	 * locationNamingStrategy
	 * 
	 * @param location
	 * @return
	 */
	...
	/**
	 * A chart location can have locations with the same name. The position and
	 * the name uniquely defines a positioned location. This method returns a
	 * List (in the same order as the original) with this pair representation of
	 * positioned location
	 * 
	 * @return
	 */
	private List<UniqueLocation> createChartUniqueLocations(
	...
	/**
	 * From the set of linearisation of this chart it generates and returns the
	 * set of operational words. The word contains CanonicalSymbol.
	 * 
	 * @return
	 */
	public Set<Word> calculateOperationalLanguageWords() {
		...
		for (List<Location> linearisation : linearisations) {
			words.add(this
					.transformLinearisationToOperationalWord(linearisation));
		}
	...
	/**
	 * From a linearisation creates an operational word. The first symbol is
	 * always epsilon, it will have conditions if there are initial conditions.
	 * 
	 * @param linearisations
	 * @return
	 */
	private Word transformLinearisationToOperationalWord(
			List<Location> linearisation) {
		...
		while (iterator.hasNext() && currentInteraction == null) {
			...
			if (Location.isConditionPredicate.evaluate(location)) {
				currentConditionsNames.add(((ConditionLocation) location)
						.getConditionName());
			} else {
		...
		if (currentInteraction != null) {
			...
			while (iterator.hasNext()) {
				...
				if (Location.isConditionPredicate.evaluate(location)) {
					...
					// create the symbol for the current interaction and the
					// conditions
					symbol = this.transformToSymbol(currentInteraction);
					symbols.add(new CanonicalSymbol(symbol,
							currentConditionsNames));
			...
			// There are no more locations. Create the symbol with the last
			// interaction
			// and the conditions
			symbol = this.transformToSymbol(currentInteraction);
	...
	private Condition conditionLocationToCondition(
			ConditionLocation conditionLocation) {
		return new ConditionImpl(conditionLocation
				.getName(this.locationNamingStrategy), conditionLocation
				.getFormula());
	}
	 ...
	 * @return
	 */
	private Set<List<UniqueLocation>> getContinuations(
			List<UniqueLocation> word,
			BinaryRelation<UniqueLocation, UniqueLocation> greaterThanPO,
			BinaryRelation<UniqueLocation, UniqueLocation> lowerThanPO,
			Set<UniqueLocation> remainingLocations) {
		Set<List<UniqueLocation>> continuations = new HashSet<List<UniqueLocation>>();

		if (remainingLocations.isEmpty()) {
			continuations.add(word);
		} else {
			// check for every remainingLocation if it's a possible continuation
			// of the word
			for (UniqueLocation symbol : remainingLocations) {
				// If
				// a) the symbol is not part of the word yet
				// b) all the locations that should happen immediately before
				// have happened
				...
				if (this.canComeNext(symbol, word, greaterThanPO)) {
					...
					// Add the symbol to the continuation and create a new
					// remaining locations
					List<UniqueLocation> continuation = new LinkedList<UniqueLocation>(
							word);
					Set<UniqueLocation> newRemainingLocations = new HashSet<UniqueLocation>(
							remainingLocations);

					continuation.add(symbol);
					newRemainingLocations.remove(symbol);

					// If there are Conditions that can happen next, they must
					// (ASAP condition' semantic)

					// Get the (immediately) greater conditions
					Set<UniqueLocation> greaterLocations = lowerThanPO
							.getImage(symbol);
					...
					while (!greaterConditions.isEmpty()) {
						...
						Iterator<UniqueLocation> greaterConditionsIt = greaterConditions
								.iterator();
						UniqueLocation greaterCondition = greaterConditionsIt
								.next();
						greaterConditionsIt.remove();

						// if the condition can come next, it must (ASAP
						// condition' semantic)
						if (this.canComeNext(greaterCondition, continuation,
								greaterThanPO)) {
							continuation.add(greaterCondition);
							newRemainingLocations.remove(greaterCondition);

							// If from that point on there are more conditions,
							// add them (ASAP semantic).
							greaterLocations = lowerThanPO
									.getImage(greaterCondition);
							this.selectConditions(greaterLocations,
									greaterConditions);
						}
					}

					// Get the continuations from the new word, and add them.
					continuations.addAll(this.getContinuations(continuation,
							greaterThanPO, lowerThanPO, newRemainingLocations));
				}
			}
	...
	/**
	 * If a) the symbol is not part of the word yet b) all the locations that
	 * should happen immediately before have happened then the symbol can come
	 * next in the word.
	 * 
	 * @param symbol
	 * @param word
	...
	private boolean canComeNext(UniqueLocation symbol,
			Collection<UniqueLocation> word,
			BinaryRelation<UniqueLocation, UniqueLocation> greaterThanPO) {
		Set<UniqueLocation> lowerLocations = greaterThanPO.getImage(symbol);
		return ((!word.contains(symbol)) && CollectionUtils.isSubCollection(
				lowerLocations, word));
	}

	/**
	 * Gets the symbols that comes first in some word in the chart's generated
	 * language and does not violate ASAP condition' semantic
	 * 
	 * @param chart
	...
	private Set<UniqueLocation> getStartingLocationsNotViolatingASAP(
			...
			BinaryRelation<UniqueLocation, UniqueLocation> greaterThanPO)
			throws ChartLanguageGenerationException {
		Set<UniqueLocation> startingLocations = this.getStartingLocations(
				uniqueLocations, greaterThanPO);
		...
		// If there is a starting condition, only that is the starting symbol
		// (ASAP condition' semantic)
	...
	/**
	 * Select from the Collection the ones that are Conditions
	 * 
	 * @param locations
	 * @return
	 */
	private void selectConditions(Iterable<UniqueLocation> locations,
			Collection<UniqueLocation> output) {
		CollectionUtils.select(locations,
				Chart.IS_CONDITION_UNIQUE_LOCATION_PREDICATE, output);
	}
	...
	/**
	 * Calculates the partial order defined by the chart. The interactions are
	 * considered instantaneous. The relation means <b>greater than<b>
	 * 
	 * @param instances
	 * @param prechart
	 * @return
	...
	private BinaryRelation<UniqueLocation, UniqueLocation> buildGreaterThanPartialOrder(
			List<UniqueLocation> uniqueLocations) {
		MapSetBinaryRelation<UniqueLocation, UniqueLocation> partialOrder = new MapSetBinaryRelation<UniqueLocation, UniqueLocation>();
		...
		for (String instance : instances) {
			UniqueLocation loc1;
			UniqueLocation loc2;
			List<UniqueLocation> instanceLocations = this.getInstanceLocations(
					instance, uniqueLocations);
	...
	/**
	 * Filters the list keeping just the pairs with Locations affecting the
	 * instance
	 * 
	 * @param instance
	 * @param uniqueLocations
	...
	public boolean equals(Object obj) {
		...
		return new EqualsBuilder().append(this.getLinearisations(),
				chart.getLinearisations()).isEquals();
	}

	public int hashCode() {
		return new HashCodeBuilder().append(this.getLinearisations())
				.toHashCode();
	}
	...
	private class UniqueLocation {
		...
		public boolean equals(Object obj) {
			...
			return new EqualsBuilder().append(this.getPosition(),
					uniqueLocation.getPosition()).append(this.getLocation(),
					uniqueLocation.getLocation()).isEquals();
		}
		...
		@Override
		public int hashCode() {
			return new HashCodeBuilder().append(this.getPosition()).append(
					this.getLocation()).toHashCode();
		}
