...
public class ZetaFunction implements Valuation {
	...
	/**
	 * The key is the name of the fluent. The value is the Fluent and the
	 * current valuation
	 */
	...
	public ZetaFunction(Set<Fluent> fluents) {
		...
		for (Fluent fluent : fluents) {
			this.fluentWithValuations.put(fluent.getName(),
					new FluentValuationTuple(fluent));
		}
	}
	...
	public boolean getValuation(PropositionalVariable variable) {
		FluentValuationTuple fluentWithValuation = this.fluentWithValuations
	...
	public ZetaFunction getCopy() {
		...
		Set<Entry<String, FluentValuationTuple>> entrySet = this.fluentWithValuations
				.entrySet();
		for (Entry<String, FluentValuationTuple> entry : entrySet) {
			// copy each entry
			copy.fluentWithValuations.put(entry.getKey(),
					new FluentValuationTuple(entry.getValue()));
		}
		return copy;
	}

	public void apply(Symbol symbol) {
		Collection<FluentValuationTuple> fluentValuations = this.fluentWithValuations
				.values();
		for (FluentValuationTuple fluentValuationTuple : fluentValuations) {
			if (fluentValuationTuple.getFluent().getInitiatingActions()
					.contains(symbol)) {
				// the symbol is a initiation action, the fluent is set to true
				fluentValuationTuple.setCurrentValuation(true);
			} else if (fluentValuationTuple.getFluent().getTerminatingActions()
					.contains(symbol)) {
				// the symbol is a terminating action, the fluent is set to
				// false
	...
	public boolean equals(Object anObject) {
		...
		if (anObject instanceof ZetaFunction) {
			...
			return new EqualsBuilder().append(this.fluentWithValuations,
					zetaFunction.fluentWithValuations).isEquals();
		} else {
			return false;
		}
	}
	...
	public int hashCode() {
		return new HashCodeBuilder(17, 39).append(this.fluentWithValuations)
				.toHashCode();
	}
	...
	private class FluentValuationTuple {
		...
		public FluentValuationTuple(Fluent fluent) {
			this.fluent = fluent;
			this.currentValue = fluent.initialValue();
		}
		...
		public boolean equals(Object anObject) {
			...
			if (anObject instanceof FluentValuationTuple) {
				...
				return new EqualsBuilder().append(this.getFluent(),
						fluentValuation.getFluent()).append(
						this.getCurrentValue(),
						fluentValuation.getCurrentValue()).isEquals();
			} else {
				return false;
			}
		}
		...
		public int hashCode() {
			return new HashCodeBuilder(17, 39).append(this.fluent).append(
					this.currentValue).toHashCode();
		}
