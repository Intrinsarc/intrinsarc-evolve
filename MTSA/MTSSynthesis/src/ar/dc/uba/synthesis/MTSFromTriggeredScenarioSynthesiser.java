...
import ar.dc.uba.util.*;

/**
 * 
 * Synthesizes a MTS from a modal TriggeredScenario. The MTS satisfies the
 * TriggeredScenario and every MTS that satisfies the TriggeredScenario is a
 * observational refinement of the syntheized one.
 * 
 * @author gsibay
 * 
 */
public abstract class MTSFromTriggeredScenarioSynthesiser {
	...
	private void init(TriggeredScenario lsc) {
		...
		this.restrictedSymbols = new HashSet<Symbol>(lsc.getAlphabet()
				.getSymbols());
		// the restricted symbols are initiated here and never modified
		this.restrictedSymbols = SetUtils
				.unmodifiableSet(this.restrictedSymbols);
	...
	public MTS<SynthesizedState, Symbol> synthesise(
		...
		ZetaFunction initialZetaFunction = new ZetaFunction(triggeredScenario
				.getFluents());

		SynthesizedState initialState = this
				.createInitialState(initialZetaFunction);

		// Creates the MTS with the initial state
		MTS<SynthesizedState, Symbol> mts = new MTSImpl<SynthesizedState, Symbol>(
				initialState);
		...
		while (!unprocessedStates.isEmpty()) {
			...
			unprocessedStates
					.addAll(this.processState(mts, anUnprocessedState));
	...
	/**
	 * Process the state adding new transitions and states to the mts. The new
	 * states are returned.
	 * 
	 * @param mts
	 ...
	 * @return
	 */
	protected abstract Set<SynthesizedState> processState(
			MTS<SynthesizedState, Symbol> mts,
			SynthesizedState anUnprocessedState);

	protected abstract SynthesizedState createInitialState(
	...
	/**
	 * This process may create new states. The new states must be processed
	 * next, so they�re collected and returned as a collection.
	 * 
	 * @param mts
	 * @param state
	 * @param symbol
	 * @return
	 */
	protected abstract Collection<SynthesizedState> addTransitions(
			MTS<SynthesizedState, Symbol> mts, SynthesizedState state, Symbol t);

	protected void addAsMaybeTransition(SynthesizedState state, Symbol t,
			SynthesizedState nextState, MTS<SynthesizedState, Symbol> mts,
			Set<SynthesizedState> newStates) {
		this.addIfNotPresent(nextState, mts, newStates);
		mts.addTransition(state, t, nextState, TransitionType.MAYBE);
	}

	protected void addAsRequiredTransition(SynthesizedState state, Symbol t,
			SynthesizedState nextState, MTS<SynthesizedState, Symbol> mts,
			Set<SynthesizedState> newStates) {
		this.addIfNotPresent(nextState, mts, newStates);
	 ...
	 * @param newStates
	 */
	protected void addIfNotPresent(SynthesizedState nextState,
			MTS<SynthesizedState, Symbol> mts,
	...
	protected Set<Word> nonEmptyWordFollows(Set<Word> words, Symbol symbol) {
		...
		Predicate<Word> equalsEmptyWordPredicate = new EqualPredicate<Word>(
				Word.EMPTY_WORD);
		// The empty words are removed
		CollectionUtils.filter(result, NotPredicate
				.getInstance(equalsEmptyWordPredicate));
		return result;
	}
