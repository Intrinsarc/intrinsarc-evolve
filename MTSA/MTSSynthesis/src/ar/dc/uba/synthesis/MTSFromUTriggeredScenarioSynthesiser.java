 ...
 * 
 */
public class MTSFromUTriggeredScenarioSynthesiser extends
	...
	protected Set<SynthesizedState> processState(
		...
		for (Symbol symbol : this.getRestrictedSymbols()) {
			// adds the transitions (if applicable) for the unprocessed state
			// and new states may require to be processed
	...
	protected SynthesizedState createInitialState(
		...
		// Initial state's word is the canonical representation of the empty
		// word epsilon
		...
		if (this.satisfiesPrechart(initialWord)) {
			...
			possiblePaths = this.createSingletonWordsFromSymbols(this
					.getRestrictedSymbols());
		}

		return new SynthesizedState(initialWord, new ObligationsFromUniversal(
				requiredPaths, possiblePaths), initialZetaFunction);
	}
	...
	protected Collection<SynthesizedState> addTransitions(
		...
		ObligationsFromUniversal obligations = (ObligationsFromUniversal) state
				.getObligations();
		Set<Word> reqPaths = obligations.getRequiredPaths();
		Set<Word> possPaths = obligations.getMaybePaths();

		Set<Word> reqPathsStartingWithCurrentSymbol = this.startingWith(
				reqPaths, t);
		Set<Word> posPathsStartingWithCurrentSymbol = this.startingWith(
				possPaths, t);

		SynthesizedState nextState;

		// For all next state if the prechart hold in the next state
		// the required path will be all the mainchart (and nothing else)
		// and the maybe path will be empty
		if (!reqPathsStartingWithCurrentSymbol.isEmpty()) { // there is a
															// required path
															// starting with the
															// current symbol
			...
			for (Word reqPathStartingWithCS : reqPathsStartingWithCurrentSymbol) {
				...
				if (this.satisfiesPrechart(nextSignificativeSuffix)) {
					...
					// the required path will be all the mainchart (and nothing
					// else)
					// and the maybe path will be empty
					...
					// create next state obligations with possible and required
					// paths
					Obligations nextStateObligations = new ObligationsFromUniversal(
							nextStateReqPaths, nextStateMayPaths);
					// Add as maybe transition
					nextState = new SynthesizedState(nextSignificativeSuffix,
							nextStateObligations, nextZetaFunction);
					this.addAsMaybeTransition(state, t, nextState, mts,
							newStates);
				} else {

					// ----- NEXT STATE REQUIRED PATHS ------
					// the new required paths will be:
					// the selected required path without the first symbol (if
					// not empty)
					nextStateReqPaths = this.nonEmptyWordFollows(Collections
							.singleton(reqPathStartingWithCS), t);

					// ----- NEXT STATE POSSIBLE PATHS ------

					// pos and req paths after current symbol are needed
					Set<Word> mayPathsAfterCurrentSymbol = this
							.nonEmptyWordFollows(
									posPathsStartingWithCurrentSymbol, t);
					Set<Word> reqPathsAfterCurrentSymbol = this
							.nonEmptyWordFollows(
									reqPathsStartingWithCurrentSymbol, t);

					// the next state possible paths are:
					// all possible path after current symbol and, all required
					// paths after current symbol
					// (each one will be required in another branch created by
					// the current "for") after current symbol
					// but the required path starting with CS being processed.
					// nextStateReqPaths is only one path.
					nextStateMayPaths = new HashSet<Word>(CollectionUtils
							.subtract(CollectionUtils.union(
									mayPathsAfterCurrentSymbol,
									reqPathsAfterCurrentSymbol),
									nextStateReqPaths));

					// if there are no req obligations then all the restricted
					// symbols are possible
					if (nextStateReqPaths.isEmpty()) {
						nextStateMayPaths = new HashSet<Word>(CollectionUtils
								.union(nextStateMayPaths, this
										.createSingletonWordsFromSymbols(this
												.getRestrictedSymbols())));
					}
					...
					nextState = new SynthesizedState(nextSignificativeSuffix,
							nextStateObligations, nextZetaFunction.getCopy());
					this.addAsRequiredTransition(state, t, nextState, mts,
							newStates);
				}

			}
		} else // there is NO required path starting with the current symbol
		if (!posPathsStartingWithCurrentSymbol.isEmpty()) { // there is a
															// possible path
															// starting with the
															// current symbol
			...
			for (Word possPathStartingWithCS : posPathsStartingWithCurrentSymbol) {
				...
				if (this.satisfiesPrechart(nextSignificativeSuffix)) {
					...
					// ----- NEXT STATE REQUIRED PATHS ------
					// the current possible path will be required in the next
					// state (taking this maybe transition
					// makes the possible path mandatory)
					// the selected possible path without the first symbol (if
					// not empty)
					nextStateReqPaths = this.nonEmptyWordFollows(Collections
							.singleton(possPathStartingWithCS), t);
					...
					// ----- NEXT STATE POSSIBLE PATHS ------
					// the next state possible paths are:
					// all possible path after current symbol except the one
					// being processed that is going
					// to be required (nextStateReqPaths is only one word).
					nextStateMayPaths = new HashSet<Word>(CollectionUtils
							.subtract(mayPathsAfterCurrentSymbol,
									nextStateReqPaths));

					// if there are no req paths in the next state then all the
					// restricted symbols are added as possible
					if (nextStateReqPaths.isEmpty()) {
						nextStateMayPaths = new HashSet<Word>(CollectionUtils
								.union(nextStateMayPaths, this
										.createSingletonWordsFromSymbols(this
	...
	/**
	 * Creates a set of words from the set of symbols. The result contains one
	 * word for each symbol. The word is just the symbol.
	 * 
	 * @param symbols
	 * @return
	 */
