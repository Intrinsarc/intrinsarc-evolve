...
public class MTS2FSP<State, Action> {
	...
	public String toFSP(MTS<State, Action> mts, String mtsName) {
		...
		// After the last process put a "\{TAU_SYMBOL}." instead of a ",". End
		// of processes.
		// The TAU_SYMBOL can not be tau for MTSA�s implementations details.
		// The workarround is to use another symbol for tau and then hide it (so
		// it becomes a tau).
		fsp.delete(fsp.length() - 2, fsp.length()).append("\\{").append(
				Alphabet.TAU).append("}.\n");
	...
	/**
	 * Writes as a comment the mapping between a FSP�s Proccess and a MTS�
	 * State.
	 * 
	 * @param fsp
	 */
	private void writeStatesIdMapping(StringBuffer fsp) {
		...
		for (Entry<State, String> entry : mapEntry) {
			fsp.append(entry.getValue()).append(" = ").append(entry.getKey())
					.append("\n");
		}
	...
	/**
	 * Writes the state as a Process in FSP. Example: E_1 = (k? -> E_1 |t -> E_2
	 * |t? -> E_3 |k -> E_4 ), E_2 = STOP, E_3 = STOP, E_4 = STOP.
	 * 
	 * @param state
	 * @param mts
	 * @param fsp
	 */
	private void writeProccessForState(State state, MTS<State, Action> mts,
			StringBuffer fsp) {
		...
		BinaryRelation<Action, State> reqTransitions = mts.getTransitions(
				state, TransitionType.REQUIRED);
		BinaryRelation<Action, State> mayTransitions = mts.getTransitions(
				state, TransitionType.MAYBE);
		if (reqTransitions.isEmpty() && mayTransitions.isEmpty()) { // There are
																	// no
																	// transitions
																	// from this
																	// state
	 ...
	 * @param fsp
	 */
	private void writeTransitions(State fromState,
			BinaryRelation<Action, State> transitions, boolean isMaybe,
			StringBuffer fsp) {
		for (Pair<Action, State> pair : transitions) {
			...
			fsp.append(" -> ").append(this.statesId.get(pair.getSecond()))
					.append("\n").append("\t|");
		}
	}

	/**
	 * Assigns id to the states. The initial state has the mtsName, and then
	 * they are named E_ with a number starting from 1. The states have
	 * consecutives numbers (except the first one). The state is then identified
	 * as E_ and then the number. For instance, if there are 8 different states,
	 * they will be numerated from 1 to 8. The states will be E_1 to E_8.
	 * 
	 * @param mts
	 */
