...
/**
 * A significative suffix calculator. Calculates the significative suffix of
 * words for a fixed Prechart.
 * 
 * @author gsibay
 * 
 */
public class SynthesiserHelper {
	...
	/**
	 * Builds the calculator from the canonical bounded linearisations. Each
	 * linearisation represents a set of potentially infinite canonical
	 * linarisation.
	 * 
	 * @param canonicalBoundedLinearisations
	 */
	public SynthesiserHelper(TriggeredScenario triggeredScenario) {
		this.qtyInteractions = triggeredScenario.getPrechart()
				.getQtyInteractions();
		this.prechartConditions = triggeredScenario.getPrechart()
				.getConditions();
		this.isLinearisationPrefix = new IsCanonicalLinearisationPrefixPredicate(
				triggeredScenario.getPrechart());
	}
	...
	public Set<String> getConditionsHolding(ZetaFunction zetaFunction) {
		...
		CollectionUtils.select(this.prechartConditions, satisfies,
				conditionsHolding);
		...
		CollectionUtils.collect(conditionsHolding, toConditionNameTransformer,
				conditionsHoldingNames);
	...
	/**
	 * Calculates the significative suffix of the parameter word. It�s the
	 * maximun (in lenght) suffix of the word that is a preffix of LP.LM
	 * 
	 * @param word
	 * @param zetaFunction
	 * @return
	 */
	public Word calculateSignificativeSuffix(Word word,
			ZetaFunction zetaFunction) {

		// calculate the canonical suffixes
		// every suffix has to have an epsilon with the conditions holding as
		// first symbol
		...
		while (!symbols.isEmpty()) {
			// From the first symbol we get the conditions holding and then we
			// discard it
			Set<String> conditionsHoldingBeforeSymbol = ((CanonicalSymbol) symbols
					.get(0)).getConditionsNames();
			symbols.remove(0);

			// The next suffix first symbol has the conditions holding
			CanonicalSymbol epsilonSymbol = new CanonicalSymbol(
					conditionsHoldingBeforeSymbol);
		...
		Collection<Word> sufixesThatAreLinearisationPrefixes = CollectionUtils
				.select(suffixes, this.isLinearisationPrefix);

		if (sufixesThatAreLinearisationPrefixes.isEmpty()) {
			// there is no suffixes that is also a prefix of some linearisation.
			// return an epsilon symbol with all the conditions holding with the
			// current zeta function
			Symbol symbol = new CanonicalSymbol(this
					.getConditionsHolding(zetaFunction));
			return new Word(Collections.singletonList(symbol));
		} else {
			// select the suffixes that also are prefixes and then select the
			// maximum.
			return Collections.max(sufixesThatAreLinearisationPrefixes,
					Word.WORD_LENGTH_COMPARATOR);
		}
	}

	/**
	 * Returns a new word that is the next(word, symbol). If the symbol is Tau
	 * it returns a new word equals to the parameter. The zetaFunction is
	 * updated reflecting the occurrence of newSymbol
	 * 
	 * @param word
	 * @param newSymbol
	 * @return
	...
	public Word next(Word word, Symbol newSymbol, ZetaFunction zetaFunction) {
		if (Alphabet.TAU.equals(newSymbol)) {
			...
			// Get the canonical representation of the symbol for the new
			// valuation (zetaFunction)
			CanonicalSymbol newCanonicalSymbol = new CanonicalSymbol(newSymbol,
					this.getConditionsHolding(zetaFunction));

			List<Symbol> wordAsSymbols = new LinkedList<Symbol>(word
					.getSymbols());
			wordAsSymbols.add(newCanonicalSymbol);

			Word wordWithNewSymbol = new Word(wordAsSymbols);

			return this.calculateSignificativeSuffix(wordWithNewSymbol,
					zetaFunction);
		}
	}
	...
	public boolean satisfiesPrechart(Word canonicalWord) {
		...
		// <epsilon,conditions> as initial symbol and then one for each
		// interaction
		// That's why a full lenght Prechart word will
		// have a lenght of ( this.qtyInteractions ) + 1
		boolean isSizeOfPrechartWord = canonicalWord.length() == (this.qtyInteractions + 1);
