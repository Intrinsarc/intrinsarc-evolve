...
import ac.ic.doc.mtstools.model.impl.WeakBisimulationSemantics;
...
import controller.ControllerTestsUtils;
...
public class TopStatesTests extends TestCase {
	public void testNoTopStates() throws Exception {
		Fluent fluentNorthFar = new FluentImpl("A", ControllerTestsUtils
				.buildSymbolSetFrom(new String[] { "a" }), ControllerTestsUtils
				.buildSymbolSetFrom(new String[] { "b" }), false);
		...
		MTS<Long, String> mtsWithTopStates = compiler.compileMTS(withTopStates, "withTop");
		MTS<Long, String> mtsNoTop = compiler.compileMTS(noTopStates, "noTop");

		MTS<Long, String> removeTopStates = ControllerUtils.removeTopStates(mtsWithTopStates, Collections.singleton(fluentNorthFar));

		WeakBisimulationSemantics bisimulationSemantics = new WeakBisimulationSemantics(Collections
				.emptySet());

		boolean bisimilar = TransitionSystemDispatcher.isRefinement(mtsNoTop, " original ",
				removeTopStates, " synthesised ", bisimulationSemantics, new StandardOutput());
	...
	public void testComponeBien() throws Exception {
		...
		CompositeState compiledNoTopDO = compiler.compileCompositeState(noTopDO, "noTopDO");
		CompositeState compiledNoTopFarNorth = compiler.compileCompositeState(noTopFarNorth, "noTopFarNorth");
		CompositeState compiledNoTopFarSouth = compiler.compileCompositeState(noTopFarSouth, "noTopFarSouth");
		CompositeState compiledModel = compiler.compileCompositeState(model, "model");
		...
		MTS<Long, String> compositeCompositionTopFree = AutomataToMTSConverter.getInstance().convert(c.getComposition());

		MTS<Long, String> mtsCompiledNoTopDO = compiler.compileMTS(noTopDO, "noTopDO");
		MTS<Long, String> mtscompiledNoTopFarNorth = compiler.compileMTS(noTopFarNorth,"noTopFarNorth");
		MTS<Long, String> mtscompiledNoTopFarSouth = compiler.compileMTS(noTopFarSouth,"noTopFarSouth");
		MTS<Long, String> mtscompiledModel = compiler.compileMTS(model, "model");
		...
		MTS mtsCompositionTopFree = new MTSMultipleComposer(compositionRuleApplier).compose(toCompose);

		assertBisimilar(compositeCompositionTopFree, mtsCompositionTopFree);

		Fluent fluentDoorClosed = new FluentImpl("DoorClosed", ControllerTestsUtils
		...
		MTS<Long, String> noTopForDoorClosed = ControllerUtils.getModelFrom(fluentDoorClosed);
		noTopForDoorClosed.addAction("tau");
		MTS<Long, String> noTopForNorthFar = ControllerUtils.getModelFrom(fluentNorthFar);
		noTopForNorthFar.addAction("tau");
		MTS<Long, String> noTopForSouthFar = ControllerUtils.getModelFrom(fluentSouthFar);
		noTopForSouthFar.addAction("tau");
		...
		MTS<Long, String> removeTopStates = ControllerUtils.removeTopStates(mtscompiledModel, fluents);
	...
	private void assertBisimilar(MTS<Long, String> convert, MTS compose) {
		...
		boolean bisimilar = TransitionSystemDispatcher.isRefinement(convert, " original ", compose,
				" synthesised ", bisimulationSemantics, new StandardOutput());
