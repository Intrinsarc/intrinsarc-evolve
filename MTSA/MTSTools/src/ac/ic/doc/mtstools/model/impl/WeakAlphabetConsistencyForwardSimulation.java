...
public class WeakAlphabetConsistencyForwardSimulation<A> extends
		AbstractForwardSimulation<A> {

	private Set<A> silentActions;
	private Set<A> nonSharedActions;
	private Set<A> commonAlphabet;

	public WeakAlphabetConsistencyForwardSimulation(MTS<?, A> m, MTS<?, A> n,
			Set<A> silentActions) {
		super(REQUIRED, POSSIBLE);
		this.setSilentActions(silentActions);

		Set<A> nonSharedActions = new HashSet<A>(CollectionUtils.subtract(n
				.getActions(), m.getActions()));
		nonSharedActions.addAll(this.getSilentActions());
		this.setNonSharedActions(nonSharedActions);

		Set<A> commonAlphabet = new HashSet<A>(CollectionUtils.intersection(n
				.getActions(), m.getActions()));
	...
	protected <S> Iterator<List<Pair<A, S>>> getPaths(MTS<S, A> mts, S state,
		...
		if (this.getCommonAlphabet().contains(label)) {
			return PathBuilder.getInstance().getPathsIterator(mts, state,
					label, this.getSimulatorTransitionType(),
					this.getNonSharedActions());
		} else {
			A silentAction = this.getSilentActions().iterator().next();
			return ClousurePathBuilder.getInstance().getPathsIterator(mts,
					state, silentAction, this.getSimulatorTransitionType(),
					this.getSilentActions());
		}
	}
		...
		while (it.hasNext()) {
			...
			if (!this.getSilentActions().contains(action)) {
				if (prevIntermidiateState != null
						&& !relation.contains(Pair.create(s1,
								prevIntermidiateState))) {
					return false;
				}
	...
	private void setSilentActions(Set<A> silentActions) {
		if (silentActions == null || silentActions.isEmpty()) {
			this.silentActions = (Set<A>) Collections.singleton(new Object()); // hack
																				// since
																				// silentActions
																				// shouldn't
																				// be
