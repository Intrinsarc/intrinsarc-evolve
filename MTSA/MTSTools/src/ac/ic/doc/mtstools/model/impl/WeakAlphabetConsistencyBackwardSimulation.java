...
public class WeakAlphabetConsistencyBackwardSimulation<A> extends
	...
	public WeakAlphabetConsistencyBackwardSimulation(MTS<?, A> m, MTS<?, A> n,
			Set<A> silentActions) {
		super(REQUIRED, POSSIBLE);
		this.setSilentActions(silentActions);

		Set<A> nonSharedActions = new HashSet<A>(CollectionUtils.subtract(m
				.getActions(), n.getActions()));
		nonSharedActions.addAll(this.getSilentActions());
		this.setNonSharedActions(nonSharedActions);

		Set<A> commonAlphabet = new HashSet<A>(CollectionUtils.intersection(m
				.getActions(), n.getActions()));
	...
	protected <S1, S2> boolean simulate(S2 s2, Pair<A, S2> transition, S1 s1,
		...
		while (it.hasNext()) {
			...
			if (!this.getSilentActions().contains(action)) {
				if (prevIntermidiateState != null
						&& !relation.contains(Pair.create(
								prevIntermidiateState, s2))) {
					return false;
				}
		...
		return relation.contains(Pair.create(pathTransition.getSecond(),
