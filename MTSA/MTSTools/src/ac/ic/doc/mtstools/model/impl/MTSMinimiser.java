...
public class MTSMinimiser<Action> {
	...
	private void initalizeMinimiser(MTS<Long, Action> mts) {
		...
		for (Long state : mts.getStates()) {
			StateSucessors<Action> actualSuccessors = new StateSucessors<Action>(
					state);

			for (Pair<Action, Long> transition : mts.getTransitions(state,
					TransitionType.REQUIRED)) {
				actualSuccessors.addSuccessor(transition.getSecond(),
						transition.getFirst(), TransitionType.REQUIRED);
			}
			for (Pair<Action, Long> transition : mts.getTransitions(state,
					TransitionType.POSSIBLE)) {
				actualSuccessors.addSuccessor(transition.getSecond(),
						transition.getFirst(), TransitionType.POSSIBLE);
			}
		...
		simulationRelation
				.add(new Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>(
						initialEquivalenceClass, initialEquivalenceClass));
	 ...
	 * @return
	 */
	private MTS<Long, Action> buildMTS(
			MTS<Long, Action> mts,
			Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>> simulationRelation) {
		MTS<Long, Action> result = new MTSImpl<Long, Action>(
				code(getEquivalenceClassOf(mts.getInitialState())));
		...
		for (EquivalenceClass<Long> equivalenceClass : equivalenceClasses) {
			...
			for (Long state : equivalenceClass) {
				BitSet stateSuccessors = stateSucessors[state.intValue()]
						.getSuccessors();
			...
			BinaryRelation<Action, Long> requiredTransitionsFrom = mts
					.getTransitions(state, required);
			for (Pair<Action, Long> transition : requiredTransitionsFrom) {
				Long from = code(equivalenceClass);
				EquivalenceClass<Long> equivalenceClassOf = getEquivalenceClassOf(transition
						.getSecond());
				...
				if (nextStates.get(to.intValue())) {
					...
					result.addTransition(from, transition.getFirst(), to,
							required);
				}
			}
			BinaryRelation<Action, Long> maybeTransitionsFrom = mts
					.getTransitions(state, maybe);
			for (Pair<Action, Long> transition : maybeTransitionsFrom) {
				...
				if (nextStates.get(to.intValue())) {
					...
					result.addAction(transition.getFirst());
					result
							.addTransition(from, transition.getFirst(), to,
									maybe);
				}
			}
		}
	...
	private BitSet getSuccessorsClasses(BitSet stateSuccessors) {
		...
		for (int i = stateSuccessors.nextSetBit(0); i >= 0; i = stateSuccessors
				.nextSetBit(i + 1)) {
	...
	private Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>> computeMaximalSimulationRelation(
		...
		while (change) {
			...
			// la anterior
			change |= update(mts, iterationEquivalenceClasses,
					iterationSimulationRelation);
	...
	private boolean update(
			MTS<Long, Action> mts,
			Set<EquivalenceClass<Long>> iterationEquivalenceClasses,
			...
			for (EquivalenceClass<Long> equivalenceClassA : iterationEquivalenceClasses) {
				for (EquivalenceClass<Long> equivalenceClassB : iterationEquivalenceClasses) {
					if (relationItem.getFirst().hasAll(equivalenceClassA)
							&& relationItem.getSecond().hasAll(
									equivalenceClassB)) {
						Set<EquivalenceClass<Long>> fi = buildFi(
								equivalenceClassB, mts);
						if (fi.containsAll(buildPi(equivalenceClassA, mts))
								&& classBRefinesClassA(equivalenceClassA,
										equivalenceClassB)) {
							iterationSimulationRelation
									.add(new Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>(
											equivalenceClassA,
	...
	private boolean classBRefinesClassA(
			EquivalenceClass<Long> equivalenceClassA,
			EquivalenceClass<Long> equivalenceClassB) {
		return stateBRefinesStateA(equivalenceClassA.iterator().next(),
				equivalenceClassB.iterator().next());
	}

	private Set<EquivalenceClass<Long>> buildPi(
			EquivalenceClass<Long> equivalenceClass, MTS<Long, Action> mts) {
	...
	private Set<EquivalenceClass<Long>> buildFi(
			EquivalenceClass<Long> equivalenceClass, MTS<Long, Action> mts) {
		Set<EquivalenceClass<Long>> result = new HashSet<EquivalenceClass<Long>>();
		Set<EquivalenceClass<Long>> piEquivalenceClass = buildPi(
				equivalenceClass, mts);
	...
	private boolean refine(
		...
		for (EquivalenceClass<Long> clazz : equivalenceClasses) {
			for (Long state : clazz) {
				EquivalenceClass<Long> newEquivalenceClass = computeNewEquivalenceClass(
						state, clazz);
	...
	private EquivalenceClass<Long> computeNewEquivalenceClass(Long state,
			EquivalenceClass<Long> equivalenceClass) {
	...
	private boolean stateBRefinesStateA(Long stateA, Long stateB) {
		...
		// BaseSemanticsByRelation weak = new
		// WeakSemantics(Collections.EMPTY_SET);
		// BaseSemanticsByRelation weak = new
		// WeakSemantics(Collections.singleton(MTSExamples.TAU));
		// return weak.isARefinement(mtsToMinimise, mtsToMinimise, stateA,
		// stateB);
		...
		for (Entry<Action, BitSet> successorFromAByAction : aSucessors
				.getRequiredSuccessorsByAction().entrySet()) {
			BitSet successorsFromA = successorFromAByAction.getValue();
			BitSet successorsFromB = bSucessors.getRequiredSuccessorsByAction()
					.get(successorFromAByAction.getKey());

			if (!successorsOfBRefinesSuccessorsOfA(successorsFromA,
					successorsFromB)) {
				return false;
			}
		}
		...
		for (Entry<Action, BitSet> successorFromBByAction : bSucessors
				.getPossibleSuccessorsByAction().entrySet()) {
			BitSet successorsFromB = successorFromBByAction.getValue();
			BitSet successorsFromA = aSucessors.getPossibleSuccessorsByAction()
					.get(successorFromBByAction.getKey());

			if (!successorsOfBRefinesSuccessorsOfA(successorsFromB,
					successorsFromA)) {
				return false;
			}
		}
	 ...
	 */
	private boolean successorsOfBRefinesSuccessorsOfA(BitSet successorsFromA,
			BitSet successorsFromB) {
		if (successorsFromB == null) {
			return false;
		}
		for (int succA = successorsFromA.nextSetBit(0); succA >= 0; succA = successorsFromA
				.nextSetBit(succA + 1)) {
	...
	private boolean hasRefinesSuccesor(BitSet successorsFromB, Integer succA) {
		for (int succB = successorsFromB.nextSetBit(0); succB >= 0; succB = successorsFromB
				.nextSetBit(succB + 1)) {
			// TODO ISSUE Desing, no quiero construir pares
			if (simulationRelation
					.contains(new Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>(
							getEquivalenceClassOf(Long.valueOf(succB)),
							getEquivalenceClassOf(Long.valueOf(succA))))) {
				return true;
			}
		}
		return false;
	...
	private EquivalenceClass<Long> getEquivalenceClassOf(Long state) {
		...
		throw new RuntimeException("There is no equivalence class for state: ["
				+ state + "]");
	}
