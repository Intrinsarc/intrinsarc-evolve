...
public class BranchingByTransformationSemantics implements Refinement {
	...
	public static class BranchingExpansion {
		...
		public <S, A> MTS<Integer, A> transform(MTS<S, A> input) {
			...
			int nextState = 0;
			Map<S, Integer> state2int = new HashMap<S, Integer>(input
					.getStates().size());
			for (S state : input.getStates()) {
				state2int.put(state, nextState++);
			}

			MTS<Integer, A> result = new MTSImpl<Integer, A>(state2int
					.get(input.getInitialState()));
			...
			for (S source : input.getStates()) {
				...
				for (Pair<A, S> transition : input.getTransitions(source,
						TransitionType.REQUIRED)) {
					result.addTransition(iSource, transition.getFirst(),
							state2int.get(transition.getSecond()),
							TransitionType.REQUIRED);
				}
				for (Pair<A, S> transition : input.getTransitions(source,
					...
					if (silentActions.contains(transition.getFirst())) {
						...
						result.addTransition(iSource, silentAction, proxyState,
								TransitionType.MAYBE);
						result.addTransition(proxyState, silentAction, iSource,
								TransitionType.REQUIRED);
						result.addTransition(proxyState, transition.getFirst(),
								state2int.get(transition.getSecond()),
								TransitionType.REQUIRED);
					}
				}
