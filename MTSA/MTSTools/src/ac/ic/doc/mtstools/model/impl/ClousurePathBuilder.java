...
public class ClousurePathBuilder {
	...
	public <S, A> Iterator<List<Pair<A, S>>> getPathsIterator(
		...
		return new ClousurePathIterator<S, A>(mts, state, label, type,
				silentActions);
	}

	private static class ClousurePathIterator<S, A> implements
			Iterator<List<Pair<A, S>>> {
		...
		public ClousurePathIterator(MTS<S, A> mts, S state, A label,
				TransitionType type, Set<A> silentActions) {
		...
		public List<Pair<A, S>> next() {
			...
			this.clousureSilentActions(finalState, TransitionType.REQUIRED,
					QueueType.POSTACTION);
		...
		private void clousureSilentActions(S state,
				MTS.TransitionType transitionType, QueueType queueType) {
			BinaryRelation<A, S> transitions = mts.getTransitions(state,
					transitionType);
			for (A silentAction : silentActions) {
				this.enqueueToVisit(transitions.getImage(silentAction),
						queueType);
			}
		}
		...
		private void processPreVisibleTransition() {
			...
			while (this.unprocessed.get(QueueType.POSTACTION).isEmpty()
				...
				if (type != TransitionType.MAYBE
						&& silentActions.contains(label)) {
					this.enqueueToVisit(state, QueueType.POSTACTION);
				}

				this.clousureSilentActions(state, this.type,
						QueueType.PREACTION);
				this.enqueueToVisit(mts.getTransitions(state, this.type)
						.getImage(this.label), QueueType.POSTACTION);
			}
		}
