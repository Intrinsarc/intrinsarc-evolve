...
public class MTSCompositionAdapter<Action> {
	...
	public Map<Action, List<Pair<Long, TransitionType>>> groupByAction(
			Long state) {
		...
		return (Map<Action, List<Pair<Long, TransitionType>>>) cacheTransitionsFromStateGroupedByAction
				.get(state);
	}

	private Map<Action, List<Pair<Long, TransitionType>>> getTransitionsByLabel(
			Long state) {
	...
	public Collection<Transition<Action>> getTransitions(Long state) {
		...
		return (Collection<Transition<Action>>) cacheTransitionsFromState
				.get(state);
	}
	...
	public Collection<Transition<Action>> getTransitions(Long state,
		...
		if (!cacheTransitionsByTypeAndByState.containsKey(type)) {
			cacheTransitionsByTypeAndByState.put(type,
					new HashMap<Long, Collection<Transition<Action>>>());
		}
		// me fijo si ya agregue las transiciones al map para ese type
		Map<Long, Collection<Transition<Action>>> localCacheTransitionByType = cacheTransitionsByTypeAndByState
				.get(type);
		if (!localCacheTransitionByType.containsKey(state)) {

			Collection<Transition<Action>> retValue = new ArrayList<Transition<Action>>();
			for (Pair<Action, Long> transition : adapteeMTS.getTransitions(
					state, type)) {
	 ...
	 * @param to
	 * @return
	 * @see ac.ic.doc.mtstools.model.MTS#addPossible(java.lang.Object,
	 ...
	 * @param to
	 * @return
	 * @see ac.ic.doc.mtstools.model.MTS#addRequired(java.lang.Object,
	 *      java.lang.Object, java.lang.Object)
	 */
	 ...
	 * @param type
	 * @return
	 * @see ac.ic.doc.mtstools.model.MTS#addTransition(java.lang.Object,
	 *      java.lang.Object, java.lang.Object,
	 *      ac.ic.doc.mtstools.model.MTS.TransitionType)
	 */
	public boolean addTransition(Long from, Action label, Long to,
			TransitionType type) {
