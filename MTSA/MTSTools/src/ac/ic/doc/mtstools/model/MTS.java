...
public interface MTS<State, Action> extends TransitionSystem<State, Action> {
	...
	public enum TransitionType {
		REQUIRED {
			public <State, Action> boolean addTransition(
					MTS<State, Action> mts, State from, Action label, State to) {
				return mts.addRequired(from, label, to);
			}

			public <State, Action> boolean removeTransition(
					MTS<State, Action> mts, State from, Action label, State to) {
		...
		public abstract <State, Action> boolean addTransition(
				MTS<State, Action> mts, State from, Action label, State to);

		public abstract <State, Action> boolean removeTransition(
				MTS<State, Action> mts, State from, Action label, State to);
