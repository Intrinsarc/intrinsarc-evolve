	...
	public <S, A> Iterator<List<Pair<A, S>>> getPathsIterator(
		...
		return new Iterator<List<Pair<A, S>>>() {
...

			{
				...
				this.actualStates = new TreeSet<S>();
				this.next = null;
			...
			private void calculateNext() {
				if (endStateIt.hasNext()) {
					this.next = (LinkedList<Pair<A, S>>) this.actualPath
							.clone();
					this.next.addLast(Pair.create(label, endStateIt.next()));
					return;
				}

				Pair<A, S> nextTransition = this.queue.get(
						this.actualPath.size()).poll();
				if (nextTransition == null) {
					...
					BinaryRelation<A, S> transitions = mts.getTransitions(
							newState, type);
					...
					for (A silentAction : silentActions) {
						for (S toState : transitions.getImage(silentAction)) {
							if (!this.actualStates.contains(toState)) {
								silentTransitions.add((Pair<A, S>) Pair.create(
										silentAction, toState));
							}
						}
					}
			...
			private Iterator<S> getEndStateIt(S from,
					BinaryRelation<A, S> transitions) {
				if (type != MAYBE && silentActions.contains(label)
						&& !transitions.getImage(label).contains(from)) {
					// XXX improve performance
					Set<S> endStates = new HashSet<S>(transitions
							.getImage(label));
					endStates.add(from);
