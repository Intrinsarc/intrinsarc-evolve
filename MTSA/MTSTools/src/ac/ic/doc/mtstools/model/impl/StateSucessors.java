...
public class StateSucessors<Action> {
	...
	private Collection<Entry<Action, BitSet>> getMaybeTransitions() {
		return CollectionUtils.subtract(possibleSuccessorsByAction.entrySet(),
				requiredSuccessorsByAction.entrySet());
	}
	...
	public Set<Pair<Action, TransitionType>> getTransitionsTo(Long toState) {
		...
		for (Entry<Action, BitSet> requiredSuccessor : requiredSuccessorsByAction
				.entrySet()) {
			if (requiredSuccessor.getValue().get(toState.intValue())) {
				result.add(new Pair<Action, TransitionType>(requiredSuccessor
						.getKey(), TransitionType.REQUIRED));
			}
		}
		for (Entry<Action, BitSet> maybeSuccessor : getMaybeTransitions()) {
			if (maybeSuccessor.getValue().get(toState.intValue())) {
				result.add(new Pair<Action, TransitionType>(maybeSuccessor
						.getKey(), TransitionType.MAYBE));
			}
		}
		return result;
	}
	...
	public BitSet getSuccessors() {
		...
		for (Entry<Action, BitSet> successorByAction : possibleSuccessorsByAction
				.entrySet()) {
