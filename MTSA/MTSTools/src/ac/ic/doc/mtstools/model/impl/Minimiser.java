...
public class Minimiser {
	...
	// IEventState [] T; // tau adjacency lists - stores reflexive transitive
	// closure
	//
	// //maquina a minimizar
	...
	// }
	// for (int i=0; i<m.getStates().length; i++)
	// m.getStates()[i] = EventState.add(m.getStates()[i],new
	// EventState(Declaration.TAU,i));
	// output.out(".");
	// return m;
	// }
	//
	...
	// = EventState.add(machine.getStates()[es],new
	// EventState(machine.getAlphabet().length,es));
	// }
	// if (machine.hasTau()) {
	// initTau();
	...
	// else {
	// m.setEndseq(((Integer)oldtonew.get(new
	// Integer(machine.getEndseq()))).intValue());
	// /* remove marking transition */
	// m.getStates()[m.getEndseq()]
	// = EventState.remove(m.getStates()[m.getEndseq()],new
	// EventState(m.getAlphabet().length,m.getEndseq()));
	// }
	//          
	// for (int i = 0; i<machine.getMaxStates(); i++) {
	// int newi = ((Integer)oldtonew.get(new Integer(i))).intValue();
	// IEventState tmp =
	// EventState.renumberStates(machine.getStates()[i],oldtonew);
	// m.getStates()[newi] = EventState.union(m.getStates()[newi],tmp);
	// }
	//
	// for (int i = 0; i<m.getMaxStates(); i++) // remove reflexive tau
	// m.getStates()[i] = EventState.remove(m.getStates()[i],new
	// EventState(Declaration.TAU,i));
	// return m;
	// }
	//
