...
public class WeakAlphabetMergeBuilder implements MergeBuilder {
	...
	public <S1, S2, A> MTS<Object, A> merge(MTS<S1, A> mtsA, MTS<S2, A> mtsB) {
		...
		RefinementByRelation refA = SemanticType.WEAK
				.getRefinement(alphabetBminusA);
		RefinementByRelation refB = SemanticType.WEAK
				.getRefinement(alphabetAminusB);
		RefinementByRelation refCR = SemanticType.WEAK
				.getRefinement(this.silentActions);

		MTS<Object, A> bestCR = this.cloneMTS(this.getCrOperator().compose(
				mtsA, mtsB));
		bestCR.removeUnreachableStates();
		int maxStates = bestCR.getStates().size() * 4;

		Validate.isTrue(refA.isARefinement(mtsA, bestCR),
				"CR doesn't refine model A");
		Validate.isTrue(refB.isARefinement(mtsB, bestCR),
				"CR doesn't refine model B");

		Queue<Object> stateToProcess = new LinkedList<Object>();

		Set<Set<A>> actionsPartition = this.createActionsPartition(mtsA
				.getActions(), mtsB.getActions());
		...
		do {
			...
			while ((state = stateToProcess.poll()) != null
					&& bestCR.getStates().size() < maxStates
					&& !Thread.currentThread().isInterrupted())
				for (Set<A> actions : actionsPartition) {

					// Collects all transitions by action. If action if
					// unobservable by one of the models,
					// then it also collects all the transitions by all
					// unobservable actions to that model.
					...
					for (A reqAction : actions) {
						reqTransitions.getImage(reqAction).addAll(
								bestCR.getTransitions(state, REQUIRED)
										.getImage(reqAction));
					}

					if (reqTransitions.isEmpty())
						continue;
					...
					for (Set<Pair<A, Object>> coverSet : new PowerSet<Pair<A, Object>>(
							reqTransitions)) {
						...
						if (refA.isARefinement(mtsA, cr)
							...
							doMerge |= this.updateMCRsSet(mcrs, cr, refCR,
									state);
						}
					...
					if (doMerge) {
						...
						if (mcrs.size() > 1) {
							...
							bestCR = this.mergeMCRs(mcrs, state, actions,
									stateToProcess);
							break;
						} else {
							improvements++;
		...
		} while (changed && bestCR.getStates().size() < maxStates
				&& !Thread.currentThread().isInterrupted());

		if (bestCR.getStates().size() == maxStates) {
			System.out
					.println("Merge algorithm stopped after reaching maximum amount of clones");
		}
	...
	private <A> boolean updateMCRsSet(Collection<MTS<Object, A>> mcrs,
			MTS<Object, A> mcrCandidate, RefinementByRelation refCR,
			Object state) {
		boolean addCr = true;
	...
	private <A> void setAsRequiredTransitions(MTS<Object, A> cr,
			Object sourceSate, Set<Pair<A, Object>> targetTransitions) {
		for (Pair<A, Object> reqTransition : targetTransitions) {
			cr.addRequired(sourceSate, reqTransition.getFirst(), reqTransition
					.getSecond());
		}
	}

	private <A> void setAsPossibleTransitions(MTS<Object, A> cr,
			Object sourceState, Set<Pair<A, Object>> targetTransitions) {
		for (Pair<A, Object> reqTransition : targetTransitions) {
			cr.removeRequired(sourceState, reqTransition.getFirst(),
					reqTransition.getSecond());
			cr.addPossible(sourceState, reqTransition.getFirst(), reqTransition
					.getSecond());
		}
	}
	 ...
	 * @return
	 */
	private <A> MTS<Object, A> mergeMCRs(Collection<MTS<Object, A>> mcrs,
			Object stateToClone, Set<A> actions, Collection<Object> newStates) {

		// takes one mcr in order to take from it the common structure of the
		// mcrs.
		MTS<Object, A> mcr0 = mcrs.iterator().next();

		Map<Object, Collection<Object>> state2newStates = this
				.createNewStatesMapping(mcr0, mcrs.size(), stateToClone,
						newStates);

		Object initialState = state2newStates.get(mcr0.getInitialState())
				.iterator().next();

		// creates a new MTS setting the initial state, the actions, and the
		// states.
		...
		for (TransitionType type : TransitionType.values()) {
			...
			for (Object source : mcr0.getStates())
				for (Pair<A, Object> transition : mcr0.getTransitions(source,
						type)) {
					Object target = transition.getSecond();
					A transitionAction = transition.getFirst();

					// if the transition is one of the transitions over which
					// the abstraction is taking place it is ignored.
					if (source.equals(stateToClone)
							&& actions.contains(transitionAction))
						continue;

					for (Object newSource : state2newStates.get(source))
						for (Object newTarget : state2newStates.get(target)) {
							result.addTransition(newSource, transitionAction,
									newTarget, type);
						}
				}
		}
		...
		Iterator<Object> newStateIt = state2newStates.get(stateToClone)
				.iterator();
		while (mcrIt.hasNext()) {
			...
			for (TransitionType type : TransitionType.values()) {
				if (MAYBE.equals(type))
					continue;
				for (Pair<A, Object> transition : mcr.getTransitions(
						stateToClone, type)) {
					A transitionAction = transition.getFirst();

					if (!actions.contains(transitionAction))
						continue;
					Object target = transition.getSecond();
					for (Object newTarget : state2newStates.get(target)) {
						result.addTransition(newSource, transitionAction,
	...
	/**
	 * Returns a map, which maps every mts' state to a singleton with itself as
	 * a unique element except the stateToClone. The mapping for the
	 * stateToClone is a set of new object instance, the cardinality of that set
	 * if amountNewState.
	 * 
	 * @param mts
	 ...
	 * @return
	 */
	private Map<Object, Collection<Object>> createNewStatesMapping(
			MTS<Object, ?> mts, int amountNewState, Object stateToClone,
			Collection<Object> newStates) {
		Map<Object, Collection<Object>> result = new HashMap<Object, Collection<Object>>(
				(int) (mts.getStates().size() / 0.75 + 1));
