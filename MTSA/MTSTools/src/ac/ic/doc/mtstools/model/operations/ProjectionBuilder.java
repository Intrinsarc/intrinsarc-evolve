...
public class ProjectionBuilder {
	...
	public <S, A> Iterator<List<Pair<A, S>>> getProjectionIterator(
		...
		return new ProjectionIterator<S, A>(mts, state, label, type,
				silentActions);
	}

	private static class ProjectionIterator<S, A> implements
		...
		public ProjectionIterator(MTS<S, A> mts, S state, A label,
		...
		private void processPreVisibleTransition() {
			...
			while (this.unprocessed.get(QueueType.POSTACTION).isEmpty()
				...
				Set<S> image = mts.getTransitions(state, this.type).getImage(
						this.label);
