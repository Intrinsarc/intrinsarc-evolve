...
public class StrongPlusCROperator {
	...
	public <S1, S2, A> MTS<Pair<S1, S2>, A> compose(MTS<S1, A> mtsA,
		...
		Set<Pair<S1, S2>> consistencyRelation = this.getStrongConsistency()
		...
		for (Pair<S1, S2> state : result.getStates()) {
			...
			this.processRule(state, mtsA.getTransitions(POSSIBLE).get(
					state.getFirst()), mtsB.getTransitions(POSSIBLE).get(
					state.getSecond()), POSSIBLE, result);

			this.processRule(state, mtsA.getTransitions(REQUIRED).get(
					state.getFirst()), mtsB.getTransitions(POSSIBLE).get(
					state.getSecond()), REQUIRED, result);

			this.processRule(state, mtsA.getTransitions(POSSIBLE).get(
					state.getFirst()), mtsB.getTransitions(REQUIRED).get(
					state.getSecond()), REQUIRED, result);
	...
	private <S1, S2, A> void processRule(Pair<S1, S2> startState,
			BinaryRelation<A, S1> transitionsA,
			BinaryRelation<A, S2> transitionsB,
			TransitionType newTranstitionsType, MTS<Pair<S1, S2>, A> mtsCR) {
		for (Pair<A, S1> transitionA : transitionsA) {
			for (S2 endStateB : transitionsB.getImage(transitionA.getFirst())) {
				Pair<S1, S2> endState = Pair.create(transitionA.getSecond(),
						endStateB);
				if (mtsCR.getStates().contains(endState)) {
					mtsCR.addTransition(startState, transitionA.getFirst(),
							endState, newTranstitionsType);
				}
			}
		}
	}
