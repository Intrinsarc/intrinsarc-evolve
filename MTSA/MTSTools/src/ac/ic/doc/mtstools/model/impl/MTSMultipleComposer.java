...
public class MTSMultipleComposer<State, Action> {
	...
	public MTS<Long, Action> composeAdapter(
			List<MTSCompositionAdapter<Action>> mtss) {

		Set<CompositionState> ready = new HashSet<CompositionState>();
		Queue<CompositionState> toProcess = new LinkedList<CompositionState>();

		CompositionState initial = stateFactory
				.getNextStateFor(buildInitialStatesList(mtss));
		...
		while (!toProcess.isEmpty()) {
			...
			Collection<Transition<Action>> composableTransitions = getComposableTransitions(
					mtss, actualCompositeState);
			for (Iterator<Transition<Action>> it = composableTransitions
					.iterator(); it.hasNext();) {
				Transition<Action> transition = it.next();
				CompositionState toState = transition.getCompositionState();
				if (compositionRulesApplier.composableStates(originalMTSs,
						toState)) {
					Long to = transition.getTo();
					retValue.addState(to);
					retValue.addTransition(actual, transition.getAction(), to,
							transition.getType());
	...
	private void addNewState(CompositionState stateToAdd,
			Set<CompositionState> ready, Queue<CompositionState> toProcess) {
	...
	private List<Map<Action, List<Pair<Long, TransitionType>>>> groupByActionForAll(
			List<MTSCompositionAdapter<Action>> mtss, List<Long> states,
		...
		int i = 0;
		for (MTSCompositionAdapter<Action> compositionAdapter : mtss) {
			Map<Action, List<Pair<Long, TransitionType>>> groupByAction = compositionAdapter
					.groupByAction(states.get(i));
			retValue.add(groupByAction);

			for (Iterator<Action> it = groupByAction.keySet().iterator(); it
					.hasNext();) {
	...
	private void calculateIsInAlphabet(Action action,
			List<MTSCompositionAdapter<Action>> mtss, BitSet isInAlphabetOf) {
		for (int i = 0; i < mtss.size(); i++) {
			isInAlphabetOf.set(i, mtss.get(i).getActions().contains(action));
		}
	}

	private void calculateIsInTransitionsFrom(Action action,
			List<Map<Action, List<Pair<Long, TransitionType>>>> mtss,
			BitSet isInTransitionsFromOf, BitSet isInAlphabetOf) {
		for (int i = isInAlphabetOf.nextSetBit(0); i >= 0; i = isInAlphabetOf
				.nextSetBit(i + 1)) {
			isInTransitionsFromOf.set(i, mtss.get(i).containsKey(action));
		}
	}

	private Collection<Transition<Action>> getComposableTransitions(
			List<MTSCompositionAdapter<Action>> mtss, CompositionState x) {
		...
		for (Iterator<Action> it = allFromActions.iterator(); it.hasNext();) {
			...
			calculateIsInTransitionsFrom(action,
					transisionesDesdeEstadoActualAgrupadasPorAction,
					isInTransitionsFrom, isInAlphabetOf);

			if (isInAlphabetOf.cardinality() > 1
					&& !MTSConstants.TAU.equalsIgnoreCase(action.toString())) {
				if (isInAlphabetOf.equals(isInTransitionsFrom)) {
					...
					for (Pair<Vector<Long>, TransitionType> statesToCompose : estadosAlcanzablesDesdeAccionActual) {
						...
						CompositionState nextState = stateFactory
								.getNextStateFor(statesToCompose.getFirst());
						Transition<Action> transition = new Transition<Action>(
								nextState.getCompositeState(), action,
				...
				for (int mtsPosition = isInTransitionsFrom.nextSetBit(0); mtsPosition >= 0; mtsPosition = isInTransitionsFrom
					...
					for (Pair<Long, TransitionType> transition : transisionesDesdeEstadoActualAgrupadasPorAction
							.get(mtsPosition).get(action)) {
						Vector<Long> nextState = new Vector<Long>(state);
						nextState.set(mtsPosition, transition.getFirst());

						CompositionState nextCompositionState = stateFactory
								.getNextStateFor(nextState);
						Transition<Action> newTransition = new Transition<Action>(
								nextCompositionState.getCompositeState(),
								action, compositionRulesApplier
										.applyCompositionRules(transition
												.getSecond()));
	...
	private Set<Pair<Vector<Long>, TransitionType>> buildReachableStates(
			List<MTSCompositionAdapter<Action>> mtss,
		...
		for (Pair<Long, TransitionType> transition : transisionesDesdeEstadoActualAgrupadasPorAction
				.get(i).get(action)) {
			Vector<Long> firstComponentOfNextState = buildEmptyCompositionState(mtss
					.size());
			firstComponentOfNextState.set(i, transition.getFirst());
			estadosAlcanzablesDesdeAccionActual.add(Pair.create(
					firstComponentOfNextState, transition.getSecond()));
		}

		for (i = isInTransitionsFrom.nextSetBit(i + 1); i >= 0; i = isInTransitionsFrom
				.nextSetBit(i + 1)) {
			estadosAlcanzablesDesdeAccionActual = armoConjuntoEstados(i,
					transisionesDesdeEstadoActualAgrupadasPorAction.get(i).get(
							action), estadosAlcanzablesDesdeAccionActual);
		}
	 ...
	 */
	private Set<Pair<Vector<Long>, TransitionType>> armoConjuntoEstados(
			int mtsPosition,
		...
		for (Pair<Long, TransitionType> transitionOnActualAction : transitionsOnActualAction) {
			...
			for (Pair<Vector<Long>, TransitionType> acumulatedState : acumulatedCompositeStatesSet) {
				List<Long> compositeState = new Vector(acumulatedState
						.getFirst());
				compositeState.set(mtsPosition, transitionOnActualAction
						.getFirst());

				TransitionType compositeTransitionType = compositionRulesApplier
						.applyCompositionRules(transitionOnActualAction,
								acumulatedState);
	 ...
	 * @return
	 */
	private List<MTSCompositionAdapter<Action>> buildMTSCompositionAdaptersFor(
			List<MTS<Long, Action>> mtss) {
	...
	public Vector<Long> buildInitialStatesList(
			List<MTSCompositionAdapter<Action>> mtss) {
		Vector<Long> retValue = new Vector<Long>();
		for (MTSCompositionAdapter<Action> compositionAdapter : mtss) {
