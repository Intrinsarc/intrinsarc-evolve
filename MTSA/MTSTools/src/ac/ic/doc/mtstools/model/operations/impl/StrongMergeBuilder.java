...
public class StrongMergeBuilder implements MergeBuilder {
	...
	public <S1, S2, A> MTS<Object, A> merge(MTS<S1, A> mtsA, MTS<S2, A> mtsB) {
		...
		MTS<Object, A> cr = this.cloneMTS(this.getCrOperator().compose(mtsA,
				mtsB));
		cr.removeUnreachableStates();

		BinaryRelation<S1, Object> refRelationACR = this.getRefinement()
				.getLargestRelation(mtsA, cr);
		BinaryRelation<S2, Object> refRelationBCR = this.getRefinement()
				.getLargestRelation(mtsB, cr);
		BinaryRelation<Object, Object> refRelationCRCR = this.getRefinement()
				.getLargestRelation(cr, cr); // new
												// IdentityRelation<Object>(cr.getStates());
		...
		do {
			...
			for (A action : cr.getActions())
				for (Object state : cr.getStates()) {
					...
					Set<Object> reqReachableStates = cr.getTransitions(state,
							REQUIRED).getImage(action);

					if (reqReachableStates.isEmpty())
						continue;

					reqReachableStates = new HashSet<Object>(reqReachableStates); // clone
																					// the
																					// set
																					// in
																					// order
					...
					this.setAsPossibleTransitions(cr, state, action,
							reqReachableStates);

					for (Set<Object> coverSet : new PowerSet<Object>(
							reqReachableStates)) {
						this.setAsRequiredTransitions(cr, state, action,
								coverSet);
						...
						if (this.getRefinement().isAValidRelation(mtsA, cr,
							...
							doMerge |= this.updateMCRsSet(mcrs, cr,
									refRelationCRCR);
						}
					...
					if (doMerge) {
						...
						if (mcrs.size() > 1) {
							...
							refRelationACR = this.getRefinement()
									.getLargestRelation(mtsA, bestCR);
							refRelationBCR = this.getRefinement()
									.getLargestRelation(mtsB, bestCR);
							refRelationCRCR = this.getRefinement()
									.getLargestRelation(bestCR, bestCR); // TODO
																			// prove
																			// that
																			// this
																			// relation
	...
	private <A> boolean updateMCRsSet(Collection<MTS<Object, A>> mcrs,
			MTS<Object, A> mcrCandidate,
			BinaryRelation<Object, Object> refRelationCRCR) {
		boolean addCr = true;
		for (Iterator<MTS<Object, A>> iter = mcrs.iterator(); iter.hasNext();) {
			MTS<Object, A> mcr = iter.next();
			if (this.getRefinement().isAValidRelation(mcr, mcrCandidate,
					refRelationCRCR)) {
				addCr = false;
				break;
			}

			if (this.getRefinement().isAValidRelation(mcrCandidate, mcr,
					refRelationCRCR)) {
				iter.remove();
			}
		}
	...
	private <A> void setAsRequiredTransitions(MTS<Object, A> cr,
			Object sourceSate, A action, Set<Object> targetStates) {
	...
	private <A> void setAsPossibleTransitions(MTS<Object, A> cr,
			Object sourceState, A action, Set<Object> targetStates) {
		for (Object toState : targetStates) {
			...
			for (Object source : mcr0.getStates())
				for (Pair<A, Object> transition : mcr0.getTransitions(source,
					...
					if (source.equals(stateToClone)
							&& transitionAction.equals(action))
						continue;
		...
		while (mcrIt.hasNext()) {
			...
			for (TransitionType type : TransitionType.values()) {
				...
				for (Pair<A, Object> transition : mcr.getTransitions(
					...
					if (!transitionAction.equals(action))
						continue;
					Object target = transition.getSecond();
					for (Object newTarget : state2newStates.get(target)) {
						result.addTransition(newSource, transitionAction,
