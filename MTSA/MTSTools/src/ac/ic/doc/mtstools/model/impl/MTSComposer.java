...
public class MTSComposer {
	...
	public <Action> MTS<Long, Action> compose(MTS<Long, Action> mtsA,
			MTS<Long, Action> mtsB) {
		Set<Long> ready = new HashSet<Long>();
		Queue<Long> toProcess = new LinkedList<Long>();
		Long initial = code(mtsA, mtsB, mtsA.getInitialState(), mtsB
				.getInitialState());
		...
		while (!toProcess.isEmpty()) {
			...
			for (Iterator<Transition<Action>> it = composableTransitions
				...
				retValue.addTransition(actual, transition.getAction(),
						transition.getTo(), transition.getType());
	...
	private static <Action> Collection<Transition<Action>> getComposableTransitions(
			MTS<Long, Action> mtsA, MTS<Long, Action> mtsB, Long x) {
		Collection<Transition<Action>> retValue = new ArrayList<Transition<Action>>();
		Pair<Long, Long> state = decode(mtsA, mtsB, x);
		Map<Action, List<Pair<Long, TransitionType>>> fromA = groupByAction(getTransitions(
				mtsA, state.getFirst()));
		Map<Action, List<Pair<Long, TransitionType>>> fromB = groupByAction(getTransitions(
				mtsB, state.getSecond()));
		...
		for (Iterator<Action> it = allActions.iterator(); it.hasNext();) {
			...
			if (isInA && isInB) {
				if (isInFromA && isInFromB) {
					for (Iterator<Pair<Long, TransitionType>> itA = fromA.get(
							action).iterator(); itA.hasNext();) {
						Pair<Long, TransitionType> toA = itA.next();
						for (Iterator<Pair<Long, TransitionType>> itB = fromB
								.get(action).iterator(); itB.hasNext();) {
							Pair<Long, TransitionType> toB = itB.next();
							Long nextState = code(mtsA, mtsB, toA.getFirst(),
									toB.getFirst());
							TransitionType type;
							...
							retValue.add(new Transition<Action>(nextState,
									action, type));
						}
					}
				}
				...
				if (fromA.get(action) != null) {
					for (Iterator<Pair<Long, TransitionType>> it2 = fromA.get(
							action).iterator(); it2.hasNext();) {
						Pair<Long, TransitionType> transition = it2.next();
						Long nextState = code(mtsA, mtsB,
								transition.getFirst(), state.getSecond());
						retValue.add(new Transition<Action>(nextState, action,
								transition.getSecond()));
					}
				} else {
					for (Iterator<Pair<Long, TransitionType>> it2 = fromB.get(
							action).iterator(); it2.hasNext();) {
						Pair<Long, TransitionType> transition = it2.next();
						Long nextState = code(mtsA, mtsB, state.getFirst(),
								transition.getFirst());
						retValue.add(new Transition<Action>(nextState, action,
								transition.getSecond()));
					}
	...
	private static <Action> Map<Action, List<Pair<Long, TransitionType>>> groupByAction(
		...
		for (Iterator<Transition<Action>> it = transitions.iterator(); it
				.hasNext();) {
			Transition<Action> transition = it.next();
			if (!retValue.containsKey(transition.getAction())) {
				retValue.put(transition.getAction(),
						new ArrayList<Pair<Long, TransitionType>>());
			}
	...
	private static <Action> Collection<Transition<Action>> getTransitions(
			MTS<Long, Action> mts, Long state) {
		Collection<Transition<Action>> retValue = new ArrayList<Transition<Action>>();
	...
	private static <Action> Collection<Transition<Action>> getTransitions(
			MTS<Long, Action> mts, Long state, TransitionType type) {
		Collection<Transition<Action>> retValue = new ArrayList<Transition<Action>>();
		for (Iterator<Pair<Action, Long>> iter = mts
				.getTransitions(state, type).iterator(); iter.hasNext();) {
			Pair<Action, Long> transition = iter.next();
			retValue.add(new Transition<Action>(transition.getSecond(),
					transition.getFirst(), type));
		}
		return retValue;
	}

	private static <Action> Long code(MTS<Long, Action> mtsA,
			MTS<Long, Action> mtsB, Long a, Long b) {
		return a * mtsB.getStates().size() + b;
	}

	private static <Action> Pair<Long, Long> decode(MTS<Long, Action> mtsA,
			MTS<Long, Action> mtsB, Long x) {
		return Pair.create(x / mtsB.getStates().size(), x
				% mtsB.getStates().size());
	}
