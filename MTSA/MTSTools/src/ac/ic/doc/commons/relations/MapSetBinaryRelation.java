...
public class MapSetBinaryRelation<E1, E2> extends AbstractSet<Pair<E1, E2>>
		implements BinaryRelation<E1, E2> {
	...
	public MapSetBinaryRelation() {
		this(
				(Class<? extends ConcurrentMap<E1, Set<E2>>>) ConcurrentHashMap.class,
				(Class<? extends Set<E2>>) HashSet.class);
	}

	private MapSetBinaryRelation(
			Class<? extends ConcurrentMap<E1, Set<E2>>> mapClass,
			Class<? extends Set<E2>> setClass) {
		try {
			this.size = 0;
	 ...
	 * 
	 * @see
	 * ac.ic.doc.common.relations.BinaryRelation#addPair(ac.ic.doc.common.relations
	 * .BinaryRelationImpl.Pair)
	 */
	...
	public Iterator<Pair<E1, E2>> iterator() {
		return new Iterator<Pair<E1, E2>>() {
			...
			{
				this.firstIt = MapSetBinaryRelation.this.internalRelation
						.entrySet().iterator();
			...
			public boolean hasNext() {
				return this.actualSetIt.hasNext()
						|| this.nextNotEmptyEntry != null;
			}

			public Pair<E1, E2> next() {
				if (!this.actualSetIt.hasNext()
						&& this.nextNotEmptyEntry != null) {
					this.actualSetIt = this.nextNotEmptyEntry.getValue()
							.iterator();
			...
			private void updateNextNotEmptyEntry() {
				...
				while (this.firstIt.hasNext()
						&& (entry = this.firstIt.next()).getValue().isEmpty())
					;
