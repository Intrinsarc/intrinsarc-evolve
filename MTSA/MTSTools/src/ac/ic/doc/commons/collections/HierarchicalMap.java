package ac.ic.doc.commons.collections;
 ...
 * @author dfischbein
 */
public class HierarchicalMap<K, V> extends AbstractMap<K, V> implements
		Serializable {
	...
	/**
	 * Establishes if a key is contained either in the local map or the parent
	 * map.
	 * 
	 * @see Map#containsKey(java.lang.Object)
	 */
	public boolean containsKey(Object key) {
		return this.getLocalMap().containsKey(key)
				|| this.getParentOrEmptyMap().containsKey(key);
	}

	/**
	 * Establishes if a value is contained in the local map, or if it not,
	 * checks if the value is in the parent map related with an inherited key.
	 * 
	...
	public boolean containsValue(Object value) {
		if (this.getLocalMap().containsValue(value)) {
			...
			if (this.getParentOrEmptyMap().containsValue(value)) {
				Iterator<Entry<K, V>> parentEntryIt = this
						.getParentOrEmptyMap().entrySet().iterator();
	...
	/**
	 * Returns a <b>unmodifiable </b> set view of the mappings contained in this
	 * map. Each element in this set is a Map.Entry.
	 * 
	 * @see java.util.Map#entrySet()
	 */
	public Set<Entry<K, V>> entrySet() {
		Set<Entry<K, V>> entrySet = new HashSet<Entry<K, V>>(this.getLocalMap()
				.entrySet());

		Iterator<Entry<K, V>> iter = this.getParentOrEmptyMap().entrySet()
				.iterator();
		while (iter.hasNext()) {
	...
	/**
	 * Establishes if a Hierarchical Map is equals to the received object based
	 * on the comparisson of both the Local Map and the Parent Map. Both Local
	 * and Parents Maps must be equal for two Hierarchical Maps to be equals,
	 * for example, two Hierarchical Maps can have equal entrySets yet some
	 * particular values of one of them can be inherited while not being the
	 * case in the other.
	 */
	public boolean equals(Object o) {
		if (o instanceof HierarchicalMap) {
			...
			return this.getLocalMap().equals(o2.getLocalMap())
					&& this.getParentOrEmptyMap().equals(
							o2.getParentOrEmptyMap());
		}
		return false;
	}

	/**
	 * Retrieves a map entry for a given key from the local map first and if not
	 * found from the parent map.
	 * 
	...
	public boolean isKeyInherited(K key) {
		return this.getParentOrEmptyMap().containsKey(key)
				&& !this.getLocalMap().containsKey(key);
	}
	...
	/**
	 * Returns an <b>unmodifiable </b> set set view of the keys contained in
	 * this map
	 * 
	...
	/**
	 * Associates the specified value with the specified key in the local map .
	 * If the hierarchy previously contained a mapping for this key, the old
	 * value is replaced.
	 * 
	...
	/**
	 * Copies all of the mappings from the specified map to the local map. These
	 * mappings will replace any mappings that this map had for any of the keys
	 * currently in the local map.
	 * 
	...
	/**
	 * Returns an <b>unmodifiable </b> collection view of the values contained
	 * in this map.
	 * 
