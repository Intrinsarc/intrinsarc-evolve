...
public class HPWindow extends JFrame implements LTSManager, LTSInput,
	...
	public HPWindow(AppletButton isap) {
		...
		for (String uiName : Arrays.asList(new String[] { "ames-learning-ui",
				"ames-interface-learning-ui" })) {
			try {
				// Initialise so that the absense of the lstar package doesn't
				// stop
				// compilation or execution.
				Object compositionUI = Class
						.forName(System.getProperty(uiName)).getConstructor(
								LTSInput.class, LTSOutput.class,
								LTSError.class, LTSManager.class,
								EventManager.class).newInstance(this, this,
								this, this, eman);

				Map<Component, String> menuItems = (Map) compositionUI
						.getClass().getMethod("getMenuItems").invoke(
								compositionUI);

				Set<? extends Component> windows = (Set) compositionUI
