...
public class Expression {
	...
	protected static int countSet(Symbol token, Hashtable locals,
			Hashtable globals) {
	...
	protected static String indexSet(Stack s, Hashtable locals,
			Hashtable globals) {
		s.pop();
	...
	private static int eval(Stack s, Hashtable locals, Hashtable globals) {
		...
		case Symbol.UPPERIDENT:
			...
			if (constant == null)
				Diagnostics.fatal(
						"constant or parameter not defined- " + token, token);
			if (constant.isLabel())
				Diagnostics.fatal(
						"not integer constant or parameter- " + token, token);
