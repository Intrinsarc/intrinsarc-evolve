...
/**
 * Compresses integer vector of a a composite machine state into a bit packed
 * byte vector
 * 
 */
...
public class StateCodec {
	...
	static int masks[] = { 0x0, 0x1, 0x3, 0x7, 0xF, 0x1F, 0x3F, 0x7F, 0xFF,
	...
	public StateCodec(int[] sm) {
		...
		for (int i = sm.length - 1; i >= 0; --i) {
			...
			if ((NBIT + bitSize[i]) > longwords * 64) {
				NBIT = longwords * 64;
				++longwords;
	...
	/*
	 * test harness public static void main ( String[] args ) { int test[] =
	 * {1024, 1024, 1024,1024,1024,1024,1024,1024,1024,1024, 1024,
	 * 1024,1024,1024,1024,1024,1024,1024}; StateCodec c = new StateCodec(test);
	 * System.out.print("bitSize: {"); for(int i=0; i<c.bitSize.length;++i)
	 * System.out.print(" "+c.bitSize[i]+","); System.out.println("}");
	 * System.out.println("NBIT: "+c.NBIT); System.out.println("NBYTE:
	 * "+c.NBYTE); System.out.print("boundaries: {"); for(int i=0;
	 * i<c.boundaries.length;++i) System.out.print(" "+c.boundaries[i]+",");
	 * System.out.println("}"); int state[] =
	 * {999,2,999,0,4,5,6,7,1,999,1,999,999,4,5,6,7,1}; byte code[] =
	 * c.encode(state); int result[] = c.decode(code); System.out.print("code:
	 * {"); for(int i=0; i<code.length;++i) System.out.print(" "+code[i]+",");
	 * System.out.println("}"); System.out.print("result: {"); for(int i=0;
	 * i<c.bitSize.length;++i) System.out.print(" "+result[i]+",");
	 * System.out.println("}"); System.out.println("hash: "+hash(code)); }
	 */
