...
class Converter extends CompactState {
	...
	private void makeStates(LabelFactory lf) {
		maxStates = g.getNodeCount() + iacc + 1; // add extra node for
		// completion
	...
	private void addAccepting() {
		for (int id = 0; id < maxStates - 1; id++) {
			if (accepting.get(id)) {
				states[id + iacc] = EventState.add(states[id + iacc],
						new EventState(alphabet.length - 1, id + iacc));
			}
		}
	}
	...
	void addTrueNode(int id, HashMap trl) {
		...
		for (int i = 0; i < tr.size(); ++i) {
			if (tr.get(i)) {
				states[id] = EventState.add(states[id], new EventState(i + 1,
						id));
			}
		}
	}
	...
	void complete(int id, BitSet all) {
		for (int i = 0; i < alphabet.length - 2; ++i) {
			if (!all.get(i)) {
				states[id + iacc] = EventState.add(states[id + iacc],
						new EventState(i + 1, maxStates - 1));
			}
		}
	}
	...
	void addEdge(Edge e, int id, HashMap trl, BitSet all) {
		...
		for (int i = 0; i < tr.size(); ++i) {
			if (tr.get(i)) {
				states[id + iacc] = EventState.add(states[id + iacc],
						new EventState(i + 1, e.getNext().getId() + iacc));
			}
		}
	}
	...
	public void printFSP(PrintStream printstream) {
		...
		for (Iterator iterator = g.getNodes().iterator(); iterator.hasNext(); printNode(
				node, printstream)) {
			printstream.println(",");
	...
	protected BitSet getAcceptance() {
		...
		for (Iterator iterator1 = g.getNodes().iterator(); iterator1.hasNext();) {
			gov.nasa.ltl.graph.Node node1 = (gov.nasa.ltl.graph.Node) iterator1
					.next();
	...
	void printNode(gov.nasa.ltl.graph.Node n, PrintStream printstream) {
		...
		for (Iterator iterator = n.getOutgoingEdges().iterator(); iterator
				.hasNext();) {
