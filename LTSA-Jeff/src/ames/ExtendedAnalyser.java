...
// provided ports
	private IExtendedAnalyserAnalyserImpl analyser_IExtendedAnalyserProvided = new IExtendedAnalyserAnalyserImpl();
// setters and getters
	...
	public ames.IExtendedAnalyser getAnalyser_IExtendedAnalyser(Class<?> required) { return analyser_IExtendedAnalyserProvided; }
// end generated code
	...
	public final static int DEADLOCK = 1;
	public final static int ERROR = 2;
	public final static int FOUND = 3;
	...
	private class IExtendedAnalyserAnalyserImpl implements IExtendedAnalyser, IAnimator
	{

		@Override
		public void activate(CompositeState composite, LTSOutput out, boolean ignoreAsterisk)
		{
			cs = composite;
			output = out;
	...
	// action name chosen - only valid after a step
		...
		public BitSet initialise(Vector menu) {
			...
			// to 0
			if (events != null)
				events.post(new LTSEvent(LTSEvent.NEWSTATE, currentA)); // initialise
			// animation
			// to 0
			getMenu(menu);
		...
		public List eligibleTransitions(int[] state) {
		...
		public CompactState getErrorStateMachine() {
			return sm[errorMachine];
		}
		...
		private CompactState private_compose(boolean dohiding) {
			...
			CompactState c = new CompactState(stateCount.get(), cs.name, analysed,
			...
			long finish = System.currentTimeMillis();
			outStatistics(stateCount.get(), compTrans.size());
				...
				if (analysed.marked()) {
					...
					if (transitions == null) {
						...
						while (e.hasNext()) {
							int[] next = (int[]) e.next();
							byte[] code = coder.encode(next);
							compTrans.add(stateCount.get() - 1, code, next[Nmach]);
							if (code == null) {
								int i = 0;
								while (next[i] >= 0)
									i++;
		...
		private int newState_analyse(byte[] fromState, byte[] target) {
			...
			while (!hh.empty()) {
				...
				if (stateCount.get() % 10000 == 0) {
					...
					outStatistics(stateCount.get(), nTrans);
				}
				// determine eligible transitions
				List transitions = eligibleTransitions(state);
				hh.pop();
		...
		private int[] step(int action) { // take step from current and post it
			...
			if (events != null) {
				events.post(new LTSEvent(LTSEvent.NEWSTATE, tmp, actionName[action]));
			}
			return tmp;
		}
		...
		@Override
		public void makePossiblePartialOrderReduction(MyHashQueue hh)
		{
			if (partialOrderReduction)
				partial = new PartialOrder(alphabet, actionName, sm,
		...
		@Override
		public void outStatistics(int nTrans)
		{
			outStatistics(stateCount.get(), nTrans);
		}

		@Override
		public LinkedList setTrace(MyHashQueue hh, MyHashQueueEntry qe) {
			trace = hh.getPath(qe, actionName);
			return trace;
		}

		@Override
		public void setErrorMachine(int i) {
			errorMachine = i;
		}
