...
public class Minimiser {
	...
	public CompactState minimise() {
		...
		output.outln("");
		output.outln("Minimised States: " + c.maxStates + " in "
				+ (finish - start) + "ms");
		return c;
	}
	...
	private CompactState makeNewMachine() {
		...
		if (machine.endseq < 0)
			...
			m.endseq = ((Integer) oldtonew.get(new Integer(machine.endseq)))
					.intValue();
		...
		for (int i = 0; i < machine.maxStates; i++) {
			...
			EventState tmp = EventState.renumberStates(machine.states[i],
					oldtonew);
