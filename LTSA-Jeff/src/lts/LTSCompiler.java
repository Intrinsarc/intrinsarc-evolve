...
public class LTSCompiler implements ILifecycle
{
...
// end generated code
	...
	public lts.LTSCompiler getMain_LTSCompiler(Class<?> required) { return this; }
	private Lex lex;
	...
	{
		lex = new Lex(inout);
		Diagnostics.init(inout_LTSOutputRequired);
		SeqProcessRef.output = inout_LTSOutputRequired;
		StateMachine.output = inout_LTSOutputRequired;
	...
	public CompositeState compile(String name) {
		...
		AssertDefinition.compileAll(inout_LTSOutputRequired);
	...
	private void compileProcesses(Hashtable h, Hashtable compiled) {
		...
		while (e.hasMoreElements()) {
			...
			if (!p.imported()) {
				...
				inout_LTSOutputRequired.outln("Compiled: " + c.name);
				compiled.put(c.name, c);
			} else {
				CompactState c = new AutCompactState(p.name, p.importFile);
				inout_LTSOutputRequired.outln("Imported: " + c.name);
				compiled.put(c.name, c);
			}
		}
		AssertDefinition.compileConstraints(inout_LTSOutputRequired, compiled);
	}
		...
		while (current.kind != Symbol.EOFSYM) {
			if (current.kind == Symbol.CONSTANT) {
				...
				if (current.kind != Symbol.OR) {
					...
					c.output = inout_LTSOutputRequired;
	...
	private ProcessSpec importDefinition() {
		...
		p.importFile = new File(currentDirectory.get(), current.toString());
		return p;
	}
