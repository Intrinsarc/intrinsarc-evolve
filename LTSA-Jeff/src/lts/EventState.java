...
public class EventState {
	...
	public EventState nondet;// used for additional non-deterministic transitions
			...
			while (q != null) {
				...
				if (o != null) {
					if (o instanceof Integer) {
						newhead = add(newhead, new EventState(((Integer) o)
								.intValue(), q.next));
					} else {
						Vector v = (Vector) o;
						for (Enumeration e = v.elements(); e.hasMoreElements();) {
							newhead = add(newhead, new EventState(((Integer) e
									.nextElement()).intValue(), q.next));
						}
					}
				}
				q = q.nondet;
	...
	public static EventState renumberStates(EventState head, MyIntHash oldtonew) {
		...
		while (p != null) {
			EventState q = p;
			while (q != null) {
				int next = q.next < 0 ? Declaration.ERROR : oldtonew
						.get(q.next);
				newhead = add(newhead, new EventState(q.event, next));
				q = q.nondet;
			}
	...
	public static int search(EventState trace, EventState[] states,
		...
		int id = 0;
		EventState val[] = new EventState[states.length + 1]; // shift by 1 so
		// ERROR is 0
		while (head != null) {
			int k = head.next;
