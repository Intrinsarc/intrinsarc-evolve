...
/**
 * Utility class for traversing XML DOM trees. This is an abstract class that
 * has methods which are called as different elements are located in a DOM tree.
 * It is designed to be extended for each nore in a specific DOM tree, with its
 * methods overriden to handle specific elements and attributes of the tree.
 * 
 * @deprecated The design of this class is misguided, it is better to use SAX,
 *             and SAXTraverser. Will be removed from future FreeHEP releases
 * @author Tony Johnson (tonyj@slac.stanford.edu)
 * @version $Id: XMLNodeTraverser.java,v 1.1 2009-03-04 22:46:59 andrew Exp $
 */
public abstract class XMLNodeTraverser
{
  ...
  /**
   * Called to handle a subnode of the node being traversed. The default
   * implementation calls handleElement, handleTextNode or handleOtherNode as
   * appropriate.
   * 
   * @param node
   *          The subnode being handled
   * @param name
  ...
  /**
   * Called to handle the attributes associated with the element being
   * traversed. The default implementation calls handleAttributeNode for each
   * attribute
   * 
   * @param nnm
   *          The attrinbutes being handled
   */
  ...
  /**
   * Handle a node other than an Element or TextNode. The default implementation
   * throws an exception
   * 
   * @param node
  ...
  /**
   * Convenience method to get the value of some attribute. Note, this will
   * always return null if called while the traverse method is not active.
