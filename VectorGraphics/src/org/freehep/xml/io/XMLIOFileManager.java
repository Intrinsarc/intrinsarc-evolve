...
public class XMLIOFileManager extends XMLIOStreamManager
{
  ...
  /**
   * After opening the input file, converts the text in xml format and returns
   * the main root Element
   * 
   * @return the jdom main Element or <code>null</code> if something went
   *         wrong in the text to xml conversion (the trace is printed).
   * @exception JDOMException
   *              from XMLIOStreamManager
  ...
  {
    if (isXMLFileZipped())
      super.setXMLOutputStream(new GZIPOutputStream(new FileOutputStream(xmlFile)));
    else
      super.setXMLOutputStream(new FileOutputStream(xmlFile));
