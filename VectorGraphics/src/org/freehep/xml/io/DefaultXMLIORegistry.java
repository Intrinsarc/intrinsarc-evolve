...
public class DefaultXMLIORegistry implements XMLIORegistry
{
   ...
   * 
   * @param xmlioFactory
   *          the XMLIOFactory being registered
   * @param factoryId
  ...
  public void register(Object obj)
  {
    if (!(obj instanceof XMLIOFactory) && !(obj instanceof XMLIOProxy))
      throw new IllegalArgumentException("Cannot register object " + obj + "."
    ...
    if (obj instanceof XMLIOFactory)
    {
      ...
      {
        if (factoryHash.containsKey(classes[i]))
    ...
    if (obj instanceof XMLIOProxy)
    {
      ...
      for (int i = 0; i < classes.length; i++)
      {
        if (proxyHash.containsKey(classes[i]))
  ...
  /**
   * Get the XMLIOFactory corresponding to the identification string
   * <code>factoryId<\code>
