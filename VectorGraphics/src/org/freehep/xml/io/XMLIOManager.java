...
public class XMLIOManager
{
   ...
   * 
   * @param xmlioRegistry
   *          The XMLIORegistry from which the XMLIOManager gets the
   *          XMLIOFactories and the XMLIOProxies to save and restore objects.
   * @param xmlioStreamManager
   *          The xmlioStreamManager that deals with the input/output file.
   * 
   */
   ...
   * 
   * @param nodeEl
   *          The node containing the object's information. This method should
   *          be used by the user to restore nested objects within the restore
   *          method of an object implementing the XMLIO interface.
   * @return The restored object. The object is first created by invoking the
   *         create method on the corresponding XMLIOFactory. Once created the
   *         restore method is invoked either on the object itself (if it
   *         implements the XMLIO interface) or on the appropriate XMLIOProxy.
   * 
   */
  public Object restore(Element nodeEl)
  {
    ...
    if (objIdRef != null)
      if (useId)
        if (isObjIdRegistered(objIdRef))
          ...
          return restore((Element) refTable.get(objIdRef));

    String objId = (String) nodeEl.getAttributeValue("objId");
    if (useId && isObjIdRegistered(objId))
      return getObj(objId);
    ...
    Object xmlio = getXMLIOFactory(objClass).createObject(objClass);
    if (useId)
      registerObj(xmlio, objId);
    if (xmlio instanceof XMLIO)
      ((XMLIO) xmlio).restore(this, nodeEl);
    // else getXMLIOProxy( xmlio.getClass() ).restore( xmlio, this, nodeEl);
    else
      getXMLIOProxy(objClass).restore(xmlio, this, nodeEl);
    return xmlio;
  }
   ...
   * @return The jdom Element containing the object's unique identification
   *         string (in the attribute "objId") and whose name is the object's
   *         identifier. The user will have to append to this node all the
   *         attributes and other jdom elemets required to save the object's
   *         configuration.
   * 
   */
  ...
  {
    String nodeName;
    if (nodeHash.containsKey(clazz))
      nodeName = (String) nodeHash.get(clazz);
    else
      nodeName = (String) xmlioRegistry.getClassId(clazz);
    Element el = new Element(nodeName);

    if (isObjRegistered(obj))
      el.setAttribute("objIdRef", getObjId(obj));
    else
    {
      if (useId)
        el.setAttribute("objId", registerObj(obj));
      if (obj instanceof XMLIO)
        ((XMLIO) obj).save(this, el);
      else
        getXMLIOProxy(clazz).save(obj, this, el);
    }
    return el;
  }
   ...
   *          The Class.
   * @param id
   *          The new identifier for the class.
   * 
   */
  ...
  private String makeObjId(Object obj)
  {
    ...
    int i = 1;
    if (idHash.containsKey(idName))
      i = ((Integer) (idHash.get(idName))).intValue() + 1;
   ...
   * 
   * @param obj
   *          the object whose unique id we are looking for
   * @return the unique String Id of object <code>obj</code> if no string is
   *         found the object itself is returned
   */
  private String getObjId(Object obj)
  {
    for (Enumeration keys = objIdHash.keys(); keys.hasMoreElements();)
    {
      ...
      if (objIdHash.get(key).equals(obj))
        return key;
    }
    return (String) obj;
  }
  ...
  /**
   * Get the object corresponding to unique Id for cross-reference among xml
   * nodes
   * 
   * @param objId
   *          the object's unique String Id
   * @return the object corrisponding to unique Id <code>objId</code> if the
   *         object is registered or the string <code>objId</code> if the
   ...
   * @return <code>true</code> if the object is registered <code>false</code>
   *         otherwise
   * 
   */
