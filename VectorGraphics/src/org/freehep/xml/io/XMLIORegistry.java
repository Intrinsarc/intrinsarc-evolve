...
/**
 * Interface of an XMLIORegistry. The XMLIOFactories used in the restore
 * procedure of an object and the XMLIOProxyes should be register to an
 * XMLIORegistry that should be set in the XMLIOManager passed to the restore
 * method.
 * 
 * @author turri
 * @version 1.0
...
public interface XMLIORegistry
{
   ...
   * @throws An
   *           IllegalArgumentException if the object does not implement either
   *           XMLIOFactory or XMLIOProxy.
   * 
   */
  void register(Object obj) throws IllegalArgumentException;

  /**
   * Get the appropriate XMLIOFactory to create the object that wrote the xml
   * node. The XMLIOFactory identification string is attached to the node's
   * attribute <code>xmlioFactory<\code>.
  ...
  /**
   * Get the appropriate XMLIOProxy to restore the object that wrote the xml
   * node. The XMLIOProxy identification string is the node name. The
   * identification string is assigned by the XMLIOManager by using the object's
   * final part of the Class name.
   * 
   * @param objClass
   *          The Class of the object to be restored.
   * @return The corresponding XMLIOProxy.
   * @throws IllegalArgumentException
   *           if the XMLIOProxy does not exist.
   * 
   */
