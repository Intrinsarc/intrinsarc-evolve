...
public class ObjEFProxyFFactory implements org.freehep.xml.io.XMLIOProxy, org.freehep.xml.io.XMLIOFactory
{
  ...
  /**
   * Returns the identifiers of the objects that the factory is able to create.
   * This identification is left by the object in the jdom node during the save
   * procedure and should be used to identify the objectFactory during the
   * restore phase.
   * 
   ...
   * @param nodeEl
   *          Is the jdom node containing the object's configuration info
   */
   ...
   * 
   * @param obj
   *          The object to be saved to xml.
   * @param xmlioManager
   *          The objects ID manager.
   * @return Element A jdom node containing the object's configuration info.
   * @throws An
   *           IllegalArgumentException of the object is not saveble by this
   *           proxy.
   * 
   */
  ...
  {
    if (objClass == ObjF.class)
      return new ObjF();
    else
      throw new IllegalArgumentException("ObjEFProxyFFactory cannot create object of class " + objClass);
  }
