...
public class ObjBCDEFactory implements XMLIOFactory
{
  ...
  {
    if (objClass == ObjB.class)
      return new ObjB();
    else
      if (objClass == ObjC.class)
        return new ObjC();
      else
        if (objClass == ObjD.class)
          return new ObjD();
        else
          if (objClass == ObjE.class)
            return new ObjE();
          else
            throw new IllegalArgumentException("ObjBCDEFactory cannot create object of class " + objClass);
  }

  /**
   * Returns the identifier of the objects that the factory is able to restore.
   * This identifier is left by the object in the jdom node during the save
