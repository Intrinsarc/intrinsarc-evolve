 ...
 * @version $Id: EMFDisplay.java,v 1.1 2009-03-04 22:46:50 andrew Exp $
 */
import java.awt.*;
...
public class EMFDisplay extends JPanel
{
  ...
  private void map(Tag tag, Graphics2D g2)
  {
    if (tag instanceof Arc)
    {
      ...
      // );
      // The points (nLeftRect, nTopRect) and (nRightRect, nBottomRect) specify
      // the bounding rectangle.
      // An ellipse formed by the specified bounding rectangle defines the curve
      // of the arc.
      // The arc extends in the current drawing direction from the point where
      // it intersects the
      // radial from the center of the bounding rectangle to the (nXStartArc,
      // nYStartArc) point.
      // The arc ends where it intersects the radial from the center of the
      // bounding rectangle to
      // the (nXEndArc, nYEndArc) point. If the starting point and ending point
      // are the same,
      ...
      Arc2D arc2d = new Arc2D.Double(arc.getStart().x, arc.getStart().y, arc.getBounds().width, arc.getBounds().height,
          alpha0, alpha1 - alpha0, Arc2D.OPEN);
      currentShape = arc2d;
      g2.draw(arc2d);
    }
    else
      if (tag instanceof ArcTo)
      {
        ...
        // counterclockwise from the point where it intersects the radial line
        // from the center of the bounding
        // rectangle to the (nXRadial1, nYRadial1) point. The arc ends where it
        // intersects the radial line from
        // the center of the bounding rectangle to the (nXRadial2, nYRadial2)
        // point. If the starting point and
        // ending point are the same, a complete ellipse is drawn.
        //
        // A line is drawn from the current position to the starting point of
        // the arc.
        // If no error occurs, the current position is set to the ending point
        // of the arc.
        //
          ...
          if (tag instanceof Chord)
          {
            ...
            if (tag instanceof CloseFigure)
            {
              ...
              if (tag instanceof CreatePen)
              {
                ...
                // The pen can subsequently be selected into a device context
                ...
                if (lpen.getPenStyle() == EMFConstants.PS_DASH)
                {
                  ...
                  if (lpen.getPenStyle() == EMFConstants.PS_DASHDOT)
                  {
                    dash = new float[]{5, 2, 1, 2};
                  }
                  else
                    if (lpen.getPenStyle() == EMFConstants.PS_DASHDOTDOT)
                    {
                      dash = new float[]{5, 2, 1, 2, 1, 2};
                    }
                    else
                      if (lpen.getPenStyle() == EMFConstants.PS_DOT)
                      {
                        dash = new float[]{1, 2};
                      }
                      else
                        if (lpen.getPenStyle() == EMFConstants.PS_SOLID)
                        {
                          dash = new float[]{1};
                        }
                        else
                ...
                BasicStroke bs = new BasicStroke(lpen.getWidth(), BasicStroke.CAP_SQUARE, BasicStroke.JOIN_BEVEL, 1,
                    dash, 0);
                g2.setStroke(bs);
              }
              else
                if (tag instanceof Ellipse)
                {
                  ...
                  // ellipse is the center of the specified bounding rectangle.
                  // The ellipse is outlined by using the current pen and is
                  ...
                  if (tag instanceof LineTo)
                  {
                    ...
                    // The line is drawn by using the current pen and, if the
                    // pen is a geometric pen, the current brush.
                    ...
                    if (tag instanceof MoveToEx)
                    {
                      ...
                      if (tag instanceof Pie)
                      {
                        ...
                        Arc2D arc2d = new Arc2D.Double(arc.getStart().x, arc.getStart().y, arc.getBounds().width, arc
                            .getBounds().height, alpha0, alpha1 - alpha0, Arc2D.PIE);
                        currentShape = arc2d;
                        g2.draw(arc2d);
                      }
                      else
