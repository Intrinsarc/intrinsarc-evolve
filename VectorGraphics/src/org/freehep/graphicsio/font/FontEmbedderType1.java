 ...
 * @version $Id: FontEmbedderType1.java,v 1.1 2009-03-04 22:47:00 andrew Exp $
 */
public class FontEmbedderType1 extends FontEmbedder
{
  ...
  private int asciiEnd, encEnd; // remember the lengths of the three portions
  ...
  protected void openIncludeFont() throws IOException
  {
    ...
    fontFile.println("/FontMatrix [" + 1 / FONT_SIZE + " 0.0 0.0 " + 1 / FONT_SIZE + " 0.0 0.0] readonly def");
  }
  ...
  protected void openGlyphs() throws IOException
  {
    ...
    if (ENCRYPT)
    {
      if (HEX_ENC)
      {
        encrypted = new PrintStream(new EEXECEncryption(new ASCIIHexOutputStream(fontFile), EEXECEncryption.EEXEC_R));
      }
      else
      {
  ...
  protected void writeGlyph(String characterName, Shape glyph, GlyphMetrics glyphMetrics) throws IOException
  {
    ...
    CharstringEncoder charString = (HEX_ENC_CHARSTRINGS ? new CharstringEncoder(new EEXECEncryption(
        new ASCIIHexOutputStream(bytes), EEXECEncryption.CHARSTRING_R)) : new CharstringEncoder(new EEXECEncryption(
        bytes, EEXECEncryption.CHARSTRING_R)));

    charString.startChar(sidebearing, (glyphMetrics != null ? glyphMetrics.getAdvance() : getUndefinedWidth())); // bounds.getWidth());
