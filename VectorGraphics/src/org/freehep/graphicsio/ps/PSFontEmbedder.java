 ...
 * 
 * @author Sami Kama
 * @version $Id: PSFontEmbedder.java,v 1.1 2009-03-04 22:46:57 andrew Exp $
 */
public class PSFontEmbedder extends FontEmbedder
{
    ...
    while (!pIter.isDone())
    {
      switch (pIter.currentSegment(points))
      {
        ...
        case PathIterator.SEG_QUADTO :
          ...
          pc.cubic(controlPoint[0], controlPoint[1], controlPoint[2], controlPoint[3], points[2], points[3]);
          lastPoint[0] = points[2];
          lastPoint[1] = points[3];
          break;

        case PathIterator.SEG_CUBICTO :
          pc.cubic(points[0], points[1], points[2], points[3], points[4], points[5]);
  ...
  protected void openIncludeFont() throws IOException
  {
    ...
    os.println("/FontBBox [" + (int) llx + " " + (int) lly + " " + (int) urx + " " + (int) ury + " ] def");
  ...
  protected void writeBuildProcs() throws IOException
  {
    ...
    os.println("\t\t\t0");
    os.println("\t\t\t" + (int) llx + " " + (int) lly + " " + (int) urx + " " + (int) ury);
