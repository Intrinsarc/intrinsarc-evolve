 ...
 * @version $Id: EPSIEncoder.java,v 1.1 2009-03-04 22:46:57 andrew Exp $
 */
public class EPSIEncoder extends ImageEncoder
{
  ...
  final private static byte[] lowBitMask = {(byte) 0, (byte) 1, (byte) 3, (byte) 7, (byte) 15, (byte) 31, (byte) 63,
      (byte) 127, (byte) 255};
   ...
   *          The image to encode.
   * @param out
   *          The stream to write the GIF to.
   * @param grayscaleBits
   *          Number of grayscale bits to use.
   * @param portrait
   *          Flag indicating a portrait orientation.
   */
  ...
  protected void encodeStart(int width, int height) throws IOException
  {
    ...
    int bytesPerScan = (bitsPerScan / maxBitsPerByte) + ((bitsPerScan % maxBitsPerByte == 0) ? 0 : 1);
    int linesPerScan = (bytesPerScan / maxBytesPerScan) + ((bytesPerScan % maxBytesPerScan == 0) ? 0 : 1);
    ...
    putString("%%BeginPreview " + width + " " + height + " " + grayscaleBits + " " + lines + "\n");
  }
  ...
  protected void encodePixels(int x, int y, int w, int h, int[] rgbPixels, int off, int scansize) throws IOException
  {
    ...
    for (int row = 0; row < h; ++row)
    {
      ...
      {
        grayPixels[y + row][column] = toGrayscale(rgbPixels[row * scansize + off + column]);
      }
    }
  }
  ...
  private byte toGrayscale(int argb)
  {
    ...
    return (byte) (255. * Math.max(0., (1. - (0.3 * red + 0.59 * green + 0.11 * blue))));
  }
