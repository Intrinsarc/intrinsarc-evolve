 ...
 * @version $Id: PSFontTable.java,v 1.1 2009-03-04 22:46:57 andrew Exp $
 */
public class PSFontTable extends FontTable
{
  ...
  protected void firstRequest(Entry e, boolean embed, String embedAs) throws IOException
  {
    ...
    if (embed)
    {
      if (embedAs.equals(FontConstants.EMBED_FONTS_TYPE3))
      {
        ...
        if (embedAs.equals(FontConstants.EMBED_FONTS_TYPE1))
        {
  ...
  private static boolean isStandardFont(Font font)
  {
    ...
    return (fontName.indexOf("helvetica") >= 0) || (fontName.indexOf("avantgarde") >= 0)
        || (fontName.indexOf("bookman") >= 0) || (fontName.indexOf("courier") >= 0)
        || (fontName.indexOf("newcenturysclbk") >= 0) || (fontName.indexOf("palatino") >= 0)
        || (fontName.indexOf("times") >= 0) || (fontName.indexOf("zapfdingbats") >= 0)
        || (fontName.indexOf("zapfchancery") >= 0) || (fontName.indexOf("symbol") >= 0);
  }

  /*
   * public static String toStandardFontName(Font font) { String oblique =
   * "Oblique"; boolean setstyle = true; String fontName = ""; String oFontName =
   * font.getName().toLowerCase(); if ((oFontName.indexOf("sansserif") >= 0) ||
   * (oFontName.indexOf("helvetica") >= 0)) { fontName = "Helvetica"; } else if
   * ((oFontName.indexOf("times") >= 0) || (oFontName.indexOf("serif") >= 0)) {
   * fontName = "Times"; oblique = "Italic"; } else if
   * ((oFontName.indexOf("courier") >= 0)) { fontName = "Courier"; } else if
   * ((oFontName.indexOf("dingbats") >= 0)) { fontName = "ZapfDingbats";
   * setstyle = false; } else if ((oFontName.indexOf("symbol") >= 0)) { fontName =
   * "Symbol"; setstyle = false; } else { fontName = "Helvetica"; }
   * 
