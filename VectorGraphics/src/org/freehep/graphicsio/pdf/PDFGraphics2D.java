 ...
 * @version $Id: PDFGraphics2D.java,v 1.1 2009-03-04 22:46:57 andrew Exp $
 */
public class PDFGraphics2D extends AbstractVectorGraphicsIO implements MultiPageDocument, FontUtilities.ShowString
{
   ...
   * Private/Utility Methods 9.1. drawing, shape creation 9.2. font, strings
   * 9.3. images 9.4. transformations 10. Auxiliary
   * ================================================================================
   */
  ...
  private static final CharTable STANDARD_CHAR_TABLES[] = {Lookup.getInstance().getTable("PDFLatin"),
      Lookup.getInstance().getTable("Symbol"), Lookup.getInstance().getTable("Zapfdingbats")};
  ...
  private PDFImageDelayQueue delayImageQueue; // remember images XObjects to
                                              // include in the file
  private PDFPaintDelayQueue delayPaintQueue; // remember patterns to include in
                                              // the file
  ...
  private void openPage(Dimension size, String title, Component component) throws IOException
  {
    ...
    double scaleFactor = Math.min(getWidth() / size.width, getHeight() / size.width);
  ...
  private void writeHeadline(AffineTransform pageTrafo) throws IOException
  {
    if (headerText != null)
    {
      ...
      writeLine(pageTrafo, headerFont, headerText, -metrics.getLeading() - headerFont.getSize2D() / 2, TEXT_BOTTOM,
  ...
  private void writeLine(AffineTransform trafo, Font font, TagString[] text, double ty, int yAlign, double ly,
      int underline) throws IOException
  {
    writeGraphicsSave();
  ...
  protected void writeImage(RenderedImage image, AffineTransform xform, Color bkg) throws IOException
  {
    ...
    AffineTransform imageTransform = new AffineTransform(image.getWidth(), 0.0, 0.0, -image.getHeight(), 0.0, image
        .getHeight());
