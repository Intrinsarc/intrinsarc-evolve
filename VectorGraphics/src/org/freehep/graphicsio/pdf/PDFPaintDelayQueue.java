 ...
 * @version $Id: PDFPaintDelayQueue.java,v 1.1 2009-03-04 22:46:57 andrew Exp $
 */
public class PDFPaintDelayQueue
{
   ...
   * coordinate system of the parent's content stream (in our case the flipping
   * of the page).
   */
    ...
    while (i.hasNext())
    {
      ...
      if (!e.written)
      {
        ...
        if (e.paint instanceof GradientPaint)
        {
          ...
          if (e.paint instanceof TexturePaint)
          {
            addTexturePaint(e);
          }
      }
  ...
  private void addGradientPaint(Entry e) throws IOException
  {
    ...
    if (gp.isCyclic())
    {
      // FIXME: cyclic function only eveluated between 0 and 1 for short range,
      // not repetitive.
  ...
  /**
   * Writes a type 4 (PostScript calculator) function that describes a triangle
   * function to the pdf file.
   */
  private void addCyclicFunction(String functionRef, double[] c0, double[] c1, double[] dom) throws IOException
  {
    // PDFStream function = pdf.openStream(functionRef, new String[] { "Flate",
    // "ASCII85" });
  ...
  /**
   * Sets both the page (flip) matrix and the actual transformation matrix plus
   * a translation offset (which may of course be be 0).
   */
