 ...
 * @version $Id: PDFFontEmbedderType1.java,v 1.1 2006-07-03 13:32:36 amcveigh
 *          Exp $
 */
public class PDFFontEmbedderType1 extends FontEmbedderType1
{
  ...
  public static PDFFontEmbedderType1 create(FontRenderContext context, PDFWriter pdf, String reference,
      PDFRedundanceTracker tracker)
  {
    return new PDFFontEmbedderType1(context, pdf, reference, new ByteArrayOutputStream(), tracker);
  }

  private PDFFontEmbedderType1(FontRenderContext context, PDFWriter pdf, String reference,
      ByteArrayOutputStream byteOut, PDFRedundanceTracker tracker)
  {
  ...
  // FIXME: The StemV entry is missing in the FontDescriptor dictionary, but it
  ...
  protected void openIncludeFont() throws IOException
  {
    ...
    fontDict.entry("Encoding", redundanceTracker.getReference(getEncodingTable(), PDFCharTableWriter.getInstance()));

    fontDict.entry("Widths", pdf.ref(reference + "Widths"));

    fontDict.entry("BaseFont", pdf.name(getFontName())); // = FontName in font
                                                          // program
  ...
  protected void closeEmbedFont() throws IOException
  {
    ...
    fontFile.entry("Length3", 0); // leave it to the viewer application to add
                                  // the 512 zeros
