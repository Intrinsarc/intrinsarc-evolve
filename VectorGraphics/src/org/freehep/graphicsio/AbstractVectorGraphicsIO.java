 ...
 * @version $Id: AbstractVectorGraphicsIO.java,v 1.1 2006-07-03 13:33:09
 *          amcveigh Exp $
 */
public abstract class AbstractVectorGraphicsIO extends VectorGraphicsIO
{
   ...
   * Auxiliary 10. Private/Utility Methods
   * ================================================================================
   */
   ...
   *          image to be drawn
   * @param dx1,
   *          dy1, dx2, dy2 destination image bounds
   * @param sx1,
   *          sy1, sx2, sy2 source image bounds
   * @param bgColor
   *          background color
   * @param observer
      ...
      Color bgColor, ImageObserver observer)
  {
    try
    {
      ...
      boolean flipHorizontal = (dx2 < dx1) ^ (sx2 < sx1); // src flipped and not
                                                          // dest flipped or
                                                          // vice versa
      boolean flipVertical = (dy2 < dy1) ^ (sy2 < sy1); // <=> source flipped
                                                        // XOR dest flipped
   ...
   * null) { // Draw the image on the background color // maybe we could crop it
   * and fill the transparent pixels in one go // by means of a filter.
   ...
   * <=> source flipped XOR dest flipped int destWidth = Math.abs(dx2-dx1); int
   * destHeight = Math.abs(dy2-dy1); try { return writeImage(background,
   ...
   * 
   * @param x,
   *          y, width, height rectangle to be cleared.
   */
  ...
  /**
   * Set the current transform. Since most output formats do not implement this
   * functionality, the inverse transform of the currentTransform is calculated
   * and multiplied by the transform to be set. The result is then forwarded by
   * a call to writeTransform(Transform).
   * 
   * @param transform
   *          to be set
   */
   ...
   * 
   * @param x,
   *          y, width, height rectangle for clipping
   */
  ...
  protected void writeStroke(Stroke stroke) throws IOException
  {
    if (stroke instanceof BasicStroke)
    {
      ...
      if (currentDashPhase != phase)
        different = true;
      if ((newDashArray == null && currentDashArray != null) || (newDashArray != null && currentDashArray == null))
        different = true;
      if (newDashArray != null && currentDashArray != null && newDashArray.length != currentDashArray.length)
        different = true;
  ...
  static
  {
    ...
    {
      System.err.println("Unable to install fixedPrecision NumberFormat. NumberFormat of incompatible class: "
          + fixedPrecision.getClass());
    }
  }
