 ...
 * @version $Id: SWFOutputStream.java,v 1.1 2009-03-04 22:46:54 andrew Exp $
 */
public class SWFOutputStream extends TaggedOutputStream implements SWFConstants
{
  ...
  public SWFOutputStream(OutputStream os, int version, Dimension size, float frameRate, boolean compress)
      throws IOException
  {
  ...
  public SWFOutputStream(OutputStream os, SWFTagSet tagSet, SWFActionSet actionSet, Dimension size, float frameRate,
  ...
  public void close() throws IOException
  {
    ...
    SWFHeader header = new SWFHeader(getVersion(), len, size, frameRate, frameCount, compress);
    writeHeader(header);
    append();
  ...
  public void writeMatrix(AffineTransform matrix) throws IOException
  {
    ...
    // Translation values.
    int nbits = 0;
    nbits = Math.max(nbits, minBits((int) (matrix.getTranslateX() * TWIPS), true));
    nbits = Math.max(nbits, minBits((int) (matrix.getTranslateY() * TWIPS), true));
    writeUBits(nbits, 5);
