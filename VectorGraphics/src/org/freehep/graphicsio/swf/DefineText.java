 ...
 * @version $Id: DefineText.java,v 1.1 2009-03-04 22:46:52 andrew Exp $
 */
public class DefineText extends DefinitionTag
{
  ...
  public DefineText(int id, Rectangle2D bounds, AffineTransform matrix, Vector /* TextRecord */text)
  {
    this();
    character = id;
    this.bounds = bounds;
  ...
  protected void read(int tagID, SWFInputStream swf, int len, boolean hasAlpha) throws IOException
  {
    ...
    Record record = type1 ? (Record) new RecordType1(swf, hasAlpha) : (Record) new RecordType0(swf, glyphBits,
        advanceBits);
    while (!record.isEndRecord())
    {
      text.add(record);
      type1 = swf.readBitFlag();
      record = type1 ? (Record) new RecordType1(swf, hasAlpha) : (Record) new RecordType0(swf, glyphBits, advanceBits);
    }
  }
  ...
  {
    public abstract void write(SWFOutputStream swf, int glyphBits, int advanceBits, boolean hasAlpha)
        throws IOException;
  ...
  public static class RecordType0 extends Record
  {
    ...
    RecordType0(SWFInputStream swf, int glyphBits, int advanceBits) throws IOException
    {
      ...
      {
        glyphs.add(new GlyphEntry(swf, glyphBits, advanceBits));
      }
    }
    ...
    public int getGlyphBits()
    {
      ...
      for (int i = 0; i < glyphs.size(); i++)
      {
        glyphBits = Math.max(glyphBits, ((GlyphEntry) glyphs.get(i)).getGlyphBits());
      }
      return glyphBits;
    }
    ...
    public int getAdvanceBits()
    {
      ...
      for (int i = 0; i < glyphs.size(); i++)
      {
        advanceBits = Math.max(advanceBits, ((GlyphEntry) glyphs.get(i)).getAdvanceBits());
      }
      return advanceBits;
    }
  ...
  public static class RecordType1 extends Record
  {
    ...
    RecordType1(SWFInputStream input, boolean hasAlpha) throws IOException
    {
