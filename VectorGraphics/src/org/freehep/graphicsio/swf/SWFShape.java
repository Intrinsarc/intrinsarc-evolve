 ...
 * @version $Id: SWFShape.java,v 1.1 2009-03-04 22:46:51 andrew Exp $
 */
public class SWFShape
{
  ...
  public SWFShape(SWFInputStream input, FillStyleArray fillStyles, LineStyleArray lineStyles, boolean isMorphStyle,
      boolean hasAlpha) throws IOException
  {
    ...
    do
    {
      ...
      else
      {
        ...
        {
          records.add(new ShapeRecord(this, input, numFillBits, numLineBits, isMorphStyle, hasAlpha, state));
        }
      }
    }
    while (!endOfShape);
  ...
  {
    public abstract void write(SWFOutputStream swf, int numFillBits, int numLineBits, boolean hasAlpha)
        throws IOException;
  }
  ...
  public static class ShapeRecord extends Record
  {
    ...
    ShapeRecord(SWFShape shape, SWFInputStream input, int numFillBits, int numLineBits, boolean isMorphStyle,
        boolean hasAlpha, int state) throws IOException
    {
      ...
      {
        newFillStyles = new FillStyleArray(input, isMorphStyle, hasAlpha);
        shape.setFillStyles(newFillStyles);
        newLineStyles = new LineStyleArray(input, isMorphStyle, hasAlpha);
    ...
    public void write(SWFOutputStream swf, int numFillBits, int numLineBits, boolean hasAlpha) throws IOException
    {
      ...
      if (move)
      {
        int numMoveBits = Math.max(swf.minBits(moveX, true), swf.minBits(moveY, true));
      ...
      if (newFillStyles != null)
      {
        ...
        newLineStyles.write(swf, hasAlpha);
        swf.writeUBits(numFillBits, 4);
        swf.writeUBits(numLineBits, 4);
      }
    }
    ...
    public int getNumFillBits()
    {
      int numFillBits = 0;
      if (fillStyle0 != -1)
        numFillBits = Math.max(numFillBits, BitOutputStream.minBits(fillStyle0));
      if (fillStyle1 != -1)
        numFillBits = Math.max(numFillBits, BitOutputStream.minBits(fillStyle1));
      return numFillBits;
    }
  ...
  public static class EdgeRecord extends Record
  {
    ...
    public void write(SWFOutputStream swf, int numFillBits, int numLineBits, boolean hasAlpha) throws IOException
    {
      ...
      if (curve)
      {
        numBits = Math.max(numBits, swf.minBits(controlDeltaX, true));
        numBits = Math.max(numBits, swf.minBits(controlDeltaY, true));
        numBits = Math.max(numBits, swf.minBits(anchorDeltaX, true));
        numBits = Math.max(numBits, swf.minBits(anchorDeltaY, true));
      }
      else
      {
        if ((deltaX != 0) && (deltaY != 0))
        {
          numBits = Math.max(numBits, swf.minBits(deltaX, true));
          numBits = Math.max(numBits, swf.minBits(deltaY, true));
        }
        else
        {
          if (deltaX == 0)
    ...
    public String toString()
    {
      if (curve)
      {
        return "rcurveto(" + controlDeltaX + ", " + controlDeltaY + ", " + anchorDeltaX + ", " + anchorDeltaY + ")";
      }
      else
      {
