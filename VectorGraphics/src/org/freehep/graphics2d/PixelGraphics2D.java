 ...
 * @version $Id: PixelGraphics2D.java,v 1.1 2009-03-04 22:46:56 andrew Exp $
 */
public class PixelGraphics2D extends AbstractVectorGraphics
{
      ...
      Color frameColor, double frameWidth, boolean banner, Color bannerColor)
  {
    ...
    y = getYalignment(y, metrics.getAscent(), metrics.getDescent(), vertical);
  ...
  protected GeneralPath createShape(double[] xPoints, double[] yPoints, int nPoints, boolean close)
  {
    ...
    if (nPoints > 0)
    {
      ...
      {
        if ((Math.abs(xPoints[i] - lastX) > resolution) || (Math.abs(yPoints[i] - lastY) > resolution))
        {
