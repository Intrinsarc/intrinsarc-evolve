 ...
 * @version $Id: PrintColor.java,v 1.1 2009-03-04 22:46:56 andrew Exp $
 */
public class PrintColor extends Color
{
  ...
  protected static Color[] defaultColors = {Color.RED, Color.GREEN, Color.BLUE, Color.CYAN, Color.MAGENTA,
      Color.YELLOW, Color.ORANGE, Color.PINK, Color.WHITE, Color.LIGHT_GRAY, Color.GRAY, Color.DARK_GRAY, Color.BLACK};
  ...
  private static void testColorValueRange(float asGray)
  {
    ...
    if (rangeError == true)
    {
      throw new IllegalArgumentException("PrintColor parameter outside of expected range:" + badComponentString);
    }
  }
  ...
  public PrintColor getColor(int mode)
  {
    ...
    switch (mode)
    {
      ...
      default :
        throw new IllegalArgumentException("ColorMode on PrintColor out of range: " + mode);
    }
  }
  ...
  public static PrintColor createPrintColor(Color color)
  {
    ...
    float[] gray = ColorSpace.getInstance(ColorSpace.CS_GRAY).fromRGB(color.getRGBComponents(null));
    if (gray[0] == 0.0f)
    {
      gray[0] = 1.0f;
    }
  ...
  public static Color getDefaultColor(int index)
  {
    if ((index < 0) || (index >= defaultColors.length))
      throw new IllegalArgumentException("PrintColor.getDefaultColor index outside of expected range: " + index);
  ...
  {
    return super.equals(obj) && obj instanceof PrintColor && ((PrintColor) obj).asGray == this.asGray
  ...
  public static PrintColor invert(Color color)
  {
    ...
    return new PrintColor(new Color(printColor.getRGB() ^ 0x00808080), (printColor.getAsGray() + 0.5f) % 1.0f,
