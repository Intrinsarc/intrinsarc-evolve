 ...
 * @version $Id: ScientificFormat.java,v 1.1 2009-03-04 22:46:55 andrew Exp $
 */

public class ScientificFormat extends Format
{
  /**
   * The number of significant digits the number is formatted to is recorded by
   * sigDigit. The maximum width allowed fro the returned String is recorded by
   * MaxWidth
   */
  ...
  /**
   * Sets the significant digits, maximum allowable width and number formatting
   * style (SciNote == true for Pure formatting).
   */
  ...
  /**
   * Implementation of inherited abstract method. Checks to see if object to be
   * formatted is of type Number. If so casts the Number object to double and
   * calls the format method. Returns the result.
   */
  public StringBuffer format(Object obj, StringBuffer toAppendTo, FieldPosition pos)
  {
    if (obj instanceof Number)
    {
      ...
      if (obj instanceof DoubleWithError)
      {
        ...
        {
          toAppendTo.append(dwe.plus);
          int errorSigDigit = resolveErrorSigDigit(dwe.getValue(), dwe.getPlusError());
          toAppendTo.append(formatError(errorSigDigit, dwe.getPlusError()));

          toAppendTo.append(dwe.minus);
          errorSigDigit = resolveErrorSigDigit(dwe.getValue(), dwe.getMinError());
          toAppendTo.append(formatError(errorSigDigit, dwe.getMinError()));
        }
        else
        {
  ...
  /**
   * Returns the formatting style: True means Pure scientific formatting, False
   * means standard.
   */
  ...
  /**
   * Sets the maximum allowable length of the formattted number mantissa before
  ...
  /**
   * Sets the format style used. There are two output styles: Pure and Standard
   * scientific notation. Pure formatted numbers have precisely the number of
   * digits specified by the significant digits (sigDig) parameter and always
   * specify a Base 10 Exponential(E). Standard formated numbers have the number
   * of digits specified by the significant digits (sigDig) parameter but will
   * not have a Base 10 Exponential(E) if the number of digits in the mantissa <=
   * maxWidth.
   */
  ...
  private int resolveErrorSigDigit(double x, double dx)
  {
    ...
    // these circumstances errorsigdit does equal sigdigit, excluding infinity
    // and Nan which are handled by format
  ...
  public String format(double d)
  {
    ...
    if (Double.isInfinite(d))
      return maxWidth < 8 ? "INF" : "Infinite";
    if (Double.isNaN(d))
      return "NaN";
    ...
    IntOfNumberinput = (int) Math.floor(Log10(d));// returns largest int value
    ...
    // outputs num with all sigdigs to right of decimal place or rounded up one
    // extra
