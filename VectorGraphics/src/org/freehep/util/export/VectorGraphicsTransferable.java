...
/**
 * A Transferable for putting vector graphics onto the system clipboard. This is
 * particularly useful on Windows where EMF format can be used to copy vector
 * graphics onto the system clipboard, from where it can be pasted into many
 * applications, including PowerPoint and Word. VectorGraphicsTranferable will
 * fall back to using a bitmap image format if no suitable VectorGraphics format
 * is registered.
 * <p>
 * Note: This only works with JDK 1.4 or later.
 * 
 * @author tonyj
 * @version $Id: VectorGraphicsTransferable.java,v 1.1 2006-07-03 13:33:31
 *          amcveigh Exp $
 */
public class VectorGraphicsTransferable implements ClipboardOwner, Transferable
{
  ...
  /**
   * Create a VectorGraphicsTransferable with the built-in flavor/ExportFileType
   * associations
   * 
   * @param c
   *          The Component whose graphics will be transfered
   */
   ...
   * @param c
   *          The Component whose graphics will be transfered
   * @param addDefaultAssociations
   *          Add the standard flavor/ExportFileType associations
   */
  ...
  public boolean isDataFlavorSupported(DataFlavor dataFlavor)
  {
    if (dataFlavor.match(imageFlavor))
      return true;
    if (types.containsKey(dataFlavor))
      return true;
    return false;
  }
  ...
  private static Map createDefaultTypes()
  {
    ...
    addType(result, "ENHMETAFILE", "image/emf", "Enhanced Meta File", "org.freehep.graphicsio.emf.EMFExportFileType");
  ...
  private static void addType(Map result, String atom, String mimeType, String description, String className)
  {
    ...
    {
      System.err.println("Unable to install flavor for mime type '" + mimeType
          + "' (this is expected if not using JDK 1.4)");
    }
  }
}