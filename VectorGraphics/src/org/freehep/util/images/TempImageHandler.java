...
public class TempImageHandler
{
  ...
  static
  {
    ...
    byte[] brokenImageBytes = getImageBytes("BrokenImage.gif", TempImageHandler.class);
    ...
    byte[] brokenCursorBytes = getImageBytes("BrokenCursor.gif", TempImageHandler.class);
  ...
  public static Image getImage(String name, Class clazz)
  {
    ...
    if (image == null)
    {
      ...
      {
        byte[] imageBytes = getImageBytes(name + imageExtensions[i], clazz);
        if (imageBytes != null)
        {
  ...
  public static Cursor getCursor(String name, Class clazz, int x, int y)
  {
    ...
    if (cursor == null)
    {
      ...
      if (cursorWidth < iconWidth || cursorHeight < iconHeight)
      {
        ...
        image = image.getScaledInstance(cursorWidth, cursorHeight, Image.SCALE_DEFAULT);

      }
      else
        if (iconWidth != cursorWidth || iconHeight != cursorHeight)
        {

          // Both dimensions are larger. Make a larger image and draw
          // the image into the upper, left-hand corner.
          BufferedImage bimage = new BufferedImage(cursorWidth, cursorHeight, BufferedImage.TYPE_INT_ARGB);
      ...
      if (image == brokenImage)
      {
        ...
        if (image != null)
        {
          cursor = toolkit.createCustomCursor(image, new Point(x, y), "" + name);
          if (cursor != null)
