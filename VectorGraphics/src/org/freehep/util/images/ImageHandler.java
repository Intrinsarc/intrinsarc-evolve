...
/**
 * A convenience class for handling images. Maintains a cache of images suitable
 * for small icons which may be reused often.
 * 
 * @author Tony Johnson
 * @author Cal Loomis
 * @version $Id: ImageHandler.java,v 1.1 2009-03-04 22:46:56 andrew Exp $
 */
...
public class ImageHandler
{
  ...
  /**
   * Load an image from a URL. Images are cached for efficiency. If the image
   * cannot be found will return a brokenImage. No exceptions are thrown.
   * 
   * @param url
  ...
  public static Image getImage(URL url)
  {
    ...
    if (image == null && url != null)
    {
      ...
      // Note we SHOULD include brokenImage in the cache, otherwise repeated
      // requests for
  ...
  /**
   * Create the best cursor by reading from a URL, with hostpot (0,0). The best
   * width and height are inserted into the URL in the spots with %width and
   * %height. Example:
   * 
   ...
   *          URL of the cursor
   * @param clazz
   *          Class to be used for getting resource
   * @param width
   *          suggested width
   * @param height
   *          suggested height
  ...
  public static Cursor getCursor(URL url, int x, int y)
  {
    ...
    if (cursor == null)
    {
      ...
      if (image == brokenImage)
        ...
        cursor = toolkit.createCustomCursor(image, new Point(x, y), url.getFile());
      if (url != null)
  ...
  public static Icon getIcon(URL url)
  {
    ...
    if (icon == null)
    {
      ...
      if (image == brokenImage)
        ...
        icon = new ImageIcon(image, url.getFile());
      if (url != null)
  ...
  /**
   * Convenience routine, equivalent to getImage(clazz.getResource(name)); As an
   * additional convenience, if the name does not contain an extension (eg .gif)
   * then a list of common extensions will be tried.
   * 
   * @param name
   *          The relative address of the image to load
   * @param clazz
   *          The class from which the address is based
   * @return The image, or brokenImage if no image is found.
   */
  public static Image getImage(String name, Class clazz)
  {
    if (name.indexOf('.') >= 0)
      return getImage(clazz.getResource(name));
  ...
  /**
   * Convenience routine, equivalent to getCursor(clazz.getResource(name)); As
   * an additional convenience, if the name does not contain an extension (eg
   * .gif) then a list of common extensions will be tried.
   * 
   * @param name
   *          The relative address of the cursor to load
   * @param clazz
   *          The class from which the address is based
  ...
  {
    if (name.indexOf('.') >= 0)
      return getCursor(clazz.getResource(name));

    for (int i = 0; i < imageExtensions.length; i++)
    {
      URL url = clazz.getResource(name + imageExtensions[i]);
      if (url != null)
        return getCursor(url);
    }
    return brokenCursor;
  }

  /**
   * Convenience routine, equivalent to getIcon(clazz.getResource(name)); As an
   * additional convenience, if the name does not contain an extension (eg .gif)
   * then a list of common extensions will be tried.
   * 
   * @param name
   *          The relative address of the icon to load
   * @param clazz
   *          The class from which the address is based
  ...
  {
    if (name.indexOf('.') >= 0)
      return getIcon(clazz.getResource(name));
