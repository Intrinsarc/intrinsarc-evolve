...
/**
 * A CommandTargetManager manages a set of CommandSources and a set of
 * CommandGroups, and figure out the wiring from the CommandSources to the
 * CommandTargets within the CommandGroups. The CommandGroups can be dynamically
 * added and removed from the CommandTargetManager, as sets of commands become
 * available or not.
 * 
...
public class CommandTargetManager
{
  ...
  /**
   * Override this method to provide custom handling of errors generated during
   * command processing.
   */
    ...
    if (sources.remove(source) && started)
    {
      ...
      if (t != null)
      {
        ...
        if (source instanceof Observer)
          t.getGroup().deleteObserver((Observer) source);
      }
    }
  }
  ...
  /**
   * This method links the CommandSource to any CommandGroup it hits that
   * contains a CommandTarget that accepts the CommandSource.
   */
