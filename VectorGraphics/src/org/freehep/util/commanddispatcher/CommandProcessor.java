 ...
 * 
 * Typically applications provide their own CommandProcessor(s) which extend
 * this base class and which can handle a set of commands. By default
 * CommandProcessor's acceptCommand method uses reflection to search for methods
 * in the subClass which correspond to specific commands, although subclasses
 * could also override the acceptCommand method to implement a different scheme.
 * The default scheme looks for methods of type:
 * 
 * <pre>
 *            public void onXXX()
 * </pre>
 ...
 * @version $Id: CommandProcessor.java,v 1.1 2009-03-04 22:46:58 andrew Exp $
 */
...
public class CommandProcessor extends Observable implements CommandGroup
{
  ...
  /**
   * The CommandTargetManager calls acceptCommand to find out if this
   * CommandProcessor can respond to the specified command. If it can it returns
   * a CommandTarget for the command, otherwise it returns null.
   * 
   * @param command
  ...
  /**
   * Uses reflection to check if the specified class has an "on" or "enable"
   * method for this comamnd.
   * 
   * @param klass
   *          The class to check
   * @param command
  ...
  /**
   * Override this method to provide application specific handling for errors
   * generated during command dispatch
   */
  ...
  /**
   * Boolean target is an implementation of CommandTarget for command targets
   * which can be swithced on/off. (Typically corresponding to JCheckBoxMenuItem
