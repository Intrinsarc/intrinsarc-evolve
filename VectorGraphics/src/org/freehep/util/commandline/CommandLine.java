 ...
 * @version $Id: CommandLine.java,v 1.1 2009-03-04 22:47:00 andrew Exp $
 */
public class CommandLine
{
   ...
   *          minumum number of parameters
   * @param multiLevel
   *          forces the command line to be multi-level (i.e. does not check for
   ...
   * @param shortName
   *          the short name of the option (may be set to null)
   * @param comment
   *          description of the arguments to the option
   */
   ...
   *          the short name of the option (may be set to null)
   * @param qualifiers
   *          gives a list of qualifiers you can use
   * @param comment
   *          description of the arguments to the option
   */
  ...
  /**
   * Adds an option to be recognized by parse, without doing parameter or option
   * checking. This is normally used for options such as -? -help and -version
   * 
   * @param longName
   *          the full name of the option
   * @param shortName
  ...
  /**
   * Adds an option to be recognized by parse. This option can be specified
   * multiple times on the commandline, and is immediately followed by its
   * value. Usage (-Iincludedir)
   * 
   * @param name
   *          the full name of the option (preferably one character)
   * @param valueDescription
   *          what the value after the option means
   * @param comment
   *          description of the argument to the option
   */
  ...
  /**
   * @return an enumeration of mandatory and optional parameters
   ...
   * 
   * @param args
   *          arguments for the command, including all options
   * @return true if the arguments were parsed correctly. False if a bailOut
  ...
  public boolean parse(String[] args) throws CommandLineException
  {
    ...
    if (!bailOut)
    {
      ...
      {
        exception = new MissingArgumentException("Too few arguments: " + arguments.size() + ", while "
            + numberOfParameters + " " + ((parameters.size() == 1) ? "was" : "were") + " expected.");
        throw exception;
      }
      ...
      if ((check) && (!multiLevel))
      {
            ...
            || !((Parameter) parameters.elementAt(parameters.size() - 1)).getName().endsWith("..."))
        {
          ...
          {
            exception = new TooManyArgumentsException("Too many arguments: " + arguments.size() + ", while "
                + parameters.size() + " " + ((parameters.size() == 1) ? "was" : "were") + " expected.");
            throw exception;
          }
        }
      }
  ...
  private boolean checkflag(String flag, String value, boolean qualified) throws CommandLineException
  {
    ...
    for (Enumeration e = options.elements(); e.hasMoreElements();)
    {
      ...
      if (opt.isMulti() && flag.startsWith(opt.getName()))
      {
        ...
        {
          exception = new MissingArgumentException("Missing argument <" + opt.getValue() + "> for option '"
              + opt.getName() + "'.");
          throw exception;
        }
      }
  ...
  private void setFlag(Option option, String value)
  {
    ...
    if (option.getShortName() != null)
      flags.put(option.getShortName(), value);
  }
   ...
   * @param name
   *          name of the option
   * @return the value of a specified option. null is returned in case the
   *         option was not specified. true if this option was a flag.
   */
   ...
   * @param name
   *          name of the option
   * @return the value of a specified option. An empty vector is returned in
   *         case the option was not specified.
   */
   ...
   *          name of the option
   * @param defaultValue
   *          default value if option was not set
   * @return the value of a specified option. null is returned in case the
   *         option was not specified. true if this option was a flag.
   */
  ...
  private String pad(String str, int length)
  {
    ...
    if (l == length)
      ...
      if (l < length)
        return str + padding.substring(0, length - l);
      else
