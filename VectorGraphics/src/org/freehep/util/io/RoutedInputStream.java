 ...
 * @version $Id: RoutedInputStream.java,v 1.1 2009-03-04 22:46:49 andrew Exp $
 */
public class RoutedInputStream extends InputStream
{
  ...
  private static final int ROUTEFOUND = 1; // found a route, but need to still
                                            // return buffer
  private static final int ROUTEINFORM = 2; // buffer returned, need to inform
                                            // routelistener
  private static final int ROUTED = 3; // the main stream is now routed to the
                                        // route
  private static final int CLOSING = 4; // the underlying stream is closed, but
                                        // buffer need to be emptied
   ...
   * the associated route listener is called, which should take over reading the
   * stream. In this case this method will, after the route is finished reading
   * and closed, return the first byte after the end marker. This of course
   * unless that byte is part of the next start marker.
   */
    ...
    NEWSTATE : while (true)
    {
      switch (state)
      {
        ...
        case UNROUTED :
          ...
          // the buffer is one byte longer than the longest start marker, so
          // even
          // if we find that marker we can still return the byte just in front
          // of it.
          result = buffer[sob];
        ...
        case ROUTEFOUND :
          // found a start marker, we still need to return all bytes before
        ...
        case ROUTEINFORM :
          ...
          // next call will generate callbacks to this read method in state
          // ROUTED.
          ...
          // FIXME: we need an UNROUTING here which just returns the buffer, but
          // does not refill it, in case the reads would block...
          // we return a byte from the buffer and
          // let the next call take care of rebuffering, otherwise we may block
          result = buffer[sob];
          sob = (sob + 1) % buffer.length;
          return result;
        ...
        case ROUTED :
          ...
          // return the start marker (in buffer) followed by newly read bytes.
          if (sob == eob)
          {
            result = in.read();
            if (result < 0)
  ...
  {
    addRoute(start.getBytes(), (end == null) ? null : end.getBytes(), listener);
  }
  ...
  /**
   * Checks if cmp is equal to buf (with start at index) for the length of cmp.
   */
   ...
   * 
   * If you just discard the Route, the underlying stream will still return you
   * all or part of the bytes of this route.
   * 
