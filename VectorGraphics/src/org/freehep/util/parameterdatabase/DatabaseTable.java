...
import org.freehep.util.parameterdatabase.editor.*;
import org.freehep.util.parameterdatabase.selector.*;
import org.freehep.util.parameterdatabase.types.*;
...
public class DatabaseTable implements TableModel, DatabaseListener
{
   ...
   * used to generate a ClassIterator which will be used to search the database.
   */
  private Object key = null;

  /**
   * The flag indicating whether or not the local column is shown. The two forms
   * of the table are obtained with a trick--we just add one to the column index
   * if there is no local column.
   */
   ...
   * true, then this table will show a "local variable" column. If it is false,
   * then the "local variable" column is omitted.
   */
