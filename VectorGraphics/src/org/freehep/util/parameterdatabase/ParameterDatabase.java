...
public class ParameterDatabase
{
  ...
  /**
   * Constructor creates the hashtable of the given initial size to hold all of
   * the parameters.
   */
  ...
  public boolean isParameterLocal(String parameterName, Iterator iterator)
  {
    ...
    if (iterator.hasNext())
    {
      ...
      ParameterKey key = ParameterKey.createParameterKey(object, parameterName);
   ...
   * sequence of Objects which correspond to the specific tables in which to try
   * to find the parameter. The iteration will stop when the parameter is first
   * found.
   */
  ...
  /**
   * Return a boolean indicating whether this parameter is defined for the given
   * object.
   */
   ...
   * first object returned by the given iterator. After the parameter is added,
   * the listener will be added to the value. This same listener will be removed
   * from the next visible parameter of the same name. If the parameter already
   * exists, the listener is added but the value is not modified.
   */
   ...
   * the listener will be added to the value. This same listener will be removed
   * from the next visible parameter of the same name. If the parameter already
   * exists, the listener is added but the value is not modified unless
   * overwrite is true.
   */
  public boolean addParameter(String parameterName, Object value, Iterator iterator, PropertyChangeListener listener,
      boolean overwrite)
   ...
   * is removed, the listeners from this entry will be added to the next visible
   * value.
   */
  public boolean removeParameter(String parameterName, Iterator iterator)
  {
    ...
    if (iterator.hasNext())
    {
      ...
      ParameterValue parameterValue = (ParameterValue) database.remove(key);
  ...
  /**
   * Write out the entire database as an XML file. Returns true if there were no
   * errors generated; returns false otherwise. Note that this simply appends a
   * parameters element to the given XMLWriter. The instanceMap must map
   * instances of non-Class objects to a unique Integer. The integer value is
   * used to tag "local" copies of parameters. If null is passed in as the
  ...
  public void firePropertyChange(String parameterName, ParameterValue parameterValue)
  {
    ...
    PropertyChangeEvent pcEvent = new PropertyChangeEvent(this, parameterName, null, parameterValue.getValue());
   ...
   * this iterator. Note that all parameters in the database are checked to see
   * if they match the given objects.
   */
   ...
   * constructor is called. (Note: this is a general assumption of the database
   * as a whole.)
   */
  protected Object cloneObject(Object originalObject) throws NoSuchMethodException, InstantiationException,
