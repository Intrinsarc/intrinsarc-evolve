 ...
 * @see StatusBar
 * @author tonyj
 * @version $Id: ProgressMeter.java,v 1.1 2009-03-04 22:46:56 andrew Exp $
 */
...
public class ProgressMeter extends JPanel
{
  ...
  public ProgressMeter(boolean showStopButton)
  {
    ...
    m_stopButton = new JLabel(icon); // Note: We use a label since it takes
                                      // much less space than a button
   ...
   * 
   * @param model
   *          The model, or null to clear the progress bar
   */
   ...
   * 
   * @param stop
   *          The stoppable, or null to clear
   */
  ...
  {
    m_realModel.setRangeProperties(model.getValue(), model.getExtent(), model.getMinimum(), model.getMaximum(), model
        .getValueIsAdjusting());
  }
