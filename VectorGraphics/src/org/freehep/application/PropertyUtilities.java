 ...
 * @author tonyj
 * @version $Id: PropertyUtilities.java,v 1.1 2009-03-04 22:46:56 andrew Exp $
 */
public abstract class PropertyUtilities
{
  ...
  public static Color getColor(Properties prop, final String key, final java.awt.Color def)
  {
    try
    {
      return new Color(getInteger(prop, key.concat("-r")), getInteger(prop, key.concat("-g")), getInteger(prop, key
          .concat("-b")));
    }
    catch (Exception e)
    {
   ...
   *              if the property retrieved cannot be converted to
   *              <code>int</code>
   * @param prop
   *          The Properties set
   * @param key
  ...
  /**
   * Load a URL from a properties file. If the URL begins with / it is taken to
   * be a system resource (i.e. on the classpath).
   */
  ...
  /**
   * Translates a string by substituting tokens of the form {name} to the value
   * of property name in the properties set.
   * 
   * @param prop
   *          The properties set
   * @param in
