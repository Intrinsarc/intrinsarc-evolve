 ...
 * 
 * @author tonyj
 * @version $Id: ImageViewer.java,v 1.1 2009-03-04 22:47:00 andrew Exp $
 */
public class ImageViewer extends Application implements Printable
{
  ...
  {
    new ImageViewer().createFrame(argv).show();
  }
  ...
  public class ImageViewerCommandProcessor extends ApplicationCommandProcessor
  {
    ...
    public void onOpen()
    {
      ...
      try
      {
        ...
        // Currently there is no approved way to create an image from a
        // InputSource.
        // See Java bug id 4236738 and the proposed Image IO Framwork (JSR 15).
        // For now we cheat and read the file into a byte array, and then decode
        // that :-(
  ...
  private class StoppableImageProducer implements ImageProducer, ImageConsumer, Stoppable
  {
    ...
    public void setDimensions(int width, int height)
    {
      ...
      while (i.hasNext())
        ((ImageConsumer) i.next()).setDimensions(width, height);
    }
    public void setProperties(Hashtable props)
    {
      Iterator i = consumers.iterator();
      while (i.hasNext())
        ((ImageConsumer) i.next()).setProperties(props);
    ...
    public void setColorModel(ColorModel model)
    {
      Iterator i = consumers.iterator();
      while (i.hasNext())
        ((ImageConsumer) i.next()).setColorModel(model);
    }
    public void setHints(int hintflags)
    {
      Iterator i = consumers.iterator();
      while (i.hasNext())
        ((ImageConsumer) i.next()).setHints(hintflags);
    }
    public void setPixels(int x, int y, int w, int h, ColorModel model, byte[] pixels, int off, int scansize)
    {
      Iterator i = consumers.iterator();
      while (i.hasNext())
        ((ImageConsumer) i.next()).setPixels(x, y, w, h, model, pixels, off, scansize);
    }
    ...
    public void imageComplete(int status)
    {
      Iterator i = consumers.iterator();
      while (i.hasNext())
        ((ImageConsumer) i.next()).imageComplete(status);
    }
    ...
    public void stop()
    {
      ...
      while (i.hasNext())
        ((ImageConsumer) i.next()).imageComplete(IMAGEABORTED);
    }
  }
}
