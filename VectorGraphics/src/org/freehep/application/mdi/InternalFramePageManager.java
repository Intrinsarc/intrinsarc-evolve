/*
 * InternalFramePageManager.java
 *
 * Created on March 21, 2001, 5:49 PM
 */
 ...
 * @version $Id: InternalFramePageManager.java,v 1.1 2006-07-03 13:32:48
 *          amcveigh Exp $
 */
public class InternalFramePageManager extends PageManager
{
  ...
  {
    top.setSelectedFrame(frameForPage(page));
  }
  ...
  void arrange(int index)
  {
    ...
    else
    {
      ...
      // Initially, the number left to put on the screen is the total number of
      // frames.
      ...
      /*
       * In the case of horizontal tile, n is number of columns; n is the number
       * of rows in the case of a vertical tile.
       */
      if (n < 1)
        return;
      ...
      /*
       * The above translations are used to go between rows in a vertical tile
       * and between rows in a horizontal tile.
       */
      ...
      /*
       * The above translations are used to go along rows in a vertical tile and
       * along rows in a horizontal tile.
       */
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        // translated to the starting point for this row (vert case) or column
        // (horiz case)
        nThis = nLeft / (n - i);
        // n - i is the number or rows (vert case) or columns (horiz case)
        // remaining
        nLeft -= nThis;
        if (isHorizontal)
          yTranslation_2 = size.height = paneHeight / nThis;
        // nThis is the number of rows in this column
        // yTranslation_2 is the amount of y translation between frames in this
        // column
        // Leave xTranslation_2 as zero because there is no x transition while
        // going along a column
        else
          xTranslation_2 = size.width = paneWidth / nThis;
        // nThis is the number of columns in this row
        // xTranslation_2 is the amount of x translation between frames in this
        // row
        // Leave yTranslation_2 as zero because there is no y transition while
        // traversing a row
        int j = 0;
        while (true)
        {
          do
            frame = frames[k++];
          while (frame.isIcon() || frame.isMaximum()); // Don't include
                                                        // iconified frames.
          frame.setSize(new Dimension(size));
          frame.setLocation(new Point(point));
          if (++j < nThis)
            point.translate(xTranslation_2, yTranslation_2);
          // set up the point for the next frame in this column (horiz case) or
          // row (vert case)
          else
            break;
        }
  ...
  public class InternalFrameCommandProcessor extends PageManagerCommandProcessor
  {
