 ...
 * integers. It displays as a text area with two small up and down buttons
 * beside it. Pressing the up or down button increments or decrements the
 * counter. Pressing and holding either button causes the value to change
 * continuously (unless delay is set to 0).
 */
public class JSpinBox extends JComponent
{
   ...
   * 
   * @param init
   *          The initial value for the spin box
   * @param min
   ...
   * 
   * @param The
   *          delay between increments in milliseconds, or 0 to disable repeat
   */
  ...
  private class JSpinDocument extends PlainDocument implements Runnable
  {
    ...
    public void remove(int pos, int len) throws BadLocationException
    {
      ...
      // after the remove, in case an insert is to follow immediately. Instead
      // we
