 ...
 * sheets.
 * 
 * @version $Id: CellOrientatedTable.java,v 1.1 2009-03-04 22:46:59 andrew Exp $
...
public class CellOrientatedTable extends JTable
{
   ...
   * The class hierarchy is checked first, is no more specific editor or
   * renderer is found than one for Object, then the class' implemented
   * interfaces are tried.
   */
  ...
  public TableCellRenderer getCellRenderer(int row, int column)
  {
    ...
    if (tryInterfaces && renderer == getDefaultRenderer(Object.class))
    {
      ...
      for (int i = 0; i < interfaces.length; i++)
      {
        TableCellRenderer newRenderer = getDefaultRenderer(interfaces[i]);
        if (newRenderer != null)
        {
