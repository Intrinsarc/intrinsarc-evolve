 ...
 * intended to provide a submenu-type functionality to toolbars. The component
 * has a single selected element. If the visible button is clicked, the action
 * associated with this button is run. However, if the button is pressed and
 * held, a popup toolbar with all options is shown. The user then may select the
 * desired option which will then become the selected button. The action
 * associated with this is also run.
 * 
...
public class JSubToolBar extends JLayeredPane
{
  ...
  /**
   * Add an action to this component. Currently, only actions are supported and
   * thus this should be the only add method used. A button is made from the
   * action which is then added to the sub-toolbar.
   */
   ...
   * activated. This must be sufficiently long that clicks on the component are
   * recognized as such.
   */
  ...
  /**
   * Update the preferred, minimum, and maximum sizes of this component. This is
   * the smallest rectangle which will contain all of the child components.
   */
  ...
  /**
   * This panel is used to intercept mouse events from the visible button in the
   * sub-toolbar.
   */
  private class MousePanel extends JPanel implements MouseListener, ActionListener
  {
    ...
    {
      timer.stop();
      if (!popup.isVisible() && selectedComponent != null && selectedComponent instanceof AbstractButton)
      {
