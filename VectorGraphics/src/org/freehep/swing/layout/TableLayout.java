...
/**
 * This layoutmanager uses all of the GridBagLayout's managers functionality and
 * power but allows you to set the options easily. It will construct the
 * GridBagConstraints object for you.
 * 
 * To use it, set the layout manager for your Container to TableLayout and add
 * each component with a name string for which the format is specified below:
 * 
 * "gridx gridy gridwidth gridheight ( ipadx ipady ) [ top left bottom right ] {
 * weightx weighty} options"
 * 
 * you may use spaces or commas as separators.
 * 
 * gridx and gridy are mandatory, can be * for RELATIVE gridwidth and gridheight
 * are optional, can be * for REMAINDER ipadx and ipady are optional insets
 * (top...right) are optional weights are optional and override any settings
 * made to them by the options options are optional
 * 
 * Options may contain a set of characters which will set the corresponding flag
 * to true. If not set, the flag is false.
 * 
 * "r" right align "l" left align "t" top align "b" bottom align if none of
 * these are set the component is placed in the center.
 * 
 * "w" resize both cell and component in width "W" resize only cell in width "h"
 * resize both cell and component in height "H" resize only cell in height if
 * none of these are set neither the component nor the cell is not resized. and
 * the extra space is put on the outside of the table.
 * 
 * The weight is calculated as follows, if not set explicitly by the {weight}
 * section: 1 if character set, fill is set if lowercase.
 * 
 * You may construct a TableLayout with some default name string.
 * 
 * @author Mark Donszelmann
 * @version $Id: TableLayout.java,v 1.1 2009-03-04 22:47:00 andrew Exp $
 */
public class TableLayout extends GridBagLayout
{
  ...
  {
    System.err
        .println("Usage: gridx gridy gridwidth gridheight ( ipadx ipady ) [ top left bottom right ] { weightx weighty} options");
    System.err.println();
  ...
  public static GridBagConstraints getGridBagConstraints(String name, GridBagConstraints def)
  {
    ...
    Pattern pattern = Pattern.compile(coordPattern + ipadPattern + insetsPattern + weightPattern + optionPattern
        + restPattern);
    ...
    try
    {
      ...
      // optional
      int gridwidth = matcher.group(3) != null ? getSpan(matcher.group(3)) : def.gridwidth;
      int gridheight = matcher.group(4) != null ? getSpan(matcher.group(4)) : def.gridheight;
      ...
      {
        insets = new Insets(Integer.parseInt(matcher.group(7)), Integer.parseInt(matcher.group(8)), Integer
            .parseInt(matcher.group(9)), Integer.parseInt(matcher.group(10)));
      }
      else
      {
        insets = def.insets;
      ...
      return new TableConstraints(gridx, gridy, gridwidth, gridheight, weightx, weighty, anchor, fill, insets, ipadx,
          ipady);
    }
  ...
  public static class TableConstraints extends GridBagConstraints
  {
    ...
    public TableConstraints(int gridx, int gridy, int gridwidth, int gridheight, double weightx, double weighty,
