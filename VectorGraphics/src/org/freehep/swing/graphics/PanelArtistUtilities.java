 ...
 * @version $Id: PanelArtistUtilities.java,v 1.1 2006-07-03 13:33:26 amcveigh
 *          Exp $
 */
public class PanelArtistUtilities
{
  ...
  public static AffineTransform getCCWRotateTransform(int width, int height)
  {
    ...
    return new AffineTransform(0., -((double) height) / width, ((double) width) / height, 0., 0., (double) height);
  }
  ...
  public static AffineTransform getCWRotateTransform(int width, int height)
  {
    ...
    return new AffineTransform(0., ((double) height) / width, -((double) width) / height, 0., (double) width, 0.);
  }
   ...
   * 
   * NOTE: THIS ONLY CORRECTLY HANDLES THE CASE WHEN THE USER SPACE RECTANGLE IS
   * CENTERED ON THE ORIGIN.
   * 
   * @param transform
   *          the transform which will be modified
   * @param realBounds
   *          the user space rectangle
   * @param windowBounds
   *          the window to map the user rectangle to
   */
      ...
      RectangularShape windowBounds)
  {
    ...
    else
    {
      ...
      if (realWidth > 0 && realHeight > 0)
      {
        ...
        transform.setTransform(scaleWidth, 0., 0., -scaleHeight, -scaleWidth * realBounds.getX(), scaleHeight
            * realBounds.getY());
      }
      else
      {
   ...
   * 
   * @param bounds
   *          rectangle to modify with the given component's bounds (will create
   *          new Rectangle if this is null)
   * @param c
   *          component to get the bounds from
   * 
   * @return convenience reference to the rectangle passed in (or the created
   *         rectangle)
   */
