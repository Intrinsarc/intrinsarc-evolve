...
/**
 * This class contains static methods which are useful for the ScaleBorder class
 * and any potential subclasses.
 * 
 * @author Charles Loomis
 * @version $Id: Scale.java,v 1.1 2009-03-04 22:46:55 andrew Exp $
 */
public class Scale
{
  ...
  /**
   * The size in pixels of the primary tick marks. This is a global property for
   * all scales.
   */
  protected static float primaryTickSize = 8.f;

  /**
   * The size in pixels of the secondary tick marks. This is a global property
   * for all scales.
   */
   ...
   */
  static public void setTickSizes(float primaryTickSize, float secondaryTickSize)
  {
  ...
  /**
   * A utility method to add a tick mark to the given GeneralPath at the given
   * position.
   */
  static private void addTickMark(GeneralPath gp, int location, float tickPosition, float tickLength)
  {
  ...
  static public void drawLinearScale(double value0, double value1, int scaleSize, int minPrimary, int minSeparation,
      int location, GeneralPath primaryTicks, GeneralPath secondaryTicks, String[] label, double[] position)
  {

    // First calculate the range of the scale.
    double range = Math.abs(value1 - value0);
    int sign = (value1 - value0 > 0) ? 1 : -1;
    ...
    if (optimal > 0)
    {
      ...
      for (int iv = Math.min(limit0, limit1); iv <= Math.max(limit0, limit1); iv++)
      {

        double val = iv * size;
        float tickPosition = interpolate(val, w, value0, value1);

        addTickMark(secondaryTicks, location, tickPosition, secondaryTickSize);
      }
      ...
      for (int iv = Math.min(limit0, limit1); iv <= Math.max(limit0, limit1); iv++)
      {
        ...
        addTickMark(primaryTicks, location, tickPosition, primaryTickSize);
      }
  ...
  static private float interpolate(double value, double size, double value0, double value1)
  {
