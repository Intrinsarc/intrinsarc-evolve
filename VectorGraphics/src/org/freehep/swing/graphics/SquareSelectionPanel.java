...
/**
 * A panel which selects a "square" region on the screen. The definition of
 * "square" is that this region will retain the relative scaling of the x and
 * y-axes.
 * 
 * @author Charles Loomis
 * @author Mark Donszelmann
 * @version $Id: SquareSelectionPanel.java,v 1.1 2006-07-03 13:33:26 amcveigh
 *          Exp $
 */
public class SquareSelectionPanel extends AbstractRegionSelectionPanel
{
  ...
  /**
   * Return the number of control points,5---the four corners and one point in
  ...
  /**
   * Returns the NE, SE, NW and SW cursors for the four different directions and
   * SquareCursor as the default
   */
  public Cursor getControlPointCursor(int index)
  {
    switch (index)
    {
      ...
      default :
        return compassCursor("Resize", xCtrlPts[index] - xCtrlPts[4], yCtrlPts[index] - yCtrlPts[4], 4, true);
    }
  }
  ...
  public void updateActiveControlPoint(int x, int y)
  {
    ...
    else
    {
      ...
      int xside = (int) ((((double) dx) + ratio * ((double) dy)) / (1. + ratio * ratio));
  ...
  public void paintComponent(Graphics g)
  {
    ...
    if (visible)
    {
      ...
      // Get the half-length of the side.
      int side = Math.abs(xCtrlPts[0] - xCtrlPts[4]);
      ...
      if (visibleGuides)
      {
        g.drawLine(xCtrlPts[0], yCtrlPts[0], xCtrlPts[2], yCtrlPts[2]);
        g.drawLine(xCtrlPts[1], yCtrlPts[1], xCtrlPts[3], yCtrlPts[3]);
      }
  ...
  /**
   * Determine if the selection is valid; regions of zero area are considered
   * invalid.
   * 
