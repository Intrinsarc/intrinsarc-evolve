...
/**
 * A panel which selects a rectangular region on the screen which can be
 * arbitrarily rotated.
 * 
 * @author Charles Loomis
 * @author Mark Donszelmann
 * @version $Id: RotatedRectangleSelectionPanel.java,v 1.1 2006-07-03 13:33:26
 *          amcveigh Exp $
 */
public class RotatedRectangleSelectionPanel extends AbstractRegionSelectionPanel
{
  ...
  /**
   * The number of control points is 6---the four corners and the centerpoints
   * of the first and last sides.
   * 
  ...
  public Cursor getControlPointCursor(int index)
  {
    ...
    return compassCursor(type, xCtrlPts[index] - xCtrlPts[k], yCtrlPts[index] - yCtrlPts[k], 8, true);
  }
  ...
  public AffineTransform makeAffineTransform()
  {
    ...
    double radius = getRadius(xCtrlPts[first], yCtrlPts[first], centerIndex);
    ...
    sdx = ((xCtrlPts[first] - xCtrlPts[centerIndex]) > 0) ? 1. : -1.;
    sdy = ((yCtrlPts[first] - yCtrlPts[centerIndex]) > 0) ? 1. : -1.;
    ...
    sdx = ((xCtrlPts[second] - xCtrlPts[centerIndex]) > 0) ? 1. : -1.;
    sdy = ((yCtrlPts[second] - yCtrlPts[centerIndex]) > 0) ? 1. : -1.;
    double x1 = xCtrlPts[centerIndex] + sdx * dx;
    double y1 = yCtrlPts[centerIndex] + sdy * dy;

    // The next point clockwise.
    centerIndex = (third == 0 || third == 3) ? 4 : 5;
    sdx = ((xCtrlPts[third] - xCtrlPts[centerIndex]) > 0) ? 1. : -1.;
    sdy = ((yCtrlPts[third] - yCtrlPts[centerIndex]) > 0) ? 1. : -1.;
  ...
  {
    return (visible) && (xCtrlPts[4] != xCtrlPts[5] || yCtrlPts[4] != yCtrlPts[5])
