 ...
 * @version $Id: ScaleBorder.java,v 1.1 2009-03-04 22:46:55 andrew Exp $
 */
public class ScaleBorder implements Border
{
  ...
  /**
   * Constant describing a transform in which the transformed x-axis is parallel
   * (or antiparallel) to the original x-axis and the transformed y-axis forms a
   * non-zero angle with the original one.
   */
  final static public int TYPE_X_SKEW_TRANSFORM = 2;

  /**
   * Constant describing a transform in which the transformed y-axis is parallel
   * (or antiparallel) to the original y-axis and the transformed x-axis forms a
   * non-zero angle with the original one.
   */
  ...
  /**
   * Constant describing a transform in which the transformed x-axis is parallel
   * (or antiparallel) to the original y-axis and the transformed y-axis forms a
   * non-zero angle with the original x-axis.
   */
  final static public int TYPE_SWITCHED_X_SKEW_TRANSFORM = 4;

  /**
   * Constant describing a transform in which the transformed y-axis is parallel
   * (or antiparallel) to the original x-axis and the transformed x-axis forms a
   * non-zero angle with the original y-axis.
   */
  ...
  /**
   * Constructs a ScaleBorder with the default foreground (black) and background
   * (orange) colors.
   */
  public ScaleBorder()
  {
  ...
  /**
   * Paints the border for the specified component with the specified graphics
  ...
  public void paintBorder(Component c, Graphics g, int x, int y, int width, int height)
  {
    ...
    if (vg != null)
    {
      ...
      if (scaleChanged)
      {
        Scale.drawLinearScale(minHoriz, maxHoriz, width - (insets.left + insets.right), 3, 7, Scale.RIGHT_TICKS,
            pTicks[HORIZONTAL], sTicks[HORIZONTAL], labels[HORIZONTAL], positions[HORIZONTAL]);
      }
      ...
      for (int i = 0; i < 3; i++)
      {
        ...
        {
          svg.drawString(labels[HORIZONTAL][i], (float) positions[HORIZONTAL][i], Scale.getPrimaryTickSize() + 0.2f
              * fontSize, VectorGraphics.TEXT_CENTER, VectorGraphics.TEXT_TOP);
        }
      }
      ...
      float axisPosition;
      if (Math.abs(positions[HORIZONTAL][2] - positions[HORIZONTAL][1]) > Math.abs(positions[HORIZONTAL][2]
      ...
      svg.drawString(axisLabels[HORIZONTAL] + " " + axisUnits[HORIZONTAL], axisPosition, Scale.getPrimaryTickSize()
          + 0.2f * fontSize, VectorGraphics.TEXT_CENTER, VectorGraphics.TEXT_TOP);
      ...
      if (scaleChanged)
      {
        Scale.drawLinearScale(minVert, maxVert, height - (insets.top + insets.bottom), 3, 7, Scale.LEFT_TICKS,
            pTicks[VERTICAL], sTicks[VERTICAL], labels[VERTICAL], positions[VERTICAL]);
      }
      ...
      for (int i = 0; i < 3; i++)
      {
        ...
        {
          svg.drawString(labels[VERTICAL][i], (float) positions[VERTICAL][i],
              -(Scale.getPrimaryTickSize() + 0.2f * fontSize), VectorGraphics.TEXT_CENTER, VectorGraphics.TEXT_BOTTOM);
        }
      }
      ...
      // so that the axis label should be between this position
      // and one of the other two.
      if (Math.abs(positions[VERTICAL][2] - positions[VERTICAL][1]) > Math.abs(positions[VERTICAL][2]
      ...
      svg.drawString(axisLabels[VERTICAL] + " " + axisUnits[VERTICAL], axisPosition,
          -(Scale.getPrimaryTickSize() + 0.2f * fontSize), VectorGraphics.TEXT_CENTER, VectorGraphics.TEXT_BOTTOM);
   ...
   */
  public void setScales(String horizLabel, String vertLabel, String horizUnits, String vertUnits,
      AffineTransform transform, int panelWidth, int panelHeight)
  {
   ...
   * into seven categories: parallel, switched, x-skew, y-skew, switched x-skew,
   * switched y-skew, and general. The parallel category describes
   ...
   * non-zero angle to the original one. The y-skew is similar; the switch skews
   * are just rotated (counter)clockwise by 90 degrees. The general category
   * encompasses all transforms not falling into one of the other categories.
   */
