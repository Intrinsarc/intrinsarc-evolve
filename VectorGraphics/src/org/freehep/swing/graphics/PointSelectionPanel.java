...
/**
 * This panel allows the user to select a point on the screen. The crosshair
 * cursor fill follow the system pointer when the left mouse button is pressed.
 * The system pointer will become invisible while the user is actively
 * repositioning the crosshair.
 * 
 * This panel will also respond to keystroke input.
 * <ul>
 * <li>arrow keys: move the active control point in the specified direction.
 ...
 * <li>enter key: accept selection region (send off region selected event)
 * </ul>
 * Normally, the arrow keys move the cursor by 2 pixels per key release; however
 * if the shift key is pressed simultaneously, then the arrow keys move the
 * cursor by 1 pixel.
 * 
 * @author Charles Loomis
 * @version $Id: PointSelectionPanel.java,v 1.1 2009-03-04 22:46:55 andrew Exp $
 */
public class PointSelectionPanel extends GraphicalSelectionPanel
{
  ...
  /**
   * Process key-released events. This allow the selection panel to move the
   ...
   * <li>enter key: accept selection region (send off region selected event)
   * <li>spacebar: accept selection region (send off region selected event)
   * </ul>
   * Normally, the arrow keys move the cursor by 2 pixels per key release;
   * however if the shift key is pressed simultaneously, then the arrow keys
  ...
  /**
   * The crosshair cursor will begin to follow the system pointer; the system
   * pointer is made invisible.
   * 
   * @param e
  ...
  /**
   * The crosshair cursor will stop following the system pointer when the mouse
   * is released. The system pointer will again be visible.
   * 
   * @param e
  ...
  /**
   * This method updates the position of the crosshair cursor. Normally this is
   * called internally; however, it can be called programatically by other
   * object, for example, to move the cursor on top of a selected object.
   * 
   * @param x
  ...
  public void updatePosition(int x, int y)
  {
    ...
    if (cursorVisible)
    {
      ...
      updateRect = SwingUtilities.computeUnion(xLow, yLow, 2 * cursorSize, 2 * cursorSize, updateRect);
      repaint(updateRect);
    }
  }
   ...
   * 
   * @param visible
   *          boolean indicating whether or not the crosshair cursor should be
   *          visible
   */
   ...
   * 
   * @return boolean indicating whether or not the crosshair cursor is visible
   */
  ...
  /**
   * Make the affine transform which will center the display on the current
   * point if applied.
   * 
  ...
  {
    return new AffineTransform(1., 0., 0., 1., getWidth() / 2. - currentPoint.x, getHeight() / 2. - currentPoint.y);
  }
  ...
  protected void makeSelectionEvent(int actionCode)
  {
    ...
    switch (actionCode)
    {
      ...
      case GraphicalSelectionEvent.ZOOM_NEW_VIEW :
        ...
        if (cursorVisible)
        {
          fireGraphicalSelectionMade(new PointSelectionEvent(this, actionCode, currentPoint, makeAffineTransform()));
        }
        resetSelection();
        break;
      ...
      case GraphicalSelectionEvent.UNPICK :
        ...
        if (cursorVisible)
        {
          ...
          AffineTransform scaling = new AffineTransform(sx, 0., 0., sy, halfWidth * (1. - sx), halfHeight * (1. - sy));
          trans.preConcatenate(scaling);

          fireGraphicalSelectionMade(new PointSelectionEvent(this, actionCode, currentPoint, trans));
        }
        resetSelection();
        break;
