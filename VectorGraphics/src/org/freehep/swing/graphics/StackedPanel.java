...
/**
 * StackedPanel defines an extension to JLayeredPane which allows a set of
 * equally-sized, overlayed panels to form a single 2D surface on which to draw.
 * 
 * @author Charles Loomis
 * @version $Id: StackedPanel.java,v 1.2 2009-11-29 23:20:27 Andrew Exp $
 */
public class StackedPanel extends JLayeredPane implements java.io.Serializable, PropertyChangeListener
{
  ...
  /**
   * This flag indicates that the JInternalFrame which contains this component
   * wants painting inhibited. This is generally done during a resizing
   * operation.
   */
   ...
   */
  final public static String NEED_GRAPHICAL_SELECTION_PANEL = "Class must be a type of GraphicalSelectionPanel";
  ...
  /**
   * Layer intended for interaction with the user. (e.g. zooming, picking, etc.)
   * Positioned just behind the palette layer.
   */
  ...
  /**
   * This constructor makes a new stacked panel which has no PanelArtist
   * associated with it.
   */
  public StackedPanel()
  {
  ...
  /**
   * A routine which adds a new layer to this StackedPanel. By default, the
   * layer is added above all others. Each layer will contain the given number
   * of sublayers. If the number of sublayers is larger than one and the layer
   * should be opaque, only the lowest sublayer will be opaque, all others will
   * be transparent. When retrieving the graphics context for the sublayers, the
   * topmost layer is listed first. If the number of sublayers is non-positive,
  ...
  public void addLayer(String layerName, JComponent[] panels)
  {
    ...
    if (panelList.contains(layerName))
  ...
  public void removeLayer(String layerName)
  {
    ...
    PanelArray panelArray = (PanelArray) panelHash.get(layerName);
  ...
  public void reorderLayers()
  {
    ...
    while (i.hasNext())
    {
      ...
      PanelArray panelArray = (PanelArray) panelHash.get(key);
  ...
  /**
   * Add an interaction component. Initially the component is inactive
   * (invisible).
   */
  ...
  public void activateGraphicalSelectionPanelOfClass(Class c) throws InstantiationException, IllegalAccessException
  {
    ...
    Component[] components = getComponentsInLayer(INTERACTION_LAYER.intValue());
   ...
   */
  // FIXME: this method does not do all stuff that the above method seem to do,
  // such as handling listener subscription
   ...
   * 
   * @param layerName
   *          String describing a particular layer
   * @param visible
  ...
  /**
   * Set which PanelArtist will control the graphics content of this panel. If
   * the PanelArtist is not yet available, then null can be passed in here.
   * 
   * @param panelArtist
   *          PanelArtist which provides the graphics content
   */
   ...
   * Note this can return null if no PanelArtist has been set.
   * 
  ...
  /**
   * Set the redraw flag. If set to true, the entire panel will be redrawn at
   * the next repaint. (The paintComponent method will call drawPanel() rather
   * than flushing the saved image to the screen.
   */
  ...
  /**
   * This provides an additional getGraphics method which returns the graphics
   * object associated with the image of the given layer. This will return the
   * graphics object associated with a given layer even if that layer is hidden.
   */
   ...
   * conditionally clears a particular layer depending on whether or not the
   * images are being printed.
   */
  ...
  /**
   * Override of the paintComponent() method ensures that the drawPanel() method
   * will be called if a complete redraw is needed.
   */
  public void paintComponent(Graphics g)
  {
    ...
    if (redrawNeeded && !inhibitRepaint)
    {
      ...
      if (panelArtist != null && !panelArtist.drawPanel())
        startPeriodicUpdate();
    }
  }
  ...
  public void printComponent(Graphics g)
  {
    ...
    if (panelArtist != null)
    {
      ...
      // To do the printing via the ExportDialog, the graphics
      // must be a VectorGraphics object. Make this one if it
      // isn't already.
      ...
      int w = getWidth();
      int h = getHeight();
      printGraphics = (VectorGraphics) tempGraphics.create(insets.left, insets.top, w - (insets.left + insets.right), h
          - (insets.top + insets.bottom));

      // Debug printing for the output file.
      printGraphics.printComment("StackedPanel: START ");
      ...
      {
        Component c = clist[i];
        indexed[i] = new ComponentIndex(c, getLayer(c), getPosition(c));
      }
      Arrays.sort(indexed);
      ...
      for (int i = 0; i < indexed.length; i++)
      {
        ...
        if (componentToPrint.isVisible())
        {
          ...
          {
            printGraphics.printComment("StackedPanel: " + i + " " + panelArtist.getDescription());
            panelArtist.drawPanel();
          }
          else
            if (componentToPrint instanceof JComponent)
            {
              printGraphics.printComment("StackedPanel: " + i + " JComponent "
                  + (componentToPrint.getClass()).getName());
              componentToPrint.print(g);
            }
            else
            {
              printGraphics
                  .printComment("StackedPanel: " + i + " Component " + (componentToPrint.getClass()).getName());
      ...
      printGraphics.printComment("StackedPanel: END ");
  ...
  /**
   * This callback routine is intended to be used by the controlling PanelArtist
   * after it has returned false from the drawPanel() method, indicating that
   * the redraw of the panel has not yet completed. This signals that the redraw
   * is now complete.
   */
  ...
  /**
   * Start a periodic update of this panel. This is called internally when the
   * controlling PanelArtist is drawing into the images from another, background
   * thread.
   */
  ...
  /**
   * Set the border of this component. This is intercepted so that if the size
   * of the border changes, the redraw flag can be set.
   */
  public void setBorder(Border border)
  {
    ...
    if (insets.left != borderInsets.left || insets.top != borderInsets.top || insets.right != borderInsets.right
  ...
  /**
   * Intercept a resize event so that the redraw flag can be set. (Swing
   * internally calls reshape() which is then forwarded to setBounds().)
   */
  public void setBounds(int x, int y, int w, int h)
  {
    ...
    if (!inhibitRepaint)
    {
      ...
      if (panelArtist != null)
        panelArtist.panelResized();
    }
  }

  /**
   * This inner class is the task which is executed to periodically update the
   * display.
   */
  ...
  /**
   * Look for changes in property values. If a property is set on a component
   * which has a key "InhibitRepaint" and a value of true, repainting of the
   * component will be inhibited.
   */
  ...
  public void printBorder(Graphics g)
  {
    ...
    // Get a VectorGraphics context, if possible.
  ...
  /**
   * Do NOT print the children because the printing is handled directly by the
   * PanelArtist.
   */
  ...
  /**
   * A protected class which simply wraps the set of Panels which form the
   * sublayers of a particular layer.
   */
  ...
  /**
   * This private class takes a component, its layer, and its position. It is
   * used to sort the components into the display order (lowest layer, highest
   * position).
   */
