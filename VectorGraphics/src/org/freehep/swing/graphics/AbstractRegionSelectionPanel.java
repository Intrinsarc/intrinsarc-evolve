...
/**
 * This abstract class defines the majority of the functionality needed to make
 * selections of arbitrary parallelogram regions on the screen.
 * 
 * @author Charles Loomis
 * @author Mark Donszelmann
 * @version $Id: AbstractRegionSelectionPanel.java,v 1.1 2006-07-03 13:33:26
 *          amcveigh Exp $
 */
abstract public class AbstractRegionSelectionPanel extends GraphicalSelectionPanel
{

  /**
   * A constant which flags that no control point was near the mouse-pressed
   * event.
   */
  ...
  /**
   * The bounding box of the region to repaint, usually the union of the
   * rectToDraw and lastDrawnRect rectangles.
   */
  ...
  /**
   * Flag indicating whether or not the last drawn rectangle is valid. (Not
   * valid when the box is first made visible.)
   */
  protected boolean lastDrawnRectValid;

  /**
   * The maximum distance from a control point the cursor can be and still be
   * selected.
   */
  ...
  /**
   * Which control point is the active one, or which one can be controlled from
   * the arrow keys on the keyboard?
   */
  protected int activeCtrlPt;

  /**
   * The x-coordinates of the control points. The first four of these control
   * points MUST define the outer boundries of the selected region.
   */
  protected int[] xCtrlPts;

  /**
   * The y-coordinates of the control points. The first four of these control
   * points MUST define the outer boundries of the selected region.
   */
  protected int[] yCtrlPts;

  /**
   * This constructor makes a new AbstractRegionSelectionPanel. This constructor
   * only sets the visiblilty flag and the last-drawn rectangle valid flag to
   * false.
   */
  ...
  /**
   * Process key-released events. This allows selection panels which derive from
   * this one to automatically have the default behaviour.
   * 
   * <pre>
   ...
   * user accepts the current selection.
   */
  protected void makeSelectionEvent(int actionCode)
  {
    switch (actionCode)
    {
      ...
      case GraphicalSelectionEvent.DEFAULT_MODE :
        resetSelection();
        setVisible(false);
        fireGraphicalSelectionMade(new GraphicalSelectionEvent(this, GraphicalSelectionEvent.DEFAULT_MODE, null, null));
        break;

      case GraphicalSelectionEvent.NEXT_MODE :
        resetSelection();
        setVisible(false);
        fireGraphicalSelectionMade(new GraphicalSelectionEvent(this, GraphicalSelectionEvent.NEXT_MODE, null, null));
        break;

      case GraphicalSelectionEvent.PREVIOUS_MODE :
        resetSelection();
        setVisible(false);
        fireGraphicalSelectionMade(new GraphicalSelectionEvent(this, GraphicalSelectionEvent.PREVIOUS_MODE, null, null));
        break;
      ...
      default :
        ...
        if (visible)
        {
          ...
          fireGraphicalSelectionMade(new RegionSelectionEvent(this, actionCode, makeOutlinePolygon(),
              makeAffineTransform()));
        }
        resetSelection();
        break;
  ...
  public void mouseMoved(MouseEvent e)
  {
    if (!isProcessingPopup(e))
    {
      if (visible)
      {
        int newCtrlPt = nearWhichControlPoint(e.getX(), e.getY(), hitThreshold);
  ...
  /**
   * A utility method which forces the x-coordinate to be within the component
   * boundries.
   * 
   * @param x
  ...
  /**
   * A utility method which forces the y-coordinate to be within the component
   * boundries.
   * 
   * @param y
  ...
  /**
   * This returns whether the current selected region is valid. Generally if the
   * area has zero volume, then this method should return false.
   */
  abstract public boolean isValidSelection();

  /**
   * A utility method which moves the currently active control point by the
   * given delta-x and delta-y. It does this by calling
   * updateActiveControlPoint(x,y), so subclasses shouldn't normally need to
   * override this method.
   * 
   * @param dx
   *          the distance to move the x-coordinate
   * @param dy
  ...
  /**
   * Check to see if the point (x,y) is near one of the control points. If it
   * is, return the index of the nearest one, otherwise return NO_CONTROL_POINT.
   * 
   * @param x
   *          x-coordinate to compare to control points
   * @param y
   *          y-coordinate to compare to control points
   * @param maxDist
   *          the maximum distance from a control point which still selects it
   * 
  ...
  /**
   * Repaint the panel. Calculate the bounding box of the selection box along
   * with associated control points.
   */
  protected void repaintPanel()
  {
    ...
    {
      updateRect = SwingUtilities.computeUnion(lastDrawnRect.x, lastDrawnRect.y, lastDrawnRect.width,
          lastDrawnRect.height, updateRect);
    }
    repaint(updateRect);
  }

  /**
   * Initialize the control points. Subclasses must provide an implementation of
   * this method which initializes the control points to reasonable values given
   * the first mouse-pressed coordinates, and must also set the activeCtrlPt to
   * the index of the control point which should be active.
   * 
   * @param x
   *          x-coordinate of initial mouse-pressed event
   * @param y
   *          y-coordinate of initial mouse-pressed event
   */
  abstract public void initializeControlPoints(int x, int y);

  /**
   * Change the active control point to the point (x,y). Subclasses should
   * implement this routine to get the behaviour which is desired. This is the
   * place to impose constraints on how the control points can move. NOTE:
   * repaintPanel() should be called at the end of this method to update the
   * display.
   * 
   * @param x
  ...
  /**
   * Useful subclasses must define the number of control points on the selected
   * region. The first four control points define the outer extent of the
   * selected region. This method MUST NOT return a number less than four.
   */
  abstract public int getNumberOfControlPoints();

  /**
   * Returns the Cursor to be displayed for a certain control point and the
   * default cursor for this SelectionPanel for an index of NO_CONTROL_POINT.
   * Return of null will not change the cursor. Subclasses should override this
   * method to provide a default cursor and/or to provide cursors for the
  ...
  /**
   * Repaint this component. This must be overridden by subclasses so that the
   * selection region appears correctly. The subclass should check the
   * visibility flag (visible) to decide if any painting needs to be done.
   * 
   * @param g
  ...
  /**
   * Make the outline of the selection. Note that the order of the points is not
   * guaranteed.
   * 
   * @return a polygon object which describes the outline of the selection
   */
  ...
  /**
   * A utility which makes an AffineTransform given three corner points. The
   * first point must be the upper, left-hand corner point, the second, the
   * upper, right-hand corner point, and the third, the lower, right-hand corner
   * point.
   * 
  ...
  /**
   * returns the appropriate cursor for any of the compass points. If both dx
   * and dy are zero, null is returned
   * 
   * @param type
   *          type of cursor (Resize/Rotation)
   * @param dx
   *          screen x of direction
   * @param dy
   *          screen y of direction (positive is down)
   * @param n
   *          number of compass points (4 or 8)
   * @param diagonal
