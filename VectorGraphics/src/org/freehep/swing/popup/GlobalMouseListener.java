 ...
 * works by recursively adding mouse listeners to all components in the
 * heirarchy. It also adds a ComponentListener so that it can add and remove
 * listeners from items as they are added to the tree.
 * <p>
 * Warning: This class can have unexpected side effects in some rare cases.
 * Normally swing will delegate mouse events to a component's parent if that
 * component does not listen for mouse events itself. Since this class adds
 * mouse listeners to all components, it can have the side effect of preventing
 * this mouse event delegation. (In Java 1.4 it is possible to tell if a mouse
 * listener already exists on a component, so maybe it would be possible to work
 * around this limitation. Not clear how you would know if a mouse listener was
 * subsequently added).
 * 
 * @author tonyj
 * @version $Id: GlobalMouseListener.java,v 1.1 2009-03-04 22:47:00 andrew Exp $
 */
public class GlobalMouseListener
{
   ...
   * 
   * @param c
   *          The component on which to listen for mouse events
   */
