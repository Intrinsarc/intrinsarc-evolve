 ...
 * interface HasNextPages if you want there to be pages following this page, and
 * implement the Finishable interface if you want this to be the page. You may
 * implement one or both, but implementing neither would be silly because then
 * neither the "Next" nor "Finish" button will enable and your page will be a
 * "dead end". Implement HasHelpPage if you want your page to link to a help
 * topic. Override the methods <code>getNextEnabled()</code> and
 * <code>getFinishEnabled()</code> to control how the "Next" and "Finish"
 * buttons enable.
 * <p>
 * This class is a KeyListener and has implemented the interface with
 * non-methods. If you do not override these methods, <code>doEnable()</code>
 * will be called when a key is released on components registered as
 * KeyListeners. This implementation is designed for when the buttons enable
 * when text is added to them. For example, if you page had<br>
 * <code>
 ...
 * in the field <code>text</code>. You might define
 * <code>getNextEnabled()</code> as follows:<br>
 * 
 * <pre>
...
public abstract class WizardPage extends JPanel implements KeyListener
{
  ...
  /**
   * Supply a layout manager for the panel to use. See Sun's notes on <a
   * href="http://www.javasoft.com/docs/books/tutorial/ui/layout/using.html">using
   ...
   * light-weight components. You should use only <a
   * href="http://java.sun.com/products/jfc/swingdoc-api/packages.html">swing
   * components</a>. The jas.util package has a number of <a
   * href="http://www-sldnt.slac.stanford.edu/jas/Documentation/JASUtilClasses.htm">utility
   * components</a> that may be useful for these pages.
   * 
   * @param lm
   *          the layout manager to use for this page.
  ...
  /**
   * This method is called just before the page is brought to the screen. By
   * default, it does nothing but you can override it to do some last-minute
   * setup on your page.
   */
  ...
  /**
   * Override this to provide your own behaviour for key events; does nothing by
   * default.
   */
   ...
   * the buttons will be enabled on every key event in the definition for this
   * method.
   */
  ...
  /**
   * This method is called if the user has canceled the wizard. By default it
   * does nothing, but you can override it to perform cleanup if necessary. The
   * wizard follows a preorder tree traversal, so the <code>onCancel()</code>
   * methods are called starting from the bottom and working up each branch
   * separately to the root.
   */
   ...
   * By default, this method returns whether the page implements Finishable. If
   ...
   * page as a key listener to a component, and then <code>doEnable()</code>
   * will be called each time a key is released in that component.
   * 
   * @see WizardDialog#doEnable()
   * @see Finishable
  ...
  /**
   * Forces the wizard to call <code>getNextEnabled()</code> and
   * <code>getFinishEnabled()</code> to enable the buttons on the wizard. This
   * method is called by the <code>keyReleased(KeyEvent)</code> method if you
   * add this class as a KeyListener to a text field.
   * 
   * @see WizardDialog#getNextEnabled()
   * @see WizardDialog#getFinishEnabled()
   * @see WizardDialog#keyReleased(KeyEvent)
   * @see java.awt.event.KeyEvent
   * @see java.awt.event.KeyListener
   */
  protected void doEnable()
  {
    // m_wizard will be null if this page has not yet been attached to a
    // WizardDialog
    if (m_wizard == null)
      return;
