...
/**
 * A class which allows a user to select directories or files, similar to
 * JFileChooser, except that it display files as a tree, and is better suited to
 * selecting directories than the current file chooser (see java bug id
 * 4239219).
 * 
 * TODO: Understand issue with moving mouse between double clicks (seems like
 * MOUSE_PRESSED and MOUSE_RELEASED events are generated, but not MOUSE_CLICKED)
 * See Bug ID 4218549
 * 
 * @author Tony Johnson (tony_johnson@slac.stanford.edu)
 * @version $Id: JDirectoryChooser.java,v 1.1 2009-03-04 22:46:59 andrew Exp $
 */
public class JDirectoryChooser extends JComponent
{
   ...
   *          The directory to which the tree is initially set
   */
  ...
  private void makeCurrentDirectoryVisible()
  {
    if (currentDirectory != null)
    {
      ...
      {
        if (dir == null)
          return; // Something wrong, ignore dir
        v.addElement(dir);
        if (view.isRoot(dir))
          break;
   ...
   * 
   * @param hide
   *          True if hidden files should not be shown
   */
   ...
   * 
   * @param mode
   *          either DIRECTORIES_ONLY or FILES_AND_DIRECTORIES (the default)
  ...
  public void setMultiSelectionEnabled(boolean enable)
  {
    ...
    tree.getSelectionModel().setSelectionMode(
        enable ? TreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION : TreeSelectionModel.SINGLE_TREE_SELECTION);
  }
  /**
  ...
  /**
   * Returns the currently selected file (or the first selected file if multiple
   * files are selected)
   */
   ...
   * @return either CANCEL_OPTION or JAPPROVE_OPTION
  ...
  private class ButtonListener implements ActionListener, TreeSelectionListener
  {
    public void actionPerformed(ActionEvent e)
    {
      if (e.getSource() == approve)
        returnValue = foo.APPROVE_OPTION;
      dialog.hide();
      dialog = null;
    }
