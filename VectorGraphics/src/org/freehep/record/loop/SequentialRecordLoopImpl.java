 ...
 *
 * Version $Id: SequentialRecordLoopImpl.java,v 1.1 2009-03-04 22:46:56 andrew Exp $
 *
 ...
 * <code>SequentialRecordLoop</code> interface. This class is minimally thread
 * safe i.e. only the "setInterruptResquested" and "isInterruptRequested"
 * methods are considered thread safe. The rest of the implementation is
 * designed to run in a single thread. When its RecordListener responses to any
 * RecordEvents, a return from that method signals that the listener has
 * completed all responsibilities related to that message.
 * 
 * @version $Id: SequentialRecordLoopImpl.java,v 1.1 2006-07-03 13:33:34
 *          amcveigh Exp $
 * @author patton
 */
public class SequentialRecordLoopImpl implements SequentialRecordLoop
{
  ...
  /**
   * The default progress interval, number of millisseconds, if no guideline
   * have been set.
   */
  ...
  /**
   * The number of countable records supplied by the current or last loop.
   */
  ...
  /**
   * True if interruptRequested was true at last attempt to getCurrentRecord.
   */
  private boolean stoppedDueToInterrupt;

  /**
   * True if the RecordListener should be configured rather than reconfigured.
   */
  private boolean newListener;

  /**
   * True if the current configuration object has been set since the last loop.
   */
   ...
   *           if this is called when this object has no SequentialRecordSource.
   * @see #setRecordSource
   */
  void beginLoop(long target) throws IllegalStateException
  {
    ...
    {
      ((RecordLoopListener) loopListeners.get(loopListener)).loopBeginning(loopEvent);
    }

    // Tranistion this objects listener into its configured state.
    if (newListener)
    {
      listener.configure(new ConfigurationEvent(this, configuration));
    ...
    else
    {
      if (newConfiguration)
      {
        listener.reconfigure(new ConfigurationEvent(this, configuration));
        newConfiguration = false;
      }
      else
      {
  ...
  long endLoop() throws LoopException
  {
    ...
    for (int loopListener = 0; finished != loopListener; loopListener++)
    {
      ((RecordLoopListener) loopListeners.get(loopListener)).loopEnded(loopEvent);
    }
    ...
    if (stoppedDueToInterrupt || isInterruptRequested())
    {
      ...
      throw new LoopInterruptedException(getClass().getName() + " was interrupted after " + countableSupplied
          + " records supplied.", supplied, countableSupplied);
    }
    ...
    {
      throw new LoopSourceExhaustedException("Source was exhausted after " + countableSupplied + " records supplied.",
          supplied, countableSupplied);
    }

    // Throw exception if no record could be read by the source
    if (noRecord)
    {
      throw new NoLoopRecordException("Filed to find a reocrd after " + countableSupplied + " records supplied.",
          supplied, countableSupplied);
    }
  ...
  private void fireProgress(double fraction)
  {
    ...
    for (int loopListener = 0; finished != loopListener; loopListener++)
    {
      ((RecordLoopListener) loopListeners.get(loopListener)).progress(progressEvent);
    }
  }

  /**
   * This attempts to get a new record object from the source. Before invoking
   * the getCurrentRecord method, this routine checks to see if an interrupt has
   * been requested, and if so set the internal flag to indicate this is the
   * case.
   * 
  ...
  Object getNextRecord() throws IOException
  {
    ...
    else
    {
      ...
      try
      {
        ...
        if (null == result)
        {
          throw new NullPointerException("'null' returned by" + " SequenctialRecordSource getCurrentRecord()"
              + " method");
        }
      }
  ...
  /**
   * Returns true if there may be more records available and the number supplied
   * has not reached the number requested.
   * 
   * @return true if more records are needed to complete the loop.
   */
  boolean hasMoreRecords()
  {
    return ((!stoppedDueToInterrupt) && (target != countableSupplied) && (!sourceExhausted));
  }
  ...
  public long loop(final long number) throws LoopException, IOException
  {
    beginLoop(number);
  ...
  public void reset() throws IOException
  {
    ...
    for (int loopListener = 0; finished != loopListener; loopListener++)
    {
      ((RecordLoopListener) loopListeners.get(loopListener)).loopReset(loopEvent);
    }
  }
  ...
  /**
   * Supplies the specified record object, if it is not null, to the listener of
   * this object.
   * 
   * @param record
  ...
  void supplyRecord(Object record)
  {
    ...
    listener.recordSupplied(new RecordSuppliedEvent(this, record));
    currentRecord = null;
    totalSupplied++;
    ...
    if (0 != loopListeners.size())
    {
      ...
      if (0 < recordInterval)
      {
        recordsRemaining = recordInterval + lastProgressSupplied - totalSupplied;
      }
      else
      {
      ...
      if (0 < millisInterval)
      {
        millisRemaining = millisInterval + lastProgressTime - System.currentTimeMillis();
      }
      else
      {
      ...
      if ((!(0 < recordsRemaining))
          || (!(0 < millisRemaining))
          || ((!(0 < recordInterval)) && (!(0 < millisInterval)) && (0 > DEFAULT_MILLIS_INTERVAL
