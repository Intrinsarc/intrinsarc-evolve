 ...
 *
 * Version $Id: StateMachineTester.java,v 1.1 2009-03-04 22:46:59 andrew Exp $
 *
 ...
 * @author patton
 */
public class StateMachineTester implements RecordListener
{
  ...
  {
    Assert.assertTrue("Illegal configure transition", READY_STATE == state);
    setState(CONFIGURED_STATE);
  }

  public void finish(RecordEvent event)
  {
    Assert.assertTrue("Illegal finish transition", (SUSPENDED_STATE == state));
  ...
  {
    Assert.assertTrue("Illegal recordSupplied transition", (CONFIGURED_STATE == state) || (PROCESSING_STATE == state));
    setState(PROCESSING_STATE);
  }

  public void reconfigure(ConfigurationEvent event)
  {
    Assert.assertTrue("Illegal reconfigure transition", SUSPENDED_STATE == state);
    setState(CONFIGURED_STATE);
  }

  public void resume(RecordEvent event)
  {
    Assert.assertTrue("Illegal resume transition", SUSPENDED_STATE == state);
    setState(CONFIGURED_STATE);
  }
  ...
  {
    Assert.assertTrue("Illegal suspend transition", (CONFIGURED_STATE == state) || (PROCESSING_STATE == state));
