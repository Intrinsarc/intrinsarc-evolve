...
public abstract class FindDialog extends JDialog {
	...
	JRadioButton upRadioButton, downRadioButton, allLinesRadioButton,
			selectedLinesRadioButton;
	JButton findButton, replaceButton, replaceFindButton, replaceAllButton,
			closeButton;
	JLabel statusLabel;
	...
	public FindDialog(Frame container) {
		...
		final ButtonGroup directionButtonGroup = new ButtonGroup(), scopeButtonGroup = new ButtonGroup();
		final JPanel inputPanel = new JPanel(), directionPanel = new JPanel(), scopePanel = new JPanel(), optionsPanel = new JPanel(), statusPanel = new JPanel();
		final JLabel findLabel = new JLabel("Find:"), replaceLabel = new JLabel(
				"Replace With:");
		final GridBagLayout layout = new GridBagLayout(), inputLayout = new GridBagLayout(), statusLayout = new GridBagLayout();
		...
		findButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				final JTextComponent t = currentTextComponent();

				if (t == null)
					return;

				// Find the next occurrence
				final int startIndex = findNext(t.getText(), t.getCaret(),
						findField.getText(), matchCaseCheckBox.isSelected(),
						wrapCheckBox.isSelected(), wholeWordCheckBox
								.isSelected(), selectedLinesRadioButton
								.isSelected(), downRadioButton.isSelected());
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (t.getSelectedText() != null
						&& (matchCaseCheckBox.isSelected()
								&& t.getSelectedText().equals(
										findField.getText()) || t
								.getSelectedText().equalsIgnoreCase(
										findField.getText()))) {

					final int startIndex = t.getSelectionStart();
		...
		final int w = Math.max(directionPanel.getPreferredSize().width,
				scopePanel.getPreferredSize().width), h = Math.max(
				directionPanel.getPreferredSize().height, scopePanel
	...
	int findNext(String text, Caret caret, String query, boolean matchCase,
			boolean wrap, boolean wholeWord, boolean selectedLines,
			boolean searchDown) {

		int caretStart = Math.min(caret.getDot(), caret.getMark()), caretEnd = Math
				.max(caret.getDot(), caret.getMark()), offset = 0;
		...
		int index = searchDown ? text.indexOf(query, caretEnd) : text
				.lastIndexOf(query, Math.max(caretStart - 1, 0));
		if (index < 0 && wrap)
			index = searchDown ? text.indexOf(query) : text.lastIndexOf(query);
	...
	String matchingExpression(String text, boolean matchCase) {
		if (matchCase)
			...
			final String upper = text.toUpperCase(), lower = text.toLowerCase();
