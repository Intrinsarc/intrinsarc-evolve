...
public class SceneAnimationController implements Runnable, AnimationMessage,
		AnimationListener {
	Animator animator;
	...
	public SceneAnimationController(Animator a,
			Relation<String, String> actionMap,
			Relation<String, String> controlMap, boolean replay, Object lock) {
		animator = a;
		replayMode = replay;
		canvas = lock;
	...
	void doActions() throws InterruptedException {
		try {
			while (true) {
				...
				int choice = -1;
				while ((choice = getValidControl()) < 0)
					canvas.wait();
				doMenuStep(choice);
			}
	...
	public class AnimationException extends Exception {
		private static final long serialVersionUID = -6231855129821290263L;
