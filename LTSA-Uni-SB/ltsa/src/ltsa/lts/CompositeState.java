...
public class CompositeState {
	...
	public void prefixLabels(String prefix) {
		...
		for (Enumeration<CompactState> ee = machines.elements(); ee
				.hasMoreElements();) {
			CompactState mm = ee.nextElement();
	...
	public void addAccess(Vector<String> pset) {
		...
		if (n == 0)
			return;
		String s = "{";
		Enumeration<String> e = pset.elements();
		int i = 0;
	 ...
	 */
	public CompactState relabel(Relation<String, String> oldtonew,
			LTSOutput output) {
	...
	protected void initAlphaStop() {
		...
		for (Enumeration<CompactState> e = machines.elements(); e
				.hasMoreElements();) {
			CompactState m = e.nextElement();
		...
		int j = 1;
		for (Enumeration<String> e = alpha.keys(); e.hasMoreElements();) {
