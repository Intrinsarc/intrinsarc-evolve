...
public class Determinizer {
	...
	Vector<EventState> newStates; // list of newStates, indexed transition lists
	// (EventState)
	...
	public CompactState determine() {
		...
		newStates = new Vector<EventState>(machine.maxStates * 2);
		stateSets = new Vector<BitSet>(machine.maxStates * 2);
		map = new Hashtable<BitSet, Integer>(machine.maxStates * 2);
		nextState = 0;
		currentState = 0;
	...
	protected void compute(int n) {
		...
		for (int i = 0; i < state.size(); ++i) {
			if (state.get(i))
				tr = EventState.union(tr, machine.states[i]);
		}
		...
		while (action != null) { // for each action
			...
			newtr = EventState.add(newtr, new EventState(action.event,
					newStateId));
			action = action.list;
		}
	...
	protected CompactState makeNewMachine() {
		...
		if (machine.endseq >= 0) {
			...
			if (ii != null)
				m.endseq = ii.intValue();
		}
		output.outln("DFA(" + machine.name + ") has " + m.maxStates
				+ " states.");
		return m;
	}
