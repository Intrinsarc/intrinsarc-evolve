...
public class SuperTrace {
	...
	public void analyse() {
		...
		if (ret == DEADLOCK) {
			...
			if (errorTrace.size() <= 100)
				...
				output.outln("Trace length " + errorTrace.size()
						+ ", replay using Check/Run");
		} else if (ret == ERROR) {
			output.outln("Trace to property violation in "
					+ mach.getViolatedProperty() + ":");
		...
		output.outln(String
				.format((Locale) null, "Analysed using Supertrace in %.1f ms",
						1e-6 * (finish - start)));
	}
	...
	private int hashTwo(byte[] val) {
		...
		final int h = (int) ((value ^ (value >>> 32)));
	...
	private int search() {
		...
		while (!stack.empty()) {
			if (stack.marked()) {
				...
				final byte[] currentState = stack.peek();
				stack.mark();
				if (onStack != null)
					onStack.put(currentState);
				final MyList transitions = mach.getTransitions(currentState);
				if (transitions.empty() && !mach.END(currentState))
					return DEADLOCK;
	...
	public LinkedList<String> getErrorTrace() {
		return errorTrace;
	}

	private void outStatistics(int depth, int states, int transitions) {
		output.out("-- Depth: " + depth + " States: " + states
				+ " Transitions: " + transitions);
		final Runtime r = Runtime.getRuntime();
		output.outln(" Memory used: " + (r.totalMemory() - r.freeMemory())
				/ 1000 + "K");
	}
	...
	private LinkedList<String> computeTrace(boolean error) {
		mach.disablePartialOrder();
		final LinkedList<String> trace = new LinkedList<String>();
	...
	private String findAction(byte[] from, byte[] to) {
		final MyList t = mach.getTransitions(from);
		while (!t.empty()) {
