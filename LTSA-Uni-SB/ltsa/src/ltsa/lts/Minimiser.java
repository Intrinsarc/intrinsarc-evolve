...
public class Minimiser {
	...
	private CompactState machTau(CompactState m) {
		...
		for (int i = 0; i < m.states.length; i++) {
			m.states[i] = EventState.union(m.states[i], T[i]);
			m.states[i] = EventState.actionAdd(m.states[i], m.states);
		}
		for (int i = 0; i < m.states.length; i++)
			m.states[i] = EventState.add(m.states[i], new EventState(
					Declaration.TAU, i));
		output.out(".");
		return m;
	}
	...
	public CompactState minimise() {
		...
		final CompactState saved = machine.myclone();
		/*
		 * distinguish end state from STOP with self transition using special
		 * label
		 */
		if (machine.endseq >= 0) {
			final int es = machine.endseq;
			machine.states[es] = EventState.add(machine.states[es],
					new EventState(machine.alphabet.length, es));
		}
		if (machine.hasTau()) {
			initTau();
		...
		dominimise();
		machine = saved;
		final CompactState c = makeNewMachine();
		final long finish = System.nanoTime();
		output.outln("");
		output.outln(String.format((Locale) null,
				"Minimised States: %d in %.1f ms", c.maxStates,
				1e-6 * (finish - start)));
		return c;
	}
	...
	public CompactState trace_minimise() {
		...
		if (must_minimize || machine.isNonDeterministic()) {
			must_minimize = true;
			final Determinizer d = new Determinizer(machine, output);
			machine = d.determine();
		}
		// now minimise
		if (must_minimize)
	...
	private CompactState makeNewMachine() {
		...
		final Hashtable<Integer, Integer> newtoold = new Hashtable<Integer, Integer>();
		final Counter newSt = new Counter(0);
		for (int i = 0; i < E.length; i++) {
			final Integer oldIndex = new Integer(i);
		...
		if (machine.endseq < 0)
			...
			m.endseq = oldtonew.get(new Integer(machine.endseq)).intValue();
			/* remove marking transition */
			m.states[m.endseq] = EventState.remove(m.states[m.endseq],
					new EventState(m.alphabet.length, m.endseq));
		}

		for (int i = 0; i < machine.maxStates; i++) {
			final int newi = oldtonew.get(new Integer(i)).intValue();
			final EventState tmp = EventState.renumberStates(machine.states[i],
					oldtonew);
		...
		for (int i = 0; i < m.maxStates; i++)
			...
			m.states[i] = EventState.remove(m.states[i], new EventState(
					Declaration.TAU, i));
		return m;
	}
	...
	private void privPrint(LTSOutput output, BitSet[] E) {
		if (E.length > 20)
			return;
		final char[] buf = new char[E.length * 2];
