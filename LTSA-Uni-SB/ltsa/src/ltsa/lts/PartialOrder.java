...
public class PartialOrder {
	...
	private final CompactState machines[]; // array of machines to be composed
	private final int[][] actionSharedBy; // [action number] list of machines
	// that share action
	// ordered by machine number
	private final StackChecker checker; // provides on stack check
	private final int candidates[][]; // 0=not a candidate, 1= all independent
	// actions, 2= max shared by two etc.
	private int partners[][]; // records partner for candidate = 2 states -1
	// otherwise
	private final int Nactions;

	private final BitSet visible; // visible actions
	private boolean preserveOE; // if set preserve Observational Equivalence
	private final BitSet high;

	public PartialOrder(Hashtable<String, BitSet> alphabet, // map action name
			// to bitmap of
			// shared machines
			String[] actionName, // map number to name;
			CompactState[] sm, StackChecker ck, // array of state machines to be
			// composed
			Vector<String> hidden, // vector of prefixes
	...
	public List<int[]> transitions(int[] state) {
		...
		for (int mach = 0; mach < machines.length; ++mach) {
			if (candidates[mach][state[mach]] == 1) {
				List<int[]> trs = new ArrayList<int[]>(8);
	...
	private BitSet getUnshared(int mach, int state[]) {
		...
		Enumeration<EventState> e = machines[mach].states[state[mach]]
				.elements();
		while (e.hasMoreElements()) {
			EventState es = e.nextElement();
			if (es.event == 0) {
				b.set(es.event);
			} else {
				if (actionSharedBy[es.event].length == 1)
					b.set(es.event);
			}
		}
		if (b.length() == 0)
