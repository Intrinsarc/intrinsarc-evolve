...
public class LTSCanvas extends JPanel implements Scrollable {
	...
	private static final long serialVersionUID = -5796491600261466584L;
	...
	public void setBigFont(boolean flag) {
		...
		if (drawing != null)
			for (int i = 0; i < drawing.length; i++)
				if (drawing[i] != null)
					drawing[i].setFonts(nameFont, labelFont);
		repaint();
	}
	...
	public void setDrawName(boolean flag) {
		...
		if (drawing != null)
			for (int i = 0; i < drawing.length; i++)
				if (drawing[i] != null)
					drawing[i].setDrawName(displayName);
		repaint();
	}
	...
	public void setNewLabelFormat(boolean flag) {
		...
		if (drawing != null)
			for (int i = 0; i < drawing.length; i++)
				if (drawing[i] != null)
					drawing[i].setNewLabelFormat(newLabelFormat);
		repaint();
	}
	...
	public void draw(int id, CompactState m, int last, int current, String name) {
		...
		final Dimension d = drawing[id].getSize();
		final Dimension e = getPreferredSize();
		setPreferredSize(new Dimension(Math.max(e.width, d.width), Math.max(
				e.height, d.height)));
		revalidate();
		repaint();
	}
	...
	public int clearSelected() {
		if (focus == null || singleMode || drawing == null)
			return -1;
		int ret = 0;
		while (drawing[ret] != focus)
			++ret;
		focus = null;
		drawing[ret] = null;
		repaint();
	...
	private final Rectangle rr = new Rectangle();
	...
	public void stretchHorizontal(int d) {
		if (focus != null) {
			...
			setPreferredSize(new Dimension(Math.max(p.width, rr.x + rr.width),
	...
	public void select(int n, int last[], int current[], String name) {
		...
		for (int i = 0; i < n; i++) {
			if (drawing[i] != null) {
				final int ls = last != null ? last[i] : 0;
				final int cs = current != null ? current[i] : 0;
	...
	class MyMouse extends MouseInputAdapter {
		...
		public void mouseDragged(MouseEvent e) {
			if (focus != null) {
				...
				if (start != null) {
					final double xoff = current.getX() - start.getX();
					final int newX = (int) (r.x + xoff);
					final double yoff = current.getY() - start.getY();
					final int newY = (int) (r.y + yoff);
		...
		public void mouseReleased(MouseEvent e) {
			...
			if (focus != null) {
				...
				if (!r.contains(e.getPoint())) {
					...
					final Dimension p = getPreferredSize();
					setPreferredSize(new Dimension(Math.max(p.width, r.x
							+ r.width), Math.max(p.height, r.y + r.height)));
					revalidate();
				}
			}
		}
