...
public class ProgressCheck {
	...
	public void doProgressCheck() {
		...
		output.outln(String.format((Locale) null, "Progress Check in %.1f ms",
				1e-6 * (finish - start)));
	}
	...
	public void doLTLCheck() {
		...
		if (hasERROR)
			...
			output.outln("More than " + Maxviolation + " violations");
		output.outln(String.format((Locale) null,
				"LTL Property Check in %.1f ms", 1e-6 * (finish - start)));
	}
	...
	private void findCC() { // non-recursive version
		...
		while (!stk.empty()) {
			...
			final MyList transitions = mach.getTransitions(currentState.key);
			while (!transitions.empty()) {
				...
				// transitions
				{
					final MyHashProgEntry child = hh.get(transitions.getTo());
					if (child == null) {
	 ...
	 * output.outln("state: "+e.key+" dfn: "
	 * +e.dfn+" low: "+e.low+" ret "+e.isReturn); }
	 */
	...
	private boolean component(MyHashProg hstk, Stack<byte[]> stack,
		...
		final Stack<byte[]> trace = new Stack<byte[]>();
		final BitSet hasActions = new BitSet(mach.getAlphabet().length);
		// output.out("{");
		byte[] m;
		...
		do {
			...
			if (progress) {
				...
				final MyList transitions = mach.getTransitions(m);
				while (!transitions.empty()) {
					final int act = transitions.getAction();
					hasActions.set(act);
					transitions.next();
				}
			} else {
		...
		for (final Enumeration<byte[]> e = trace.elements(); e
				.hasMoreElements();) {
			final byte[] i = e.nextElement();
			final MyHashProgEntry hse = hstk.get(i);
	...
	private boolean missing(BitSet actions) {
		final int alphalen = mach.getAlphabet().length;
		if (ProgressTest.noTests()) { // check for all actions
			...
			tnames = null;
			final Enumeration<ProgressTest> e = ProgressTest.tests.elements();
			while (e.hasMoreElements()) {
				final ProgressTest p = e.nextElement();
				if (p.cset == null) {
	...
	private boolean terminalComponent(MyHashProg hstk, Vector<byte[]> component) {
		final BitSet tc = new BitSet(10001);
		for (final Enumeration<byte[]> e = component.elements(); e
				.hasMoreElements();) {
			final byte[] i = e.nextElement();
			final MyHashProgEntry hse = hstk.get(i);
		...
		for (final Enumeration<byte[]> e = component.elements(); e
				.hasMoreElements();) {
			final byte[] i = e.nextElement();
			final MyList tr = mach.getTransitions(i);
			while (!tr.empty()) {
				if (tr.getTo() == null) {
					hasERROR = true;
					return false;
				}
				final MyHashProgEntry hse = hstk.get(tr.getTo());
				if (hse == null)
					return false;
				if (hse.dfn == 0)
					return false;
	...
	private boolean nontrivial(Vector<byte[]> component) {
		if (component.size() > 1)
			return true;
		final byte[] i = component.elementAt(0);
		final MyList transitions = mach.getTransitions(i);
		while (!transitions.empty()) {
			final int act = transitions.getAction();
	...
	private void printSet(BitSet actions, boolean missing) {
		...
		final String[] alphabet = mach.getAlphabet();
	...
	Vector<String> getRootTrace(byte[] root) {
		...
		final Vector<String> trace = mach.getTraceToState(mach.START(), root);
		if (trace == null)
			hasERROR = true;
		return trace;
	}
	...
	Vector<String> getCycleTrace(Vector<byte[]> component, byte[] root) {
		...
		// if (component!=null)
		// output.outln("Component size "+component.size());
		Vector<String> trace = null;
		final MyList transitions = mach.getTransitions(root);
		byte[] cycle = null;
		int act = 0;
		while (!transitions.empty()) {
	...
	private void printTrace(Vector<String> trace) {
		if (trace == null)
			return;
		final Enumeration<String> e = trace.elements();
		while (e.hasMoreElements())
			output.outln("\t" + e.nextElement());
	}

	private boolean stateLabel(int act) {
		final String s = (mach.getAlphabet())[act];
		return s.charAt(0) == '_';
	}
