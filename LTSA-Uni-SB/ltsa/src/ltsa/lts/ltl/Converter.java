...
class Converter extends CompactState {
	...
	public void printFSP(PrintStream printstream) {
		...
		Node node;
		for (Iterator<Node> iterator = g.getNodes().iterator(); iterator
				.hasNext(); printNode(node, printstream)) {
			printstream.println(",");
			node = iterator.next();
		}
	...
	protected BitSet getAcceptance() {
		...
		for (Iterator<Node> iterator1 = g.getNodes().iterator(); iterator1
				.hasNext();) {
