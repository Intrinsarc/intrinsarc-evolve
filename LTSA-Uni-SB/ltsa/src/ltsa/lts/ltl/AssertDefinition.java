...
public class AssertDefinition {
	...
	Hashtable<String, Value> init_params; // initial parameter values name,value
	...
	public static void put(Symbol n, FormulaSyntax f, LabelSet ls,
			Hashtable<String, Value> ip, Vector<String> p, boolean isConstraint) {
		if (definitions == null)
		...
		if (!isConstraint) {
			...
			if (constraints.put(n.toString(), new AssertDefinition(n, f, ls,
					ip, p)) != null)
				Diagnostics.fatal("duplicate LTL constraint definition: " + n,
						n);
		}
	}
	...
	private static void compileAll(
		...
		while (e.hasMoreElements()) {
			...
			p.fac.setFormula(p.ltl_formula.expand(p.fac,
					new Hashtable<String, Value>(), p.init_params));
		}
	}
	...
	public static CompactState compileConstraint(LTSOutput output, Symbol name,
		...
		if (pvalues != null) {
			...
			p.fac.setFormula(p.ltl_formula.expand(p.fac,
					new Hashtable<String, Value>(), actual_params));
		} else {
			p.fac.setFormula(p.ltl_formula.expand(p.fac,
					new Hashtable<String, Value>(), p.init_params));
		}
	...
	private static CompositeState compile(
		...
		output.outln("Formula !" + p.name.toString() + " = "
				+ p.fac.getFormula());
		Vector<String> alpha = p.alphaExtension != null ? p.alphaExtension
				.getActions(null) : null;
		if (alpha == null)
			alpha = new Vector<String>();
		if (addAsterisk && !isconstraint)
			alpha.add("*");
		GeneralizedBuchiAutomata gba = new GeneralizedBuchiAutomata(p.name
				.toString(), p.fac, alpha);
		gba.translate();
		...
		Converter c = new Converter(p.name.toString(), g1, gba
				.getLabelFactory());
		output.outln("Buchi automata:");
		c.printFSP(new PrintStream(baos));
		output.out(baos.toString());
		Vector<CompactState> procs = gba.getLabelFactory().propProcs;
		procs.add(c);
