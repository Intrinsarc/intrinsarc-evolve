...
public class FormulaFactory {
	...
	SortedSet<Formula> props; // stores the set of propositions
	Formula formula;
	Hashtable<String, Vector<String>> actionPredicates;
	private boolean hasNext = false;
	static public boolean normalLTL = true;

	public FormulaFactory() {
		nv = new NotVisitor(this);
		subf = new HashMap<String, Formula>();
		props = new TreeSet<Formula>();
		id = 1;
	...
	public void setFormula(Formula f) { // generate the negation for
		// verification
		formula = makeNot(f);
	}
	...
	public Formula make(ActionLabels act, Hashtable<String, Value> locals,
		...
		if (actionPredicates == null)
			actionPredicates = new Hashtable<String, Vector<String>>();
	...
	public SortedSet<Formula> getProps() {
		return props;
	}
	...
	public Formula make(Formula left, Symbol op, Formula right) {
		...
		case Symbol.NEXTTIME:
			if (normalLTL)
				...
				return makeNext(makeWeakUntil(makeNot(makeTick()), makeAnd(
						makeTick(), right)));
		...
		case Symbol.UNTIL:
			if (normalLTL)
				...
				return makeUntil(makeImplies(makeTick(), left), makeAnd(
						makeTick(), right));
		case Symbol.WEAKUNTIL:
			if (normalLTL)
				return makeWeakUntil(left, right);
			else
				return makeWeakUntil(makeImplies(makeTick(), left), makeAnd(
						makeTick(), right));
	...
	Formula makeAnd(Formula left, Formula right) {
		...
		if ((left instanceof Next) && (right instanceof Next)) // X a && X b -->
			// X(a && b)
			return makeNext(makeAnd(((Next) left).getNext(), ((Next) right)
					.getNext()));
	...
	Formula makeUntil(Formula left, Formula right) {
		...
		if ((left instanceof Next) && (right instanceof Next)) // X a U X b -->
			// X(a U b)
			return makeNext(makeUntil(((Next) left).getNext(), ((Next) right)
					.getNext()));
	...
	int processUntils(Formula f, List<Until> untils) {
		f.accept(new UntilVisitor(this, untils));
		return untils.size();
	}

	boolean specialCaseV(Formula f, Set<Formula> s) {
		Formula ff = makeRelease(False.make(), f);
		return s.contains(ff);
	}

	boolean syntaxImplied(Formula f, SortedSet<Formula> one,
...
class NotVisitor implements Visitor {
	...
	public Formula visit(Until u) {
		return fac.makeRelease(fac.makeNot(u.getLeft()), fac.makeNot(u
				.getRight()));
	}

	public Formula visit(Release r) {
		return fac.makeUntil(fac.makeNot(r.getLeft()), fac
				.makeNot(r.getRight()));
	}
}
...
class UntilVisitor implements Visitor {
	private final List<Until> ll;

	UntilVisitor(FormulaFactory f, List<Until> l) {
		ll = l;
	}
