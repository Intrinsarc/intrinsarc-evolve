...
public class Alphabet {
	...
	public Alphabet(CompactState sm) {
		...
		if (root != null)
			maxLevel = root.maxDepth();
	}

	public Alphabet(String[] inames) {
		final String names[] = new String[inames.length];
	...
	public Alphabet(Vector<String> names) {
...
class PrefixTree {
	...
	private void add(String s, int level) {
		final String ps = prefix(s, level);
		if (ps == null)
			return;
		if (ps.equals(name) && !lastprefix) {
			final String pps = prefix(s, level + 1);
			if (pps == null) {
				lastprefix = true;
				return;
			}
			if (subname == null)
				subname = new PrefixTree(pps);
			subname.add(s, level + 1);
		} else {
			if (list == null)
				list = new PrefixTree(ps);
			list.add(s, level);
		}
		return;
	}
	...
	PrefixTree[] getSubLists() {
		final Vector<PrefixTree> subs = new Vector<PrefixTree>();
		PrefixTree pt = this;
		PrefixTree ptt = list;
		subs.addElement(pt);
		while (ptt != null) {
		...
		final PrefixTree[] sl = new PrefixTree[subs.size()];
		subs.copyInto(sl);
		return sl;
	}
	...
	static String prefix(String s, int level) {
		...
		final int finish = s.indexOf('.', start);
		if (finish < 0)
	...
	public void getStrings(Vector<String> v, int level, String prefix) {
		PrefixTree pt = this;
		while (pt != null) {
			String pre;
			if (prefix == null)
	...
	@Override
	public String toString() {
		final PrefixTree[] subs = getSubLists();
		String s;
		if (subs.length > 2)
			s = "{";
		else
