...
public class Lex {
	...
	private final LTSInput input;
	private Symbol symbol;
	private char ch;
	private boolean eoln;
	private final boolean newSymbols; // true is return new object per symbol
	...
	private void in_number() {
		...
		final StringBuffer realBuf = new StringBuffer(); // holds potential real
		// literals
		...
		/*
		 * if (base == 10) { // determine if real boolean numIsDouble = false;
		 * 
		 * if (ch == '.') { numIsDouble = true; do { realBuf.append (ch);
		 * next_ch (); } while (Character.isDigit (ch)); }
		 * 
		 * if (ch == 'e' || ch == 'E') { numIsDouble = true; realBuf.append
		 * (ch); next_ch ();
		 * 
		 * if (ch == '+' || ch == '-') { realBuf.append (ch); next_ch (); }
		 * 
		 * if (Character.isDigit (ch)) { while (Character.isDigit (ch)) {
		 * realBuf.append (ch); next_ch (); } } else error ("exponent expected
		 * after e or E"); }
		 * 
		 * if (numIsDouble) { try { symbol.doubleValue = Double.valueOf
		 * (realBuf.toString ()).doubleValue (); symbol.kind =
		 * Symbol.DOUBLE_VALUE; } catch (NumberFormatException msg) { error
		 * ("Bad double value. " + msg); } } } else if (ch == 'U' || ch == 'u'
		 * || ch == 'L' || ch == 'U') next_ch ();
		 */
		...
		while (ch == '\\') {
			...
			case 'X':
				...
				if (!isxdigit(ch))
					...
					int hex_digits = 0;
					while (isxdigit(ch) && hex_digits < 2) { // max of two
						// hex digits
						// for IDL
	...
	private void in_identifier() {
		...
		symbol.setString(s);
		final Object kind = SymbolTable.get(s);
		if (kind == null) {
