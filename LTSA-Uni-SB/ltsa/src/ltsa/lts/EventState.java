...
public class EventState {
	...
	public int next;
	public int machine;
	public EventState list; // used to keep list in event order, TAU first
	public EventState nondet;// used for additional non-deterministic
	// transitions
	public EventState path; // used by analyser & by minimiser
	...
	public static EventState add(EventState head, EventState tr) {
		...
		EventState p = head;
		while (p.list != null && p.event != tr.event
				&& tr.event >= p.list.event)
			p = p.list;
	...
	public static EventState offsetSeq(int off, int seq, int max,
			EventState head) {
		EventState p = head;
		while (p != null) {
			EventState q = p;
	...
	public static EventState firstCompState(EventState head, int event,
			int[] state) {
		EventState p = head;
		while (p != null) {
			if (p.event == event) {
	...
	public static EventState addNonDetTau(EventState head, EventState states[],
			BitSet tauOnly) {
		EventState p = head;
		...
		while (p != null) {
			...
			while (q != null) {
				if (q.next > 0 && tauOnly.get(q.next)) {
					final int nextS[] = nextState(states[q.next],
							Declaration.TAU);
	...
	public static EventState renumberEvents(EventState head,
		...
		while (p != null) {
			EventState q = p;
			while (q != null) {
				final int event = oldtonew.get(new Integer(q.event)).intValue();
	...
	public static EventState newTransitions(EventState head,
			Relation<Integer, Integer> oldtonew) {
		EventState p = head;
		EventState newhead = null;
		while (p != null) {
			EventState q = p;
			while (q != null) {
				final Vector<Integer> v = oldtonew.get(new Integer(q.event));
				if (v != null) {
					for (final Enumeration<Integer> e = v.elements(); e
							.hasMoreElements();) {
						newhead = add(newhead, new EventState(e.nextElement()
								.intValue(), q.next));
					}
				}
				q = q.nondet;
	...
	public static EventState renumberStates(EventState head,
			Hashtable<Integer, Integer> oldtonew) {
		EventState p = head;
		EventState newhead = null;
		while (p != null) {
			EventState q = p;
			while (q != null) {
				final int next = q.next < 0 ? Declaration.ERROR : oldtonew.get(
						new Integer(q.next)).intValue();
	...
	public static EventState renumberStates(EventState head, MyIntHash oldtonew) {
		...
		while (p != null) {
			EventState q = p;
			while (q != null) {
				final int next = q.next < 0 ? Declaration.ERROR : oldtonew
						.get(q.next);
				newhead = add(newhead, new EventState(q.event, next));
				q = q.nondet;
			}
	...
	public static EventState actionAdd(EventState head, EventState[] states) {
		...
		while (tau != null) {
			if (tau.next != Declaration.ERROR)
				head = union(head, states[tau.next]);
			tau = tau.nondet;
		}
		return head;
	}
	...
	public static EventState transpose(EventState from) {
		...
		while (p != null) {
			EventState q = p;
			while (q != null) {
				res = add(res, new EventState(q.next, q.event)); // swap event &
				// next
				q = q.nondet;
			}
			p = p.list;
	...
	public static String[] eventsToNextNoAccept(EventState from,
			String[] alphabet) {
		EventState q = from;
		int size = 0;
		while (q != null) {
	...
	public static EventState reachableTau(EventState[] states, int k) {
		...
		if (head == null || head.event != Declaration.TAU)
			return null;
		final BitSet visited = new BitSet(states.length);
		visited.set(k);
	...
	public static MyIntHash reachable(EventState[] states) {
		int ns = 0; // newstate
		final MyIntHash visited = new MyIntHash(states.length);
		EventState stack = null;
		stack = push(stack, new EventState(0, 0));
		while (stack != null) {
			final int v = stack.next;
			stack = pop(stack);
	...
	public static int search(EventState trace, EventState[] states,
			int fromState, int findState, int ignoreState) {
		final EventState zero = new EventState(0, fromState);
		EventState head = zero;
		EventState tail = zero;
		int res = Declaration.SUCCESS;
		final EventState val[] = new EventState[states.length + 1]; // shift by
		// 1 so
		// ERROR is
		// 0
		while (head != null) {
			final int k = head.next;
	...
	/*-------------------------------------------------------------*/
	public static void printPath(EventState head, String[] alpha,
			LTSOutput output) {
		EventState q = head;
		while (q != null) {
