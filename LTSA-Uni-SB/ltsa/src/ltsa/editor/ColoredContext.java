...
public class ColoredContext extends StyleContext implements ViewFactory {
	...
	private static final long serialVersionUID = 7885916158419117911L;
	 ...
	 */
	private class ColoredView extends PlainView {
		private final ColoredScanner lexer;
		...
		/**
		 * Construct a simple colorized view of darwin text.
		 */
		...
		protected int drawUnselectedText(Graphics g, int x, int y,
				int beginPos, int endPos) throws BadLocationException {
			Document document;
		...
		void updateScanner(int pos) {
			try {
				...
				while (lexer.getEndOffset() <= pos) {
					lexer.next();
				}
				// can't adjust scanner... calling logic will simply render the
				// remaining text.
			} catch (Throwable e) {
				e.printStackTrace();
			}
	...
	// CONSTRUCTOR-DESTRUCTOR
	/**
	 * Constructs a set of styles to represent darwin lexical tokens. By default
	 * there are no colors or fonts specified.
	 */
	public ColoredContext() {
		super();
	}
